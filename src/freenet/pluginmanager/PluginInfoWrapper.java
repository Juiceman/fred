...
public class PluginInfoWrapper implements Comparable<PluginInfoWrapper> {
  ...
  /**
   * Tell the plugin to quit. Interrupt it if it's a thread-based plugin which might be sleeping.
   * Then call removePlugin() on it on the manager - either now, if it's threadless, or after it
   * terminates, if it's thread based.
   *
   * @param manager The plugin manager object.
   * @param maxWaitTime If a plugin is thread-based, we can wait for it to terminate. Set to -1 if
   *     you don't want to wait at all, 0 to wait forever or else a value in milliseconds.
   */
   ...
   * FredPluginFCPMessageHandler.ServerSideFCPMessageHandler} of the plugin.<br>
   * <br>
   * TODO: Code quality: Currently, all the other is...() functions are used by PluginManager just
   * to then manually cast the plugin main object to the desired type, i.e. it manually does what
   * the body of this function does. This restricts the API to require the plugin main class to
