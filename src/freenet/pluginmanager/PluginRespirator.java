...
public class PluginRespirator {
   ...
   *
   * @param path The base URI for the page being filtered. Not necessarily a FreenetURI.
   */
  ...
  /**
   * Add a valid form including the {@link NodeClientCore#formPassword}. See the JavaDoc there for
   * an explanation of the purpose of this mechanism.
   *
   ...
   * to a send function throwing {@link IOException}, it <b>must be</b> considered as dead for ever,
  ...
  /**
   * Get the ToadletContainer, which manages HTTP. You can then register toadlets on it, which allow
   * integrating your plugin into the main menus, and are a more versatile interface than
   * FredPluginHTTP.
   */
  ...
  /**
   * Get a new session manager for use with the specified path. See {@link SessionManager} for a
   * detailed explanation of what cookie paths are.
   *
   * <p>The usage of the global "/" path is not allowed. You must use {@link
   * getSessionManager(String cookieNamespace)} if you want your cookie to be valid in the "/" path.
   *
   * <p>This function is synchronized on the SessionManager-list and therefore
   * concurrency-proof. @Deprecated We want cookies to be valid in the "/" path for menus to work
   * even if the user is not in the menu of the given plugin. Therefore, we should use cookie
   * namespaces instead.
   */
  @Deprecated
  public SessionManager getSessionManager(URI cookiePath) {
    synchronized (sessionManagers) {
      for (SessionManager m : sessionManagers) {
        if (m.getCookiePath().equals(cookiePath)) return m;
      }
   ...
   *
   * <p>This function is synchronized on the SessionManager-list and therefore concurrency-proof.
   *
  ...
  /**
   * Get the plugin's SubConfig. If the plugin does not implement FredPluginConfigurable, this will
   * return null.
   */
  ...
  /**
   * Force a write of the plugin's config file. If the plugin does not implement
   * FredPluginConfigurable, don't expect magic to happen.
   */
