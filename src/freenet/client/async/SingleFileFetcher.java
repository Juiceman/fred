...
public class SingleFileFetcher extends SimpleSingleFileFetcher {
	...
	public SingleFileFetcher(
		...
		if (recursionLevel > ctx.maxRecursionLevel) throw new FetchException(
	...
	public void onSuccess(
		...
		if (
			parent instanceof ClientGetter
		...
		if (key instanceof ClientSSK) {
			context.uskManager.checkUSK(
				uri,
				persistent,
				data != null && !block.isMetadata()
			);
		}
		if (data == null) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Block " +
			(block.isMetadata() ? "is metadata" : "is not metadata") +
			" on " +
			this
		);
	...
	private void handleMetadata(Bucket data, ClientContext context) {
		...
		if (parent.isCancelled()) {
			onFailure(
				new FetchException(FetchExceptionMode.CANCELLED),
				false,
				context
			);
			data.free();
		...
		if (data.size() > ctx.maxMetadataSize) {
			onFailure(
				new FetchException(FetchExceptionMode.TOO_BIG_METADATA),
				false,
				context
			);
			data.free();
		...
		try {
			...
			onFailure(
				new FetchException(FetchExceptionMode.INVALID_METADATA, e),
				false,
				context
			);
		...
		while (true) {
			...
			if (metaStrings.size() == 0) {
				if (metadata.hasTopData()) {
					if (
						(metadata.topSize > ctx.maxOutputLength) ||
						(metadata.topCompressedSize > ctx.maxTempLength)
					) {
						// Just in case...
						if (
							metadata.isSimpleRedirect() ||
							metadata.isSplitfile()
			...
			if (metadata.isSimpleManifest()) {
				...
				if (
					...
					if (last == null || !last.isEmpty()) u = u.addMetaStrings(
						new String[] { "" }
					);
					else u = null;
					throw new FetchException(
				...
				if (name == null) {
					...
					if (metadata == null) throw new FetchException(
						FetchExceptionMode.NOT_ENOUGH_PATH_COMPONENTS,
						-1,
						false,
						null,
					...
					if (!persistent) {
						...
						if (
							metadata != null && metadata.isSymbolicShortlink()
						) {
							String oldName = name;
							name = metadata.getSymbolicShortlinkTargetName();
							if (oldName.equals(name)) throw new FetchException(
								FetchExceptionMode.INVALID_METADATA,
								"redirect loop: " + name
							);
							metadata = origMd.getDocument(name);
					...
					if (metadata == null) throw new FetchException(
						FetchExceptionMode.NOT_IN_ARCHIVE,
						"can't find " + name
					);
				}
				continue; // loop
				...
				if (ah == null || !ah.getKey().equals(thisKey)) {
					...
					ah = context.archiveManager.makeHandler(
						thisKey,
						metadata.getArchiveType(),
						metadata.getCompressionCodec(),
						(parent instanceof ClientGetter
								? ((ClientGetter) parent).collectingBinaryBlob()
								: false),
						persistent
					);
				}
				...
				if (metadataBucket != null) {
					try {
						...
						throw new FetchException(
							FetchExceptionMode.NOT_ENOUGH_DISK_SPACE
						);
					} catch (IOException e) {
						// Bucket error?
						throw new FetchException(
							...
							public void notInArchive(ClientContext context) {
								onFailure(
									new FetchException(
										...
										"No metadata in container! Cannot happen as ArchiveManager should synthesise some!"
									),
									false,
									context
								);
				...
				if (ah == null) throw new FetchException(
				...
				if (
					...
					!ctx.allowedMIMETypes.contains(
						clientMetadata.getMIMETypeNoParams()
					)
				) {
					throw new FetchException(
						FetchExceptionMode.WRONG_MIME_TYPE,
						-1,
						false,
						clientMetadata.getMIMEType()
					);
				...
				if (dataBucket != null) {
					...
					try {
						...
						if (persistent) {
							out = context.persistentBucketFactory.makeBucket(
								dataBucket.size()
							);
					...
					fetchArchive(
						...
						new ArchiveExtractCallback() {
							...
							public void gotBucket(
								...
								onSuccess(
									new FetchResult(clientMetadata, data),
									context
								);
							}
				...
				metadata.setSimpleRedirect();
				final SingleFileFetcher f = new SingleFileFetcher(
					this,
					persistent,
					false,
					metadata,
					new MultiLevelMetadataCallback(),
					ctx,
					context
				);
				...
				try {
					...
					else throw new FetchException( // This may be a desirable property. // Without redirects to USKs, all SSK and CHKs are static. // FIXME do we want to allow redirects to USKs?
						FetchExceptionMode.UNKNOWN_METADATA,
						"Redirect to a USK"
					);
				...
				if (
					...
					if (
						clientMetadata != null && !clientMetadata.isTrivial()
					) rcb.onExpectedMIME(clientMetadata, context);
				}
				...
				if (isFinal && !ctx.ignoreTooManyPathComponents) {
					if (!metaStrings.isEmpty()) {
						...
						if (addedMetaStrings > 0) {
							...
							// report to user
							FreenetURI tryURI = uri;
							tryURI = tryURI.dropLastMetaStrings(
								metaStrings.size()
							);
							rcb.onFailure(
								new FetchException(
					...
					this,
					"Not finished: rcb=" + rcb + " for " + this
				);

				final long len = metadata.dataLength();
				final long uncompressedLen = metadata.isCompressed()
					? metadata.uncompressedDataLength()
					: len;

				if (
					(uncompressedLen > ctx.maxOutputLength) ||
				...
				if (isFinal && !parent.isCurrentState(this)) {
					Logger.error(
						this,
						"isFinal but not the current state for " + this,
						new Exception("error")
					);
					reallyFinal = false;
				}
				sf = new SplitFileFetcher(
					metadata,
					rcb,
					parent,
					ctx,
				...
				Logger.error(
					this,
					"Don't know what to do with metadata: " + metadata
				);
	...
	private void addDecompressor(COMPRESSOR_TYPE codec) {
		if (logMINOR) Logger.minor(
			this,
			"Adding decompressor: " + codec + " on " + this,
			new Exception("debug")
		);
	...
	protected void innerWrapHandleMetadata(
		boolean notFinalizedSize,
		ClientContext context
	) {
		try {
			...
			try {
				if (hashes != null) {
					...
					try {
						...
						MultiHashInputStream hasher = new MultiHashInputStream(
							is,
	...
	private static ClientGetState uskCreate(
		...
		if (usk.suggestedEdition >= 0) {
			...
			if (edition <= usk.suggestedEdition) {
				context.uskManager.startTemporaryBackgroundFetcher(
					usk,
					context,
					ctx,
					true,
				...
				if (edition > usk.suggestedEdition) {
					if (logMINOR) Logger.minor(
						SingleFileFetcher.class,
						"Redirecting to edition " + edition
					);
					cb.onFailure(
						new FetchException(
					...
					edition == -1 &&
					context.uskManager.lookupLatestSlot(usk) == -1
				) { // We do not want to be going round and round here!
					// Check the datastore first.
					USKFetcherTag tag = context.uskManager.getFetcher(
					...
					GetCompletionCallback myCB = new USKProxyCompletionCallback(
						usk,
						cb,
						requester.persistent()
					);
					// Want to update the latest known good iff the fetch succeeds.
					SingleFileFetcher sf = new SingleFileFetcher(
						requester,
						myCB,
						null,
						usk.getSSK(),
	...
	public static class MyUSKFetcherCallback
		implements USKFetcherTagCallback, Serializable {
		...
		public void onFailure(ClientContext context) {
			...
			if (e == null) e = new FetchException(
		...
		public void onCancelled(ClientContext context) {
			cb.onFailure(
				new FetchException(FetchExceptionMode.CANCELLED, (String) null),
				null,
				context
			);
		}
