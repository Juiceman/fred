...
public class USKManager {
	...
	public USKFetcherTag getFetcherForInsertDontSchedule(
		...
		FetchContext fctx = ignoreUSKDatehints
			? backgroundFetchContextIgnoreDBR
			: backgroundFetchContext;
		return getFetcher(
			usk,
			persistent
				? new FetchContext(fctx, FetchContext.IDENTICAL_MASK)
				: fctx,
			true,
			client.persistent(),
			client.realTimeFlag(),
	...
	public void hintUpdate(USK usk, long edition, ClientContext context) {
		...
		final ClientGetter get = new ClientGetter(
	...
	public void hintUpdate(
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Cannot start hint fetch for " + uri + " : " + e,
				e
			);
			// Ignore
		}
		...
		synchronized (this) {
			...
			if (f == null) {
				f = new USKFetcher(
					usk,
					this,
					fctx.ignoreUSKDatehints
						? backgroundFetchContextIgnoreDBR
						: backgroundFetchContext,
					new USKFetcherWrapper(
						usk,
						RequestStarter.UPDATE_PRIORITY_CLASS,
						realTimeFlag ? rcRT : rcBulk
					),
					3,
					false,
					false,
			...
			if (prefetchContent) {
				...
				if (slot > -1 && good != slot) fetchTime =
					System.currentTimeMillis();
				temporaryBackgroundFetchersPrefetch.put(clear, fetchTime);
				if (logMINOR) Logger.minor(
					this,
					"Prefetch: set " + fetchTime + " for " + clear
				);
				schedulePrefetchChecker();
			}
			temporaryBackgroundFetchersLRU.push(clear, f);
			while (
				temporaryBackgroundFetchersLRU.size() >
				NodeClientCore.getMaxBackgroundUSKFetchers()
			) {
				USKFetcher fetcher = temporaryBackgroundFetchersLRU.popValue();
				temporaryBackgroundFetchersPrefetch.remove(
					fetcher.getOriginalUSK().clearCopy()
				);
				if (!fetcher.hasSubscribers()) {
					if (toCancel == null) toCancel = new ArrayList<USKFetcher>(
						2
					);
			...
			executor.execute(
				new Runnable() {
					...
					public void run() {
						...
						if (scheduleMe != null) scheduleMe.schedule(
							USKManager.this.context
						);
					}
				}
			);
		...
		public void run() {
			...
			synchronized (USKManager.this) {
				for (Map.Entry<
					USK,
					Long
				> entry : temporaryBackgroundFetchersPrefetch.entrySet()) {
					empty = false;
					if (
						entry.getValue() > 0 &&
						now - entry.getValue() >= PREFETCH_DELAY
					) {
						if (toFetch == null) toFetch = new ArrayList<USK>();
						USK clear = entry.getKey();
						long l = lookupLatestSlot(clear);
						if (lookupKnownGood(clear) < l) toFetch.add(
							clear.copy(l)
						);
			...
			for (final USK key : toFetch) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Prefetching content for background fetch for edition " +
					l +
					" on " +
					key
				);
				FetchContext fctx = new FetchContext(
					realFetchContext,
					FetchContext.IDENTICAL_MASK
				);
				final ClientGetter get = new ClientGetter(
					new ClientGetCallback() {
						...
						public void onFailure(
							...
							if (e.newURI != null) {
								if (logMINOR) Logger.minor(
									this,
									"Prefetch succeeded with redirect for " +
									key
								);
	...
	void updateKnownGood(
		...
		if (logMINOR) Logger.minor(
			this,
			"Updating (known good) " + origUSK.getURI() + " : " + number
		);
		...
		if (callbacks != null) {
			...
			for (final USKCallback callback : callbacks) context.mainExecutor.execute(
				new Runnable() {
					@Override
					public void run() {
	...
	void updateSlot(
		...
		if (logMINOR) Logger.minor(
			this,
			"Updating (slot) " + origUSK.getURI() + " : " + number
		);
		USK clear = origUSK.clearCopy();
		final USKCallback[] callbacks;
		synchronized (this) {
			...
			if (temporaryBackgroundFetchersPrefetch.containsKey(clear)) {
				temporaryBackgroundFetchersPrefetch.put(
					clear,
					System.currentTimeMillis()
				);
				schedulePrefetchChecker();
	...
	public void subscribe(
		...
		if (client.persistent()) throw new UnsupportedOperationException(
			"USKManager subscriptions cannot be persistent"
		);
		USKFetcher sched = null;
		...
		if (ed < 0) {
			Logger.error(
				this,
				"Subscribing to USK with negative edition number: " + ed
			);
			ed = -ed;
		}
		long curEd;
		...
		if (goodEd > ed) cb.onFoundEdition(
			goodEd,
			origUSK.copy(curEd),
			context,
			false,
		...
		else if (curEd > ed) cb.onFoundEdition(
			curEd,
			origUSK.copy(curEd),
			context,
			false,
	...
	public void unsubscribe(USK origUSK, USKCallback cb) {
		...
		synchronized (this) {
			...
			if (newCallbacks.length > 0) subscribersByClearUSK.put(
				clear,
				newCallbacks
			);
			else {
		...
		if (toCancel != null) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Not found unsubscribing: " + cb + " for " + origUSK
			);
		}
	}
	...
	public USKRetriever subscribeContent(
		...
		if (runBackgroundFetch) {
			USKSparseProxyCallback proxy = new USKSparseProxyCallback(
				ret,
				origUSK
			);
			ret.setProxy(proxy);
	...
	public void unsubscribeContent(
		USK origUSK,
		USKRetriever ret,
		boolean runBackgroundFetch
	) {
			...
			if (backgroundFetchersByClearUSK.get(clear) == fetcher) {
				...
				if (!ignoreError) {
					...
					Logger.error(
						...
						"onCancelled for " +
						fetcher +
						" - was still registered, how did this happen??",
						new Exception("debug")
					);
				}
			}
	...
	public void checkUSK(
		...
		try {
			...
			if (!isMetadata) context.uskManager.updateKnownGood(
				usk,
				uu.getSuggestedEdition(),
				context
			);
			else context.uskManager.updateSlot( // FIXME add a callback so if the rest of the request completes we updateKnownGood(). // We don't know whether the metadata is fetchable.
				usk,
				uu.getSuggestedEdition(),
				context
			);
