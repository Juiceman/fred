...
public class ClientGetter
	...
	public boolean start(
		...
		if (logMINOR) Logger.minor(
			this,
			"Starting " + this + " persistent=" + persistent() + " for " + uri
		);
		try {
			...
			synchronized (this) {
				...
				if (!resumedFetcher) {
					...
					finalBlocksTotal = 0;
					resetBlocks();
					currentState = SingleFileFetcher.create(
						this,
						this,
						uri,
						ctx,
			...
			if (currentState != null && !finished) {
				if (
					initialMetadata != null &&
					currentState instanceof SingleFileFetcher &&
					!resumedFetcher
				) {
					((SingleFileFetcher) currentState).startWithMetadata(
							initialMetadata,
							context
						);
	...
	public void onSuccess(
		...
		try {
			if (
				binaryBlobWriter != null && !dontFinalizeBlobWriter
		...
		String mimeType = clientMetadata == null
			? null
			...
			if (mimeType == null) {
				onFailure(
					new FetchException(
						...
						"No MIME type but need specific extension \"" +
		...
		try {
			if (returnBucket == null) finalResult = context
				.getBucketFactory(persistent())
				.makeBucket(maxLen);
			else finalResult = returnBucket;
			if (logMINOR) Logger.minor(
				this,
				"Writing final data to " +
				finalResult +
				" return bucket is " +
				returnBucket
			);
			...
			if (decompressors != null) {
				...
				decompressorManager = new DecompressorThreadManager(
					dataInput,
					decompressors,
					maxLen
				);
			...
			worker = new ClientGetWorkerThread(
			...
			if (logMINOR) Logger.minor(
				this,
				"Waiting for hashing, filtration, and writing to finish"
			);
			worker.waitFinished();
			...
			if (decompressorManager != null) {
				if (logMINOR) Logger.minor(
					this,
					"Waiting for decompression to finalize"
				);
			...
			Logger.normal(
				this,
				"Error filtering content: will not validate",
				e
			);
			ex = e.createFetchException(
				ctx.overrideMIME != null ? ctx.overrideMIME : expectedMIME,
				expectedSize
			);
			...
			Logger.error(
				this,
				"URISyntaxException converting a FreenetURI to a URI!: " + e,
				e
			);
	...
	public void onSuccess(
		...
		Logger.normal(
			this,
			"Succeeding via truncation from " +
			tempFile +
			" to " +
			completionFile
		);
		...
		try {
			...
			if (raf.length() < length) throw new IOException(
				"File is shorter than target length " + length
			);
			...
			if (
				!FileUtil.renameTo(tempFile, completionFile)
			...
			Logger.error(
				this,
				"Failed while completing via truncation: " + e,
				e
			);
			ex = new FetchException(FetchExceptionMode.BUCKET_ERROR, e);
		} catch (URISyntaxException e) {
			Logger.error(
				this,
				"Impossible failure while completing via truncation: " + e,
				e
			);
			ex = new FetchException(FetchExceptionMode.INTERNAL_ERROR, e);
		} catch (FetchException e) {
	...
	public void onFailure(
		...
		if (logMINOR) Logger.minor(
			this,
			"Failed from " + state + " : " + e + " on " + this,
			e
		);
		ClientGetState oldState = null;
		if (
			expectedSize > 0 && (e.expectedSize <= 0 || finalBlocksTotal != 0)
			...
			if (e.finalizedSize()) {
				...
				if (mime != null && !mime.isEmpty()) {
					...
					UnsafeContentTypeException unsafe =
						ContentFilter.checkMIMEType(mime);
					if (unsafe != null) {
		...
		while (true) {
			if (e.mode == FetchExceptionMode.ARCHIVE_RESTART) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Archive restart on " + this + " ar=" + ar
				);
				if (ar > ctx.maxArchiveRestarts) e = new FetchException(
					FetchExceptionMode.TOO_MANY_ARCHIVE_RESTARTS
				);
				else {
					try {
						start(context);
			...
			if (!alreadyFinished) {
				try {
					...
					if (e.mode != FetchExceptionMode.CANCELLED && !force) e =
						new FetchException(
							FetchExceptionMode.BUCKET_ERROR,
							"Failed to close binary blob stream: " + ioe
						);
			...
			if (e.errorCodes != null && e.errorCodes.isOneCodeOnly()) e =
				new FetchException(e.errorCodes.getFirstCodeFetch());
			if (
				e.mode == FetchExceptionMode.DATA_NOT_FOUND &&
			...
			if (!alreadyFinished) clientCallback.onFailure(
				e1,
				ClientGetter.this
			);
			return;
	...
	public boolean canRestart() {
		if (currentState != null && !finished) {
			if (logMINOR) Logger.minor(
				this,
				"Cannot restart because not finished for " + uri
			);
			return false;
		}
		return true;
	 ...
	 */
	protected void addKeyToBinaryBlob(
		...
		try {
			...
			Logger.error(
				this,
				"Failed to write key to binary blob stream: " + e,
				e
			);
			onFailure(
				new FetchException(
			...
			Logger.error(
				this,
				"Failed to write key to binary blob stream (already closed??): " +
				e,
				e
			);
			onFailure(
	...
	public void onExpectedMIME(
		...
		if (ctx.filterData) {
			...
			if (forceCompatibleExtension != null) checkCompatibleExtension(
				mime
			);
		}
		context
	...
	private void checkCompatibleExtension(String mimeType)
		...
		if (type == null) return; // Not our problem, will be picked up elsewhere.
		if (
		...
		context
			...
			.queueNormalOrDrop(
				new PersistentJob() {
					...
					public boolean run(ClientContext context) {
						ctx.eventProducer.produceEvent(
							new ExpectedFileSizeEvent(size),
							context
						);
						return false;
					}
	...
	public void onHashes(HashResult[] hashes, ClientContext context) {
		synchronized (this) {
			if (this.hashes != null) {
				if (!HashResult.strictEquals(hashes, this.hashes)) Logger.error(
					this,
					"Two sets of hashes?!"
				);
				return;
	...
	public void enterCooldown(
		...
		if (wakeupTime == Long.MAX_VALUE) {
			...
			// Already off-thread.
			ctx.eventProducer.produceEvent(
				new EnterFiniteCooldownEvent(wakeupTime),
				context
			);
		}
	}
	...
	public byte[] getClientDetail(ChecksumChecker checker) throws IOException {
		if (clientCallback instanceof PersistentClientCallback) {
			return getClientDetail(
				(PersistentClientCallback) clientCallback,
				checker
			);
	 ...
	 * @throws IOException
	 */
	public boolean writeTrivialProgress(DataOutputStream dos)
		throws IOException {
		if (
	...
	public boolean resumeFromTrivialProgress(
		DataInputStream dis,
		ClientContext context
	) throws IOException {
		if (dis.readBoolean()) {
			try {
				...
				Logger.error(
					this,
					"Failed to restore from splitfile, restarting: " + e,
					e
				);
				return false;
