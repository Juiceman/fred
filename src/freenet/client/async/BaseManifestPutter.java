 ...
 *
 * @see freenet.client.async.PlainManifestPutter PlainManifestPutter,
 *     freenet.client.async.DefaultManifestPutter DefaultManifestPutter
 */
public abstract class BaseManifestPutter extends ManifestPutter {
   ...
   * a container (for example a directory with brazilion files it) Archives are always inserted as
   * CHK, references to items in it are normal redirects to CHK@blah,blub,AA/nameinarchive
   */
      ...
      if (freeformMode) {
        ...
        synchronized (BaseManifestPutter.this) {
          ...
          if (!allMetadatas) {
            if (logMINOR)
              Logger.minor(
                  this, "Still waiting for metadata: " + putHandlersWaitingForMetadata.size());
          }
        }
        if (allMetadatas) {
          // Will resolve etc.
  ...
  private final class JokerPutHandler extends PutHandler {
    ...
    public JokerPutHandler(
      ...
      // we dont know the final uri, so preconstructing the metadata does not help here
      // Metadata m = new Metadata(Metadata.SIMPLE_REDIRECT, null, null, FreenetURI.EMPTY_CHK_URI,
      // cm2);
    }
  ...
  abstract class PutHandler extends BaseClientPutter implements PutCompletionCallback {
      ...
      if (runningMap != null) {
        synchronized (runningMap) {
          if (runningMap.contains(this)) {
            Logger.error(
                this, "PutHandler already in 'runningMap': " + runningMap, new Error("error"));
          } else {
            runningMap.add(this);
          }
        }
    ...
    public void start(ClientContext context) throws InsertException {
      ...
      if (logDEBUG) Logger.debug(this, "Starting a PutHandler for '" + this.itemName + "' " + this);
  ...
  public void start(ClientContext context) throws InsertException {
    ...
    try {
      for (int i = 0; i < running.length; i++) {
        ...
        if (logMINOR) Logger.minor(this, "Started " + i + " of " + running.length);
        if (isFinished()) {
          if (logMINOR) Logger.minor(this, "Already finished, killing start() on " + this);
          return;
        }
      }
      ...
      if (containerMode) {
        for (int i = 0; i < containers.length; i++) {
          ...
          if (logMINOR) Logger.minor(this, "Started " + i + " of " + containers.length);
          if (isFinished()) {
            if (logMINOR) Logger.minor(this, "Already finished, killing start() on " + this);
            return;
          }
        ...
        if (logMINOR)
          Logger.minor(
              this, "Started " + containers.length + " PutHandler's (containers) for " + this);
      }
  ...
  private void resolveAndStartBase(ClientContext context) {
    ...
    try {
      ...
      if (logMINOR)
        Logger.minor(
            this, "Inserting main metadata: " + rootMetaPutHandler + " for " + baseMetadata);
