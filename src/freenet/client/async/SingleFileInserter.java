...
class SingleFileInserter implements ClientPutState, Serializable {
	...
	SingleFileInserter(BaseClientPutter parent, PutCompletionCallback cb, InsertBlock block,
					   ...
					   boolean freeData, String targetFilename, boolean forSplitfile, boolean persistent,
					   long origDataLength, long origCompressedDataLength, HashResult[] origHashes, byte cryptoAlgorithm,
					   byte[] forceCryptoKey, long metadataThreshold) {
	...
	void onCompressed(CompressionOutput output, ClientContext context) {
		...
		try {
			...
			cb.onFailure(new InsertException(InsertExceptionMode.INTERNAL_ERROR, t, null),
						 SingleFileInserter.this, context);
		}
	}
	...
	void onCompressedInner(CompressionOutput output, ClientContext context) throws InsertException {
		...
		if(bestCodec != null) {
			if(logMINOR) {
				Logger.minor(this, "The best compression algorithm is "+bestCodec+ " we have gained"+ (100-
							 (bestCompressedDataSize*100/origSize)) +"% ! ("+origSize+'/'+bestCompressedDataSize+')');
			}
		...
		if(parent == cb) {
			...
			ctx.eventProducer.produceEvent(new FinishedCompressionEvent(codecID, origSize,
										   bestCompressedDataSize), context);
			if(logMINOR) {
		...
		boolean fitsInOneBlockAsIs = bestCodec == null ? compressedDataSize <= blockSize :
									 compressedDataSize <= oneBlockCompressedSize;
		boolean fitsInOneCHK = bestCodec == null ? compressedDataSize <= CHKBlock.DATA_LENGTH :
							   compressedDataSize <= CHKBlock.MAX_COMPRESSED_DATA_LENGTH;

		if((fitsInOneBlockAsIs || fitsInOneCHK) && origSize > Integer.MAX_VALUE) {
			throw new InsertException(InsertExceptionMode.INTERNAL_ERROR,
									  "2GB+ should not encode to one block!", null);
		}
		...
		if((noMetadata || metadata) && archiveType == null) {
			if(fitsInOneBlockAsIs) {
				...
				// Just insert it
				ClientPutState bi =
					createInserter(parent, data, codecNumber, ctx, cb, metadata, (int)origSize, -1, true, context,
								   shouldFreeData, forSplitfile);
				if(logMINOR) {
		...
		if (fitsInOneCHK) {
			...
			if(reportMetadataOnly) {
				SingleBlockInserter dataPutter = new SingleBlockInserter(parent, data, codecNumber,
						FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag, cb, metadata, (int)origSize, -1, true, true, token,
						context, persistent, shouldFreeData,
						forSplitfile ? ctx.extraInsertsSplitfileHeaderBlock : ctx.extraInsertsSingleBlock, cryptoAlgorithm,
						forceCryptoKey);
				if(logMINOR) {
				...
				SingleBlockInserter dataPutter = new SingleBlockInserter(parent, data, codecNumber,
						FreenetURI.EMPTY_CHK_URI, ctx, realTimeFlag, mcb, metadata, (int)origSize, -1, true, false, token,
						context, persistent, shouldFreeData,
						forSplitfile ? ctx.extraInsertsSplitfileHeaderBlock : ctx.extraInsertsSingleBlock, cryptoAlgorithm,
						forceCryptoKey);
				if(logMINOR) {
				...
				try {
					...
					Logger.error(this, "Caught "+e, e);
					throw new InsertException(InsertExceptionMode.INTERNAL_ERROR,
											  "Got MetadataUnresolvedException in SingleFileInserter: "+e.toString(), null);
				}
				ClientPutState metaPutter = createInserter(parent, metadataBucket, (short) -1, ctx, mcb, true,
											(int)origSize, -1, true, context, true, false);
				if(logMINOR) {
		...
		if(reportMetadataOnly) {
			...
			boolean allowSizes = (cmode == CompatibilityMode.COMPAT_CURRENT
	...
	private RandomAccessBucket fixNotPersistent(RandomAccessBucket data,
	...
	private void tryCompress(ClientContext context) throws InsertException {
		...
		long wantHashes = 0;
		CompatibilityMode cmode = ctx.getCompatibilityMode();
		boolean atLeast1254 = (cmode == CompatibilityMode.COMPAT_CURRENT
							   || cmode.ordinal() >= CompatibilityMode.COMPAT_1255.ordinal());
		if(atLeast1254) {
		...
		if(tryCompress) {
			InsertCompressor.start(context, this, origData, oneBlockCompressedSize,
								   context.getBucketFactory(persistent), persistent, wantHashes, !atLeast1254, context.getConfig());
		} else {
			if(logMINOR) {
	...
	private Metadata makeMetadata(ARCHIVE_TYPE archiveType, FreenetURI uri, HashResult[] hashes) {
		...
		if(archiveType != null) {
			meta = new Metadata(DocumentType.ARCHIVE_MANIFEST, archiveType, null, uri, block.clientMetadata,
								data, compressed, req, total, topDontCompress, topCompatibilityMode, hashes);
		} else { // redirect
			meta = new Metadata(DocumentType.SIMPLE_REDIRECT, archiveType, null, uri, block.clientMetadata,
								data, compressed, req, total, topDontCompress, topCompatibilityMode, hashes);
		}
	...
	private ClientPutState createInserter(BaseClientPutter parent, Bucket data, short compressionCodec,
										  ...
										  boolean addToParent, ClientContext context, boolean freeData,
										  boolean forSplitfile) throws InsertException {
		...
		if(uri.getKeyType().equals("USK")) {
			try {
				return new USKInserter(parent, data, compressionCodec, uri, ctx, cb, isMetadata, sourceLength,
									   token,
									   addToParent, this.token, context, freeData, persistent, realTimeFlag,
									   forSplitfile ? ctx.extraInsertsSplitfileHeaderBlock : ctx.extraInsertsSingleBlock, cryptoAlgorithm,
									   forceCryptoKey);
			...
			SingleBlockInserter sbi =
				new SingleBlockInserter(parent, data, compressionCodec, uri, ctx, realTimeFlag, cb, isMetadata,
										sourceLength, token,
										addToParent, false, this.token, context, persistent, freeData,
										forSplitfile ? ctx.extraInsertsSplitfileHeaderBlock : ctx.extraInsertsSingleBlock, cryptoAlgorithm,
										forceCryptoKey);
			// pass uri to SBI
			block.nullURI();
	...
	public class SplitHandler implements PutCompletionCallback, ClientPutState, Serializable {
			...
			synchronized(this) {
				...
				if(state == sfi) {
					...
					if(!metaInsertSuccess && !metaInsertStarted) {
						...
						sfi = null;
						if(logMINOR) {
							Logger.minor(this, "Metadata already started for "+this+" : success="+metaInsertSuccess+" started="
										 +metaInsertStarted);
						}
					}
		...
		public void onFailure(InsertException e, ClientPutState state, ClientContext context) {
			...
			synchronized(this) {
				if(logMINOR) {
					Logger.minor(this, "onFailure(): "+e+" on "+state+" on "+this+" sfi = "+sfi+" metadataPutter = "
								 +metadataPutter);
				}
				if(state == sfi) {
					sfi = null;
		...
		public void onMetadata(Metadata meta, ClientPutState state, ClientContext context) {
			...
			synchronized(this) {
				...
				if(reportMetadataOnly) {
					...
					e = new InsertException(InsertExceptionMode.INTERNAL_ERROR,
											"Did not expect to get metadata for metadata inserter", null);
				} else if(state != sfi) {
					Logger.error(this, "Got metadata from unknown state "+state+" sfi="+sfi+" metadataPutter="
								 +metadataPutter+" on "+this+" persistent="+persistent, new Exception("debug"));
					e = new InsertException(InsertExceptionMode.INTERNAL_ERROR, "Got metadata from unknown state",
											null);
				} else {
			...
			try {
				...
				fail((InsertException)new InsertException(InsertExceptionMode.INTERNAL_ERROR,
						"MetadataUnresolvedException in SingleFileInserter.SplitHandler: "+e1, null).initCause(e1),
					 context);
				return;
			}
			...
			if(targetFilename != null) {
				...
				if(metaBytes.length <= Short.MAX_VALUE) {
					...
					try {
						...
						fail((InsertException)new InsertException(InsertExceptionMode.INTERNAL_ERROR,
								"MetadataUnresolvedException in SingleFileInserter.SplitHandler(2): "+e1, null).initCause(e1),
							 context);
						return;
					}
				}
			...
			synchronized(this) {
				...
				metadataPutter = new SingleFileInserter(parent, this, newBlock, true, ctx, realTimeFlag, false,
														false, token, archiveType, true, metaPutterTargetFilename, true, persistent, origDataLength,
														origCompressedDataLength, origHashes, cryptoAlgorithm, forceCryptoKey, metadataThreshold);
				if(origHashes != null) {
				...
				if(logMINOR) {
					Logger.minor(this, "Created metadata putter for "+this+" : "+metadataPutter+" bucket "
								 +metadataBucket+" size "+metadataBucket.size());
				}
			...
			if(logMINOR) {
				Logger.minor(this, "Putting metadata on "+metadataPutter+" from "+sfi+" ("+((
								 SplitFileInserter)sfi).getLength()+ ')');
			}
			if(!startMetadata(context)) {
				Logger.error(this, "onMetadata() yet unable to start metadata due to not having all URIs?!?!");
				fail(new InsertException(InsertExceptionMode.INTERNAL_ERROR,
										 "onMetadata() yet unable to start metadata due to not having all URIs", null), context);
				return;
			}
			synchronized(this) {
		...
		public void onMetadata(Bucket meta, ClientPutState state, ClientContext context) {
			...
			synchronized(this) {
				...
				if(state == metadataPutter) {
					...
					if(metadataPutter != null) {
						Logger.error(this, "Got metadata from "+sfi
									 +" even though already started inserting metadata on the next layer on "+this+" !!");
						freeIt = true;
					} else {
						// Okay, return it.
