...
public class SplitFileFetcherCrossSegmentStorage {
  ...
  private synchronized void tryDecodeOrEncode(final short prio) {
    ...
    if (cancelled) return;
    long limit =
        totalBlocks * CHKBlock.DATA_LENGTH
            + Math.max(
  ...
  private void innerDecode(MemoryLimitedChunk chunk) throws IOException {
    ...
    if (realTotalDataBlocks < dataBlockCount) {
      // Decode.
      codec.decode(
          dataBlocks, checkBlocks, dataBlocksFound, checkBlocksFound, CHKBlock.DATA_LENGTH);
   ...
   *     arrays).
   * @return An array of blocks, in the correct order. Each element is either a valid block or null
   *     if the block is invalid or hasn't been fetched yet. Will tell the ordinary segment if the
   *     block is bogus. Will also update our blocksFound.
   */
  ...
  /**
   * Should be called before scheduling, unlike restart(). Doesn't lock, i.e. part of construction.
   * But we must have read metadata on the regular segments first, which won't be true in the
   * constructor.
   */
