...
public abstract class PersistentJobRunnerImpl implements PersistentJobRunner {
	...
	public PersistentJobRunnerImpl(
		Executor executor,
		Ticker ticker,
		long interval
	) {
	...
	public void queueInternal(PersistentJob job, int threadPriority)
		...
		synchronized (sync) {
			...
			if (writing) {
				Logger.error(
					this,
					"Internal job must not be queued during writing! They should have finished before we start writing and cannot be started \"externally\"!",
					new Exception("error")
				);
				queuedJobs.add(new QueuedJob(job, threadPriority));
			} else {
	...
	public void queueInternal(PersistentJob job) {
		try {
			...
			Logger.error(
				this,
				"Dropping internal job because persistence has been turned off!: " +
				e,
				e
			);
		}
	...
	public void handleCompletion(boolean ret, int threadPriority) {
		synchronized (sync) {
			...
			if (!mustCheckpoint) {
				if (
					System.currentTimeMillis() - lastCheckpointed >
					checkpointInterval
				) {
					mustCheckpoint = true;
	...
	private void checkpoint(boolean shutdown) {
		...
		synchronized (sync) {
			...
			writing = false;
			QueuedJob[] jobs = queuedJobs.toArray(
				new QueuedJob[queuedJobs.size()]
			);
			if (logDEBUG) Logger.debug(
				this,
				"Starting " + jobs.length + " queued jobs"
			);
			for (QueuedJob job : jobs) {
				runningJobs++;
				executor.execute(
					new JobRunnable(job.job, job.threadPriority, context)
				);
			}
