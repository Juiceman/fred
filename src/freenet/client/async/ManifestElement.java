...
public class ManifestElement implements Serializable {
	...
	public freenet.support.api.ManifestElement migrate(BucketFactory bf,
			ClientContext context) throws ResumeFailedException, IOException {
		if(data == null) {
			if(targetURI == null) {
			...
			if(data.size() != dataSize) {
				throw new ResumeFailedException("Bucket in site insert changed size from "+dataSize+" to "
												+data.size());
			}
			data.onResume(context);
			RandomAccessBucket convertedData = BucketTools.toRandomAccessBucket(data, bf);
			return new freenet.support.api.ManifestElement(name, fullName, convertedData, mimeOverride,
					dataSize);
		}
	}
