...
public class MultiPutCompletionCallback implements PutCompletionCallback, ClientPutState,
	...
	public MultiPutCompletionCallback(PutCompletionCallback cb, BaseClientPutter parent, Object token,
									  boolean persistent, boolean collisionIsOK) {
		this(cb, parent, token, persistent, collisionIsOK, false);
	}

	public MultiPutCompletionCallback(PutCompletionCallback cb, BaseClientPutter parent, Object token,
									  boolean persistent, boolean collisionIsOK, boolean finishOnFailure) {
		this.cb = cb;
	...
	private void complete(InsertException e, ClientContext context) {
		synchronized(this) {
			...
			if(e != null && this.e != null && this.e != e) {
				if(e.getMode() ==
						InsertExceptionMode.CANCELLED) { // Cancelled is okay, ignore it, we cancel after failure sometimes.
		...
		synchronized(this) {
			...
			if(encodedKey != null) {
				if(key.equals(encodedKey)) {
					return;    // Squash duplicated call to onEncode().
				} else {
					Logger.error(this, "Encoded twice with different keys for "+this+" : "+encodedKey+" -> "+key);
				}
			}
			encodedKey = key;
		}
