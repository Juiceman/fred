...
public class PNGFilter implements ContentDataFilter {
	...
	public void readFilter(
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Writing the PNG header to the output bucket"
			);
			...
			while (!hasSeenIEND) {
				...
				for (int i = 0; i < 4; i++) {
					...
					if ((val >= 65 && val <= 99) || (val >= 97 && val <= 122)) {
						...
						String chunkName = HexUtil.bytesToHex(
							lengthBytes,
							0,
							4
						);
				...
				if (length > 0) {
					...
					offset += length;
					if (logMINOR) if (logDEBUG) Logger.minor(
						this,
						"data (offset=0x" +
						Long.toHexString(offset) +
						") " +
				...
				if (logMINOR) Logger.minor(
					this,
					"CRC offset=0x" + Long.toHexString(offset)
				);
				if (dos != null) dos.write(crcLengthBytes);

				if (checkCRCs) {
					long readCRC =
						(((crcLengthBytes[0] & 0xff) << 24) +
				...
				if (!skip && "IHDR".equals(chunkTypeString)) { // http://www.w3.org/TR/PNG/#11IHDR
					if (hasSeenIHDR) throwError(
						"Duplicate IHDR",
						"Two IHDR chunks detected!!"
					);
					if (length != 13) throwError(
						"IHDR length!= 13",
						"The length of the IHDR file is not 13"
					);
					long width =
						((chunkData[0] & 0xff) << 24) +
						((chunkData[1] & 0xff) << 16) +
					...
					long height =
						...
						((chunkData[6] & 0xff) << 8) +
						(chunkData[7] & 0xff);
					if (width < 1 || height < 1) throwError(
						"Width or Height is invalid",
						"Width or Height is invalid (<1)"
					);
					...
					if (compressionMethod != 0) throwError(
						"Invalid CompressionMethod",
						"Invalid CompressionMethod! " + compressionMethod
					);
					int filterMethod = chunkData[11];
					if (filterMethod != 0) throwError(
						"Invalid FilterMethod",
						"Invalid FilterMethod! " + filterMethod
					);
					int interlaceMethod = chunkData[12];
					if (interlaceMethod < 0 || interlaceMethod > 1) throwError(
						"Invalid InterlaceMethod",
						"Invalid InterlaceMethod! " + interlaceMethod
					);
				...
				if (!skip && "IEND".equals(chunkTypeString)) {
					if (hasSeenIEND) throwError(
						"Two IEND chunks detected!!",
						"Two IEND chunks detected!!"
					);
					hasSeenIEND = true;
					validChunkType = true;
				...
				if (!skip && "IDAT".equalsIgnoreCase(chunkTypeString)) {
					if (
						hasSeenIDAT && !"IDAT".equalsIgnoreCase(lastChunkType)
					) throwError(
				...
				if (
					...
					deleteTimestamp && "time".equalsIgnoreCase(chunkTypeString)
				) {
				...
				if (!validChunkType) {
					if (logMINOR) Logger.minor(
						this,
						"Skipping unknown chunk type " + chunkTypeString
					);
					skip = true;
	...
	private void throwOnInvalidColour(int bitDepth, int colourType)
		throws DataFilterException {
		switch (bitDepth) {
			case 1:
			case 2:
			case 4:
				if (colourType != 0 && colourType != 3) throwError(
	...
	private void throwError(String shortReason, String reason)
		throws DataFilterException {
		// Throw an exception
		String message = "Invalid PNG";
		if (reason != null) message += ' ' + reason;
		if (shortReason != null) message += " - " + shortReason;
		DataFilterException e = new DataFilterException(
			shortReason,
			shortReason,
			message
		);
