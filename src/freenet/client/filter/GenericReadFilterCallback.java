  ...
  public String processURI(String u, String overrideType, boolean forBaseHref, boolean inline)
    ...
    try {
      ...
      if (u.startsWith("/") || u.startsWith("%2f"))
        ...
        // Don't allow encoded /'s, they're just too confusing (here they would get decoded and then
        // coalesced with other slashes).
        noRelative = true;
  ...
  private String finishProcess(
    ...
    if (typeOverride != null) {
      ...
      if (split[1] != null) {
        ...
        if (charset != null) typeOverride = split[0] + "; charset=" + charset;
  ...
  public String processForm(String method, String action) throws CommentException {
    ...
    try {
      ...
      if (uri.getScheme() != null
          || uri.getHost() != null
          || uri.getPort() != -1
          || uri.getUserInfo() != null) throw new CommentException(l10n("invalidFormURI"));
      String path = uri.getPath();
      if (path.startsWith(PLUGINS_PREFIX)) {
        ...
        if (after.matches("[A-Za-z0-9\\.]+")) return uri.toASCIIString();
      }
    } catch (URISyntaxException e) {
