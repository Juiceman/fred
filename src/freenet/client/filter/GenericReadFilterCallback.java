...
public class GenericReadFilterCallback implements FilterCallback, URIProcessor {
	...
	private void setStrippedURI(String u) {
		...
		if (idx > 0) {
			...
			try {
				...
				Logger.error(
					this,
					"Can't strip base URI: " + e + " parsing " + u
				);
				strippedBaseURI = baseURI;
			}
		} else strippedBaseURI = baseURI;
	}
	...
	public String processURI(
		...
		try {
			...
			throw new CommentException(
				l10n("couldNotParseURIWithError", "error", e1.getMessage())
			);
		}
		...
		if (
			...
			try {
				...
				Logger.error(
					this,
					"URI " + uri + " looked like localhost but could not parse",
					e
				);
				throw new CommentException(
					"URI looked like localhost but could not parse: " + e
				);
			}
			host = null;
		}
		...
		if (host == null) {
			...
			if (rpath != null) {
				if (logMINOR) Logger.minor(
					this,
					"Resolved URI (rpath absolute): \"" + rpath + "\""
				);
			...
			if ((!isAbsolute) && (!forBaseHref)) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Resolved URI (rpath relative): " + rpath
				);
		...
		if (forBaseHref) throw new CommentException(l10n("bogusBaseHref"));
		if (
			GenericReadFilterCallback.allowedProtocols.contains(uri.getScheme())
			...
			throw new CommentException(
				l10n("protocolNotEscaped", "protocol", uri.getScheme())
			);
		}
	}
	...
	public String makeURIAbsolute(String uri) throws URISyntaxException {
		return baseURI
			.resolve(URIPreEncoder.encodeURI(uri).normalize())
			.toASCIIString();
	}

	private static String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("GenericReadFilterCallback." + key, pattern, value);
	}
	...
	private String finishProcess(
		...
		if (overrideType != null) typeOverride = overrideType;
		...
		if (typeOverride != null) {
			...
			if (split[1] != null) {
				...
				if (charset != null && charset.indexOf('&') != -1) charset =
					null;
				if (charset != null && !Charset.isSupported(charset)) charset =
					null;
				if (charset != null) typeOverride = split[0] +
				"; charset=" +
				charset;
				else typeOverride = split[0];
			}
		}
		...
		try {
			...
			if (typeOverride != null) {
				sb.append("?type=");
				sb.append(
					freenet.support.URLEncoder.encode(
						typeOverride,
						"",
						false,
						"="
			...
			if (!noRelative) uri = strippedBaseURI.relativize(uri);
			if (logMINOR) Logger.minor(
				this,
				"Returning " +
				uri.toASCIIString() +
			...
			String p = path;
			if (typeOverride != null) p += "?type=" + typeOverride;
			if (u.getFragment() != null) {
				try {
					...
					throw new Error(
						"Impossible: JVM doesn't support UTF-8: " + e,
						e
					);
				}
			}
	...
	public String onBaseHref(String baseHref) {
		...
		if (ret == null) {
			Logger.error(
				this,
				"onBaseHref() failed: cannot sanitize " + baseHref
			);
			return null;
		} else {
			try {
	...
	public String processForm(String method, String action)
		...
		if (!(method.equals("POST") || method.equals("GET"))) return null; // no irregular form sending methods
		...
		if (action.equals("/library/")) return action;
		try {
			...
			if (path.startsWith(PLUGINS_PREFIX)) {
				...
				if (after.indexOf("../") > -1) throw new CommentException(
					l10n("invalidFormURIAttemptToEscape")
				);
				if (
					after.matches("[A-Za-z0-9\\.]+")
				) return uri.toASCIIString();
			}
		} catch (URISyntaxException e) {
			throw new CommentException(
				l10n(
					"couldNotParseFormURIWithError",
					"error",
