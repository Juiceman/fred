...
public class ElementInfo {
  ...
  /**
   * If true, and above is false, allow font names only if they consist entirely of spaces, numbers,
   * letters, and ._-,+~
   */
  ...
  public static final Set<String> LANGUAGES =
      Collections.unmodifiableSet(
          new HashSet<String>(
              Arrays.asList(
                  "az", "be", "bg", "cs", "de", "el", "en", "es", "fi", "fr", "id", "it", "ja",
                  "ka", "kk", "ky", "lv", "mo", "nl", "no", "pl", "pt", "ro", "ru", "sv", "tl",
                  "tr", "tt", "uk", "zh-hans", "zh-hant")));
  ...
  public static final Set<String> GENERIC_FONT_KEYWORDS =
      Collections.unmodifiableSet(
          new HashSet<String>(
              Arrays.asList("serif", "sans-serif", "cursive", "fantasy", "monospace")));
  ...
  /**
   * According to the HTML spec, ID must begin with a letter A-Za-z, and may be followed by any
   * number of [a-zA-Z0-9-_:.]. It is not spelled out what is allowed in class, but we are assuming
  ...
  public static boolean isValidIdentifier(String name) {
    if (name.length() == 0) {
      ...
      if (escape) {
        // Still in an escape.
        // Might be dangerous e.g. escaping the ] in E[foo=blah] could change the meaning
        // completely.
        return false;
      }
  ...
  public static boolean isValidPseudoClass(String cname) {
    ...
    if (PSEUDOCLASS.contains(cname)) return true;
      ...
      return true;
        ...
        && FilterUtils.isNth(getPseudoClassArg(cname, "nth-child"))) return true;
    else if (cname.indexOf("nth-last-child") != -1
        && FilterUtils.isNth(getPseudoClassArg(cname, "nth-last-child"))) return true;
    else if (cname.indexOf("nth-of-type") != -1
        && FilterUtils.isNth(getPseudoClassArg(cname, "nth-of-type"))) return true;
    else if (cname.indexOf("nth-last-of-type") != -1
        && FilterUtils.isNth(getPseudoClassArg(cname, "nth-last-of-type"))) return true;
  ...
  public static boolean isValidStringDecoded(String s) {
    // REDFLAG: All strings which are parsed and decoded correctly (which has happened before this
    // point) are valid.
    // That is pretty much what the spec says, and there would seem to be no risk, except for with
    // wierd extensions, which can act on plain text so there isn't much we can do.
    return true;
  }
