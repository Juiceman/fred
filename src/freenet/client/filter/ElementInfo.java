...
public class ElementInfo {
	...
	public static final Set<String> VOID_ELEMENTS = Collections.unmodifiableSet(
		new HashSet<String>(
			Arrays.asList(
				"area",
				"base",
	...
	public static final Set<String> HTML_ELEMENTS = Collections.unmodifiableSet(
		new HashSet<String>(HTMLFilter.getAllowedHTMLTags())
	);
	...
	public static final Set<String> LANGUAGES = Collections.unmodifiableSet(
		new HashSet<String>(
			Arrays.asList(
				"az",
				"be",
	...
	public static final Set<String> MEDIA = Collections.unmodifiableSet(
		new HashSet<String>(
			Arrays.asList(
				"all",
				"aural",
	...
	public static final Set<String> VISUALMEDIA = Collections.unmodifiableSet(
		new HashSet<String>(
			Arrays.asList(
				"handheld",
				"print",
	...
	public static final Set<String> AURALMEDIA = Collections.unmodifiableSet(
		new HashSet<String>(Arrays.asList("speech", "aural"))
	);
	...
	public static final Set<String> FONTS = Collections.unmodifiableSet(
		new HashSet<String>(
			Arrays.asList(
				"arial",
				"helvetica",
	...
	public static final Set<String> GENERIC_VOICE_KEYWORDS =
		Collections.unmodifiableSet(
			new HashSet<String>(Arrays.asList("male", "female", "child"))
		);

	public static final Set<String> PSEUDOCLASS = Collections.unmodifiableSet(
		new HashSet<String>(
			Arrays.asList(
				"first-child",
				"last-child",
	...
	public static boolean isWordPrefixOrMatchOfSpecificFontFamily(
		String prefix
	) {
		String extraSpace = prefix + " ";
		for (String s : FONTS) if (
			s.equals(prefix) || s.startsWith(extraSpace)
		) return true;
		return false;
	}
	...
	public static boolean isValidHTMLTag(String tag) {
		return (
			HTML_ELEMENTS.contains(tag.toLowerCase()) ||
			VOID_ELEMENTS.contains(tag.toLowerCase())
		);
	}
	...
	public static boolean isBannedPseudoClass(String cname) {
		if (cname.indexOf(':') != -1) {
			...
			for (String s : split) if (isBannedPseudoClass(s)) return true;
			return false;
		}
	...
	public static String getPseudoClassArg(
		String cname,
		String cname_sans_arg
	) {
		String arg = "";
		...
		if (
			cname
				.substring(cnameIndex + cname_sans_arg.length(), firstIndex)
				.trim()
				.isEmpty() &&
			cname.substring(0, cnameIndex).trim().isEmpty() &&
			cname.substring(secondIndex + 1, cname.length()).trim().isEmpty()
		) {
			arg = CSSTokenizerFilter.removeOuterQuotes(
				cname.substring(firstIndex + 1, secondIndex).trim()
			);
		}
		return arg;
	}
