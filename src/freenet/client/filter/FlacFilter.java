	...
	public void readFilter(
		...
		for(byte magicCharacter : magicNumber) {
			if(magicCharacter != in.readByte()) {
				throw new DataFilterException(l10n("InvalidFLACStreamTitle"), l10n("InvalidFLACStreamTitle"),
		...
		while(currentState != State.STREAM_FINISHED) {
			...
			try {
				...
				if(currentState == State.UNINITIALIZED && packet instanceof FlacMetadataBlock
						&& ((FlacMetadataBlock) packet).isLastMetadataBlock()) {
