...
public class FilterUtils {
  ...
  public static boolean isColor(String value) {
    ...
    if (value.indexOf("rgb(") == 0 && value.indexOf(')') == value.length() - 1) {
      ...
      if (isValidColorParts) return true;
    }
    if (value.indexOf("rgba(") == 0 && value.indexOf(')') == value.length() - 1) {
      ...
      if (isValidColorParts && isNumber(colorParts[3])) return true;
    }
    ...
    if (value.indexOf("hsla(") == 0 && value.indexOf(')') == value.length() - 1) {
      ...
      if (isNumber(colorParts[0])
          && isPercentage(colorParts[1])
          && isPercentage(colorParts[2])
          && isNumber(colorParts[3])) return true;
    }
        ...
        if (aLength == 0
          ...
          if (bLength == 0
              || ((value.charAt(bIndex) == '+' || value.charAt(bIndex) == '-')
                  && isIntegerInRange(
                      value.substring(bIndex, value.length()), -MAX_NTH, MAX_NTH))) {
            return true;
          }
        }
      }
