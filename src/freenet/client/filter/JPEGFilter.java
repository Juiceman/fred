...
public class JPEGFilter implements ContentDataFilter {
	...
	public void readFilter(InputStream input, OutputStream output, String charset,
	...
	throws DataFilterException, IOException {
		...
		while(!finished) {
			...
			if(forceMarkerType != -1) {
				...
				if(markerStart != 0xFF) {
					throwError("Invalid marker", "The file includes an invalid marker start "+Integer.toHexString(
								   markerStart)+" and cannot be parsed further.");
				}
				if(baos != null) {
					baos.write(0xFF);
				}
			...
			if(markerType == 0xDA) {
				...
				if(type.equals("JFIF")) {
					...
					if(extensionCode == 0x10 || extensionCode == 0x11 || extensionCode == 0x13) {
						...
						throwError("Unknown JFXX extension "+extensionCode,
				...
				if(valid) {
					...
					dis.readFully(buf);
					dos.write(buf);
					Logger.minor(this, "Essential frame type "+Integer.toHexString(markerType)+" length "+
								 (blockLength-2)+" offset at end "+cis.count());
				} else {
					if(markerType >= 0xE0 && markerType <= 0xEF) {
						// APP marker. Can be safely deleted.
						if(logMINOR) {
							Logger.minor(this, "Dropping application marker type "+Integer.toHexString(
											 markerType)+" length "+blockLength);
						}
					} else {
						if(logMINOR) {
			...
			if(cis.count() != countAtStart + blockLength)
				throwError("Invalid frame", "The length of the frame is incorrect (read "+
						   (cis.count()-countAtStart)+" bytes, frame length "+blockLength+" for type "+Integer.toHexString(
							   markerType)+").");
			// Write frame
			baos.writeTo(output);
		}
	...
	private void skipRest(int blockLength, long countAtStart, CountedInputStream cis,
						  DataInputStream dis, DataOutputStream dos, String thing) throws IOException {
