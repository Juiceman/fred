...
public class Metadata implements Cloneable, Serializable {
		...
		if ((!splitfile) && ((documentType == DocumentType.SIMPLE_REDIRECT) || (documentType == DocumentType.ARCHIVE_MANIFEST))) {
			...
			if (segmentCount <= 0) {
				...
				for (int i = 0; i < segments.length; i++) {
					...
					if (logMINOR) {
						Logger.minor(this, "REQUESTING: Segment " + i + " of " + segments.length + " : " + copyDataBlocks + " data blocks " + copyCheckBlocks + " check blocks");
					}
		...
		if ((documentType == DocumentType.ARCHIVE_INTERNAL_REDIRECT) || (documentType == DocumentType.ARCHIVE_METADATA_REDIRECT) || (documentType == DocumentType.SYMBOLIC_SHORTLINK)) {
			...
			while (true) {
				if (targetName.isEmpty()) {
					throw new MetadataParseException("Invalid target name is empty: \"" + new String(buf, StandardCharsets.UTF_8) + "\"");
				}
	...
	public String getArchiveInternalName() {
		if ((documentType != DocumentType.ARCHIVE_INTERNAL_REDIRECT) && (documentType != DocumentType.ARCHIVE_METADATA_REDIRECT)) {
			throw new IllegalArgumentException();
		}
		return targetName;
	}
