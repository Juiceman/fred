...
public class FetchContext implements Cloneable, Serializable {
	...
	public void setCooldownRetries(int cooldownRetries) {
		...
		if(cooldownRetries > RequestScheduler.COOLDOWN_RETRIES) {
			throw new IllegalArgumentException("Invalid COOLDOWN_RETRIES: Must be <= "
											   +RequestScheduler.COOLDOWN_RETRIES+" since the network will not tolerate more than that");
		}
	...
	public void setCooldownTime(long cooldownTime, boolean force) {
		...
		if(cooldownTime < RequestScheduler.COOLDOWN_PERIOD && !force) {
			throw new IllegalArgumentException("Invalid COOLDOWN_PERIOD: Must be >= "
											   +RequestScheduler.COOLDOWN_PERIOD+" since ULPRs will ensure fast response at that level");
		}
