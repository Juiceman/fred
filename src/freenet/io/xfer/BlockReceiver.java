...
public class BlockReceiver implements AsyncMessageFilterCallback {
	...
	public BlockReceiver(
		...
		_timeoutHandler = timeoutHandler == null
			? nullTimeoutHandler
			: timeoutHandler;
		_sender = sender;
		_prb = prb;
	...
	private void sendAborted(int reason, String desc)
	...
	private AsyncMessageFilterCallback notificationWaiter =
		new SlowAsyncMessageFilterCallback() {
			...
			public void onMatched(Message m1) {
				if (logMINOR) Logger.minor(this, "Received " + m1);
				if ((m1 != null) && m1.getSpec().equals(DMT.sendAborted)) {
					String desc = m1.getString(DMT.DESCRIPTION);
					if (desc.indexOf("Upstream") < 0) desc =
						"Upstream transmit error: " + desc;
				...
				if ((m1 != null) && (m1.getSpec().equals(DMT.packetTransmit))) {
					...
					try {
						...
						if (CHECK_DUPES && _prb.isReceived(packetNo)) {
							...
							if (logMINOR && missing != 0) Logger.minor(
								this,
				...
				try {
					if (_prb.allReceived()) {
						try {
							...
							discardEndTime = System.currentTimeMillis() +
							CLEANUP_TIMEOUT;
							...
							if (logMINOR) Logger.minor(
								this,
								"Got data but can't send allReceived to " +
								_sender +
								" as is disconnected"
							);
						}
			...
			public void onTimeout() {
				...
				try {
					...
					_prb.abort(
						RetrievalException.SENDER_DIED,
	...
	private void complete(int reason, String description) {
		...
		if (block == null) {
			...
			callback.blockReceiveFailed(
				new RetrievalException(reason, description)
			);
		} else {
			Logger.error(
				this,
	...
	private void waitNotification(boolean truncateTimeout)
		...
		synchronized (this) {
			if (truncateTimeout) {
				timeout = (int) Math.min(
					timeStartedWaiting + RECEIPT_TIMEOUT - now,
					RECEIPT_TIMEOUT
				);
			} else {
	...
	private MessageFilter relevantMessages(long timeout) {
		MessageFilter mfPacketTransmit = MessageFilter.create()
			.setTimeout(timeout)
			.setType(DMT.packetTransmit)
			.setField(DMT.UID, _uid)
			.setSource(_sender);
		MessageFilter mfAllSent = MessageFilter.create()
			.setTimeout(timeout)
			.setType(DMT.allSent)
			.setField(DMT.UID, _uid)
			.setSource(_sender);
	...
	public void receive(BlockReceiverCompletion callback) {
		...
		try {
			...
			_prb.abort(
				...
				true
				/* kind of, it shouldn't count towards the stats anyway */
			);
	...
	public void onMatched(Message m) {
		if (logMINOR) Logger.minor(
			this,
			"discarding message post-receive: " + m
		);
		maybeResetDiscardFilter();
	}
	...
	private void incRunningBlockReceives() {
		...
		synchronized (BlockReceiver.class) {
			...
			if (logMINOR) Logger.minor(
				...
				"Started a block receive, running: " + runningBlockReceives
			);
		}
	}
	...
	private void decRunningBlockReceives() {
		...
		synchronized (BlockReceiver.class) {
			...
			if (logMINOR) Logger.minor(
				BlockTransmitter.class,
				"Finished a block receive, running: " + runningBlockReceives
			);
		}
	}
