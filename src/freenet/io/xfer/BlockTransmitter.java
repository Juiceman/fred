...
public class BlockTransmitter {
	...
	class BlockSenderJob implements PrioRunnable {
			...
			try {
				while (true) {
					...
					synchronized (_senderThread) {
						if (
							_failed || _receivedSendCompletion || _completed
						) return;
						if (_unsent.size() == 0) {
		...
		public void schedule() {
			if (_failed || _receivedSendCompletion || _completed) {
				if (logMINOR) Logger.minor(
					...
					" :" +
					(_failed ? "(failed) " : "") +
					(_receivedSendCompletion
							? "(receivedSendCompletion) "
							: "") +
			...
			_executor.execute(
				this,
				"BlockTransmitter block sender for " +
				_uid +
				" to " +
				_destination
			);
		...
		private boolean innerRun(int packetNo, BitArray copied) {
			try {
				Message msg = DMT.createPacketTransmit(
					_uid,
					packetNo,
					copied,
					_prb.getPacket(packetNo),
					realTime
				);
					...
					synchronized (_senderThread) {
						...
						if (_failed) {
							...
							if (!hadSendCompletion) {
								Logger.warning(
									this,
									"Terminating send after failure on " + this
								);
								abortReason =
									"Already failed and no acknowledgement";
							} else {
								// Waiting for transfers maybe???
								if (logMINOR) Logger.minor(
									this,
									"Trying to terminate send after timeout"
								);
								abortReason = "Already failed";
							}
						} else {
							timeString = TimeUtil.formatTime(
								(System.currentTimeMillis() - timeAllSent),
								2,
								true
							);
							Logger.warning(
							...
							abortReason =
								"Haven't heard from you (receiver) in " +
								timeString;
						}
						fail = maybeFail(
							RetrievalException.RECEIVER_DIED,
							abortReason
						);
					}
					fail.execute();
	...
	public boolean maybeAllSent() {
		if (
			blockSendsPending == 0 &&
			_unsent.size() == 0 &&
			getNumSent() == _prb._packets
		) {
			timeAllSent = System.currentTimeMillis();
			if (logMINOR) Logger.minor(
				this,
				"Sent all blocks, none unsent on " + this
			);
		...
		if (blockSendsPending == 0 && _failed) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Sent blocks and failed on " + this
			);
			return true;
		}
		if (logMINOR) Logger.minor(
	...
	public boolean maybeComplete() {
		if (!_receivedSendCompletion) {
			if (logMINOR) Logger.minor(
				this,
				"maybeComplete() not completing because send not completed on " +
				this
			);
		...
		if (_completed) {
			if (logMINOR) Logger.minor(
				this,
				"maybeComplete() already completed on " + this
			);
			return false;
		}
		if (logMINOR) Logger.minor(
			this,
			"maybeComplete() completing on " + this
		);
		_completed = true;
	...
	public Future maybeFail(final int reason, final String description) {
		if (_completed) {
			if (logMINOR) Logger.minor(
				this,
				"maybeFail() already completed on " + this
			);
			return nullFuture;
		}
		_failed = true;
		if (!_receivedSendCompletion) {
			...
			if (logMINOR) Logger.minor(
				this,
				"maybeFail() waiting for acknowledgement on " + this
			);
			if (_sentSendAborted) {
				scheduleTimeoutAfterBlockSends();
	...
	public void innerSendAborted(int reason, String desc)
		throws NotConnectedException {
		_usm.send(
			_destination,
			DMT.createSendAborted(_uid, reason, desc),
			_ctr
		);
	}
	...
	private AsyncMessageFilterCallback cbSendAborted =
		new SlowAsyncMessageFilterCallback() {
			...
			public void onMatched(Message msg) {
				if ((!_prb.isAborted()) && abortHandler.onAbort()) _prb.abort(
					RetrievalException.CANCELLED_BY_RECEIVER,
					"Cascading cancel from receiver",
					true
				);
				Future fail;
				synchronized (_senderThread) {
					...
					_receivedSendSuccess = false;
					fail = maybeFail(
						msg.getInt(DMT.REASON),
						msg.getString(DMT.DESCRIPTION)
					);
					if (logMINOR) Logger.minor(
						this,
						"Transfer got sendAborted on " + BlockTransmitter.this
					);
				}
				fail.execute();
				cancelItemsPending();
	...
	public void sendAsync() {
		...
		try {
			...
			MessageFilter mfAllReceived = MessageFilter.create()
				.setType(DMT.allReceived)
				.setField(DMT.UID, _uid)
				.setSource(_destination)
				.setNoTimeout();
			MessageFilter mfSendAborted = MessageFilter.create()
	...
	private void cancelItemsPending() {
		...
		for (MessageItem item : items) {
			if (!_destination.unqueueMessage(item)) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Message not queued ?!?!?!? on " + this + " : " + item
				);
			}
		}
	}
	...
	private static synchronized void incRunningBlockTransmits() {
		...
		if (logMINOR) Logger.minor(
			BlockTransmitter.class,
			"Started a block transmit, running: " + runningBlockTransmits
		);
	}
	...
	private static synchronized void decRunningBlockTransmits() {
		...
		if (logMINOR) Logger.minor(
			BlockTransmitter.class,
			"Finished a block transmit, running: " + runningBlockTransmits
		);
	}
	...
	private class MyAsyncMessageCallback implements AsyncMessageCallback {
		...
		public void sent() {
			if (logMINOR) Logger.minor(
				this,
				"Sent block on " + BlockTransmitter.this
			);
		...
		private void complete(boolean failed) {
			if (logMINOR) Logger.minor(
				this,
				"Completed send on a block for " + BlockTransmitter.this
			);
			boolean success = false;
			...
			synchronized (_senderThread) {
				...
				if (lastSentPacket > 0) {
					...
					if (delta > threshold) Logger.warning(
						this,
					...
					else if (delta > threshold / 5) Logger.normal(
						this,
						"Time between packets on " +
						BlockTransmitter.this +
						" : " +
