 ...
 *
 * <p>SECURITY REDFLAG WARNING: Messages should normally be recreated rather than passed on.
 * Messages can contain sub-messages, these are used to avoid having to add whole new message types
 * every time we add one field to a message... Passing on a message as-is means it includes the
 * sub-messages, which could lead to e.g. labelling, communication between colluding nodes along a
 * request route, and just wasting bytes.
 *
 * <p>FIXME we should get rid of sub-messages.
 *
 * @author ian
 */
  ...
  private static Message decodeMessage(
    ...
    try {
      for (String name : mspec.getOrderedFields()) {
        ...
        if (type.equals(
            LinkedList.class)) { // Special handling for LinkedList to deal with element type
          m.set(
              name,
              Serializer.readListFromDataInputStream(mspec.getLinkedListTypes().get(name), bb));
        } else {
  ...
  public void set(String key, Object value) {
    if (!_spec.checkType(key, value)) {
      ...
      throw new IncorrectTypeException(
          "Got " + value.getClass() + ", expected " + _spec.typeOf(key));
    }
