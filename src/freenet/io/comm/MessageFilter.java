...
public final class MessageFilter {
  ...
  /**
   * If true, timeouts are relative to the start of waiting, if false, they are relative to the time
  ...
  /**
   * This filter will expire after the specificed amount of time. Note also that where two or more
   * filters match the same message, the one with the nearer expiry time will get priority
   *
  ...
  /**
   * Modifies the filter so that it returns true if either it or the filter in the argument returns
   * true. Multiple combinations must be nested: such as filter1.or(filter2.or(filter3))). filter2
   * will be checked before filter1, so make sure to add the most common last.
   *
   ...
   * @return True if the filter has timed out, or if it has been matched already. Caller will remove
   *     the filter from _filters if we return true.
   */
  ...
  /**
   * Notify because of a dropped connection. Caller must verify _matchesDroppedConnection and
   * _source.
   *
   * @param ctx
   */
  ...
  /**
   * Notify because of a restarted connection. Caller must verify _matchesDroppedConnection and
   * _source.
   *
   * @param ctx
   */
