...
public class UdpSocketHandler
  ...
  public UdpSocketHandler(
    ...
    boolean r =
        socketOptions.setAddressPreference(
            _sock, socketOptions.SOCKET_ADDR_PREFERENCE.IPV6_PREFER_SRC_PUBLIC);
    if (logMINOR)
      Logger.minor(
          this,
  ...
  private boolean getPacket(DatagramPacket packet) {
    try {
      ...
      collector.addInfo(
          address, packet.getPort(), getHeadersLength(address) + packet.getLength(), 0, isLocal);
  ...
  public void sendPacket(byte[] blockToSend, Peer destination, boolean allowLocalAddresses)
    ...
    try {
      ...
      boolean isLocal =
          (!IPUtil.isValidAddress(address, false)) && (IPUtil.isValidAddress(address, true));
  ...
  int
      innerCalculateMaxPacketSize() { // FIXME: what about passing a peerNode though and doing it on
