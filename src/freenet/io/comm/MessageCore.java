...
public class MessageCore {
  ...
  // FIXME may not work well for newly added filters with timeouts close to the minimum, or filters
  // with timeouts close to the minimum in general.
  ...
  /**
   * Decode a packet from data and a peer. Can be called by IncomingPacketFilter's.
   *
   * @param data
   * @param offset
   * @param length
    ...
    synchronized (_filters) {
      for (ListIterator<MessageFilter> i = _filters.listIterator(); i.hasNext(); ) {
        ...
        if (f.timedOut(tStart)) {
          ...
          if (f.hasCallback() && nextTimeout > f.getTimeout()) nextTimeout = f.getTimeout();
        }
  ...
  public void checkFilters(Message m, PacketSocketHandler from) {
    ...
    if (!matched) {
       ...
       *
       * <p>Specifically: - Thread A receives packet - Thread A checks filters. It doesn't match
       * any. - Thread A feeds to Dispatcher. - Thread B creates filter. - Thread B checks
       * _unmatched. - Thread B adds filter. - Thread B sleeps. - Thread A returns from Dispatcher.
       * Which didn't match. - Thread A adds to _unmatched.
       *
       * <p>OOPS! The only way to fix this is to have checking the filters and unmatched be a single
       * atomic operation. Another race is possible if we merely recheck the filters after we return
  ...
  public void addAsyncFilter(
    ...
    // Drop any _unclaimed messages that the filter doesn't match that are also older than
    ...
    synchronized (_filters) {
      // Once in the list, it is up to the callback system to trigger the disconnection, however, we
      // may
  ...
  /**
   * Wait for a filter to trigger, or timeout. Blocks until either the trigger is activated, or it
   * times out, or the peer is disconnected.
   *
    ...
    if (ret == null) {
      ...
      synchronized (filter) {
        try {
          ...
          // Check reallyTimedOut() too a) for paranoia, b) for filters with a callback (we could
          // conceivably waitFor() them).
          ...
          if (filter.droppedConnection() != null) throw new DisconnectedException();
