	...
	public void checkFilters(Message m, PacketSocketHandler from) {
		...
		if (dT > 50) {
			if (dT > 3000) {
				...
				Logger.minor(this, "checkFilters took " + (dT) + "ms with unclaimedFIFOSize of " + _unclaimed.size() + " for matched: " + matched);
			}
		}
	}
	...
	public Message waitFor(MessageFilter filter, ByteCounter ctr) throws DisconnectedException {
			...
			if (ret == null) {
				...
				while (true) {
					...
					if (mf.getTimeout() > filter.getTimeout()) {
						...
						i.add(filter);
						if (logMINOR) {
							Logger.minor(this, "Added in middle - mf timeout=" + mf.getTimeout() + " - my timeout=" + filter.getTimeout() + " filter " + filter);
						}
						break;
					}
				}
		...
		if (tEnd - now > 50) {
			if (tEnd - now > 3000) {
				...
				Logger.minor(this, "waitFor _unclaimed iteration took " + (tEnd - now) + "ms with unclaimedFIFOSize of " + _unclaimed.size() + " for ret of " + ret);
			}
		}
