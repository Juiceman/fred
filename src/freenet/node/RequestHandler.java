...
public class RequestHandler implements PrioRunnable, ByteCounter, RequestSenderListener {
	 ...
	 * @param needsPubKey
	 */
	public RequestHandler(PeerNode source, long id, Node n, short htl, Key key, RequestTag tag,
						  KeyBlock passedInKeyBlock, boolean realTimeFlag, boolean needsPubKey) {
		node = n;
		uid = id;
	...
	private void applyByteCounts() {
		synchronized(this) {
			...
			if(!((!finalTransferFailed) && rs != null && status != RequestSender.TIMED_OUT
					&& status != RequestSender.GENERATED_REJECTED_OVERLOAD && status != RequestSender.INTERNAL_ERROR)) {
				return;
			}
		}
		int sent, rcvd;
	...
	private void realRun() throws NotConnectedException {
		...
		if(o == null) { // ran out of htl?
			...
			node.failureTable.onFinalFailure(key, null, htl, htl, FailureTable.RECENTLY_FAILED_TIME,
			...
			new ReceiverAbortHandler() {
				...
				public boolean onAbort() {
					...
					if(node.failureTable.peersWantKey(key, source)) {
						...
						Logger.error(this, "Downstream transfer successful but upstream transfer to "+source.shortToString()
									 +" failed. Reassigning tag to self because want the data for peers on "+RequestHandler.this);
						node.tracker.reassignTagToSelf(tag);
					...
					if(node.clientCore != null && node.clientCore.wantKey(key)) {
						 ...
						 */
						Logger.error(this, "Downstream transfer successful but upstream transfer to "+source.shortToString()
									 +" failed. Reassigning tag to self because want the data for ourselves on "+RequestHandler.this);
						node.tracker.reassignTagToSelf(tag);
						return false; // Want it
					}
					return true;
	...
	public void onRequestSenderFinished(int status, boolean fromOfferedKey, RequestSender rs) {
		...
		node.nodeStats.remoteRequest(key instanceof NodeSSK, status == RequestSender.SUCCESS, false, htl,
									 key.toNormalizedDouble(), realTimeFlag, fromOfferedKey);
		...
		if(tooLate) {
			...
			Logger.normal(this, "requestsender took too long to respond to requestor (" + TimeUtil.formatTime((
							  now - searchStartTime), 2, true) + "/" + (rs == null ? "null" : rs.getStatusString()) +
						  ") routed to " + (routedLast == null ? "null" : routedLast.shortToString()));
	...
	private void sendSSK(byte[] headers, final byte[] data, boolean needsPubKey2,
						 DSAPublicKey pubKey) throws NotConnectedException {
		...
		mcb = new MultiMessageCallback() {
			...
			public void finish(boolean success) {
				sentPayload(
					data.length); // FIXME report this at the time when that message is acked for more accurate reporting???
				applyByteCounts();
	...
	static void sendSSK(byte[] headers, byte[] data, boolean needsPubKey, DSAPublicKey pubKey,
						final PeerNode source, long uid, ByteCounter ctr,
						boolean realTimeFlag) throws NotConnectedException, WaitedTooLongException, PeerRestartedException,
	...
	private void finishOpennetNoRelayInner(OpennetManager om, byte[] noderef) {
		...
		try {
			...
			Logger.error(this, "Bad signature on opennet noderef for " + this + " from " + source + " : " + e,
						 e);
		}
	}
	...
	private void finishOpennetRelay(byte[] noderef, final OpennetManager om) {
		...
		OpennetManager.waitForOpennetNoderef(true, source, uid, this, new NoderefCallback() {
			...
			public void timedOut() {
				tag.unlockHandler();
				try {
					dataSource.sendAsync(DMT.createFNPOpennetCompletedTimeout(uid), rs.finishOpennetOnAck(dataSource),
										 RequestHandler.this);
				} catch (NotConnectedException e) {
					// Ignore
				}
