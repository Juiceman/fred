...
public class NodeDispatcher implements Dispatcher, Runnable {
	...
	public boolean handleMessage(Message m) {
		...
		if (spec == DMT.FNPPing) {
			...
			spec == DMT.FNPVisibility && source instanceof DarknetPeerNode
		) {
			...
			if (source instanceof DarknetPeerNode) {
				...
				if (logMINOR) Logger.minor(
					this,
					"The peer (" + source + ") asked us to set routing=" + value
				);
			...
			source.isRealConnection() && spec == DMT.FNPLocChangeNotificationNew
		) {
			double newLoc = m.getDouble(DMT.LOCATION);
			ShortBuffer buffer =
				((ShortBuffer) m.getObject(DMT.PEER_LOCATIONS));
			 ...
			 */
			if (
				(OpennetManager.MAX_PEERS_FOR_SCALING < locs.length) &&
				(source.isOpennet())
			) {
			...
			spec == DMT.FNPPeerLoadStatusByte ||
		...
		if (spec == DMT.FNPSwapRequest) {
			...
			spec == DMT.FNPGetYourFullNoderef &&
			source instanceof DarknetPeerNode
		) {
			((DarknetPeerNode) source).sendFullNoderef();
			return true;
		} else if (
			spec == DMT.FNPMyFullNoderef && source instanceof DarknetPeerNode
		) {
	...
	private boolean handleOfferKey(Message m, PeerNode source) {
		...
		byte[] authenticator =
			((ShortBuffer) m.getObject(DMT.OFFER_AUTHENTICATOR)).getData();
	...
	private boolean handleGetOfferedKey(Message m, PeerNode source) {
		...
		if (
			!HMAC.verifyWithSHA256(
				node.failureTable.offerAuthenticatorKey,
				key.getFullKey(),
				authenticator
			)
		) {
			...
			try {
				source.sendAsync(
					DMT.createFNPGetOfferedKeyInvalid(
						uid,
						DMT.GET_OFFERED_KEY_REJECTED_BAD_AUTHENTICATOR
					),
					null,
		...
		if (
			!tracker.lockUID(uid, isSSK, false, true, false, realTimeFlag, tag)
		) {
			if (logMINOR) Logger.minor(
				this,
				"Could not lock ID " + uid + " -> rejecting (already running)"
			);
			...
			try {
				...
				Logger.normal(
					this,
					"Rejecting request from " + source.getPeer() + ": " + e
				);
			}
			return true;
		} else {
		...
		try {
			...
			RejectReason reject = nodeStats.shouldRejectRequest(
				true,
				false,
				isSSK,
				false,
			...
			if (reject != null) {
				Logger.normal(
					this,
					"Rejecting FNPGetOfferedKey from " +
					source +
					" for " +
					key +
					" : " +
				...
				Message rejected = DMT.createFNPRejectedOverload(
					uid,
					true,
					true,
					realTimeFlag
				);
				if (reject.soft) rejected.addSubMessage(
					DMT.createFNPRejectIsSoft()
				);
				try {
					source.sendAsync(
						rejected,
					...
					Logger.normal(
						this,
						"Rejecting (overload) data request from " +
						source.getPeer() +
						": " +
						e
					);
		...
		try {
			node.failureTable.sendOfferedKey(
				key,
				isSSK,
				needPubKey,
				uid,
	...
	private void finishDisconnect(final Message m, final PeerNode source) {
		...
		if (remove) {
			...
			if (source instanceof DarknetPeerNode) System.out.println( // FIXME add a useralert? // FIXME remove, dirty logs.
		...
		if (purge) {
			...
			if (
				om != null && source instanceof OpennetPeerNode
			) om.purgeOldOpennetPeer((OpennetPeerNode) source);
		}
		// Process parting message
		int type = m.getInt(DMT.NODE_TO_NODE_MESSAGE_TYPE);
		ShortBuffer messageData = (ShortBuffer) m.getObject(
			DMT.NODE_TO_NODE_MESSAGE_DATA
		);
		...
		public void run() {
			while (true) {
				try {
					...
					innerHandleDataRequest(
						msg,
						(PeerNode) msg.getSource(),
						isSSK
					);
	...
	private final ArrayBlockingQueue<Message> requestQueue =
	 ...
	 */
	private void innerHandleDataRequest(
		Message m,
		PeerNode source,
		boolean isSSK
	) {
		if (!source.isConnected()) {
			if (logMINOR) Logger.minor(
				this,
				"Handling request off thread, source disconnected: " +
				source +
				" for " +
				m
			);
		...
		if (!source.isRoutable()) {
			if (logMINOR) Logger.minor(
				this,
				"Handling request off thread, source no longer routable: " +
				source +
				" for " +
				m
			);
		...
		if (
			!tracker.lockUID(id, isSSK, false, false, false, realTimeFlag, tag)
		) {
			if (logMINOR) Logger.minor(
				this,
				"Could not lock ID " + id + " -> rejecting (already running)"
			);
		...
		RejectReason rejectReason = nodeStats.shouldRejectRequest(
			!isSSK,
			false,
			isSSK,
			false,
		...
		if (rejectReason != null) {
			...
			if (rejectReason.soft) rejected.addSubMessage(
				DMT.createFNPRejectIsSoft()
			);
			try {
				source.sendAsync(rejected, null, ctr);
		...
		if (key instanceof NodeSSK) needsPubKey = m.getBoolean(
			DMT.NEED_PUB_KEY
		);
	...
	private void handleInsertRequest(
		...
		if (
			!tracker.lockUID(id, isSSK, true, false, false, realTimeFlag, tag)
		) {
			if (logMINOR) Logger.minor(
				this,
				"Could not lock ID " + id + " -> rejecting (already running)"
		...
		if (forkControl != null) forkOnCacheable = forkControl.getBoolean(
			DMT.ENABLE_INSERT_FORK_WHEN_CACHEABLE
		);
		Message lowBackoff = m.getSubMessage(DMT.FNPSubInsertIgnoreLowBackoff);
		if (lowBackoff != null) ignoreLowBackoff = lowBackoff.getBoolean(
			DMT.IGNORE_LOW_BACKOFF
		);
		Message preference = m.getSubMessage(DMT.FNPSubInsertPreferInsert);
		if (preference != null) preferInsert = preference.getBoolean(
			DMT.PREFER_INSERT
		);
		...
		if (rejectReason != null) {
			...
			try {
				...
				Logger.normal(
					this,
					"Rejecting (overload) insert request from " +
					source.getPeer() +
					": " +
					e
				);
		...
		if (m.getSpec().equals(DMT.FNPSSKInsertRequest)) {
			...
			byte[] headers =
				((ShortBuffer) m.getObject(DMT.BLOCK_HEADERS)).getData();
			short htl = m.getShort(DMT.HTL);
			if (htl <= 0) htl = 1;
			SSKInsertHandler rh = new SSKInsertHandler(
				key,
				data,
				headers,
				htl,
			...
			short htl = m.getShort(DMT.HTL);
			if (htl <= 0) htl = 1;
			CHKInsertHandler rh = new CHKInsertHandler(
				key,
				htl,
				source,
				id,
	...
	private boolean handleAnnounceRequest(Message m, PeerNode source) {
		...
		if (
			...
			Message msg = DMT.createFNPRejectedOverload(
				uid,
				true,
				false,
				false
			...
			if (logMINOR) Logger.minor(
				this,
				"Got bogus announcement message from " + source
			);
			return true;
		}
		...
		if (om == null || !source.canAcceptAnnouncements()) {
			if (
				om != null && source instanceof SeedClientPeerNode
			...
			if (logMINOR) Logger.minor(
				this,
				"Rejected announcement (opennet or announcement disabled) from " +
				source
			);
			return true;
		}
		...
		try {
			...
			NodeStats.AnnouncementDecision shouldAcceptAnnouncement =
				node.nodeStats.shouldAcceptAnnouncement(uid);
			if (
				!(NodeStats.AnnouncementDecision.ACCEPT ==
				...
				Message msg = null;
				if (
					NodeStats.AnnouncementDecision.OVERLOAD ==
					shouldAcceptAnnouncement
				) {
					msg = DMT.createFNPRejectedOverload(
						uid,
						true,
						false,
						false
					...
					NodeStats.AnnouncementDecision.LOOP ==
					shouldAcceptAnnouncement
				) {
					msg = DMT.createFNPRejectedLoop(uid);
					if (logMINOR) Logger.minor(
						this,
						"Rejected announcement (loop) from " + source
					);
				} else {
			...
			if (!source.shouldAcceptAnnounce(uid)) {
				...
				if (logMINOR) Logger.minor(
					this,
					"Rejected announcement (peer limit) from " + source
				);
				return true;
			}
			if (om != null && source instanceof SeedClientPeerNode) {
				if (
					!om.seedTracker.acceptAnnounce(
						(SeedClientPeerNode) source,
						node.fastWeakRandom
					)
				) {
					node.nodeStats.endAnnouncement(uid);
					...
					if (logMINOR) Logger.minor(
						this,
						"Rejected announcement (seednode limit) from " + source
					);
					return true;
				}
			}
			if (source instanceof SeedClientPeerNode) {
				...
				if (htl < maxHTL - 1) {
					Logger.error(
						this,
						"Announcement from seed client not at max HTL: " +
						htl +
						" for " +
						source
					);
				...
				cb = new AnnouncementCallback() {
					...
					public void completed() {
						synchronized (this) {
							Logger.minor(
								this,
								"Announcement from " + origin + " completed"
							);
						}
						int shallow =
							node.maxHTL() - (totalAdded + totalNotWanted);
						if (acceptedSomewhere) Logger.minor(
							this,
							"Announcement from " +
							origin +
							" completed (" +
					...
					public void nodeFailed(PeerNode pn, String reason) {
						Logger.minor(
							this,
							"Announcement from " + origin + " failed: " + reason
						);
					}
			...
			AnnounceSender sender = new AnnounceSender(
				target,
				htl,
				uid,
				source,
			...
			if (logMINOR) Logger.minor(
				this,
				"Accepted announcement from " + source
			);
			return true;
		} finally {
	...
	private boolean handleRoutedRejected(Message m) {
		...
		if (htl == 0) {
			...
			if (rc.source != null) {
				try {
					rc.source.sendAsync(
						DMT.createFNPRoutedRejected(id, (short) 0),
						null,
					...
					Logger.error(
						this,
						"Unable to relay probe DNF: peer disconnected: " +
						rc.source
					);
				}
			}
			...
			forward(
				...
				rc.source,
				htl,
				rc.msg.getDouble(DMT.TARGET_LOCATION),
				rc,
				rc.identity
			);
		}
	...
	boolean handleRouted(Message m, PeerNode source) {
		...
		short htl = m.getShort(DMT.HTL);
		byte[] identity =
			((ShortBuffer) m.getObject(DMT.NODE_IDENTITY)).getData();
		...
		if (logMINOR) Logger.minor(
			this,
			"id " + id + " from " + source + " htl " + htl + " target " + target
		);
	...
	boolean handleRoutedReply(Message m) {
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Lost connection forwarding " + m + " to " + pn
			);
		}
		return true;
	}
	...
	private boolean forward(
		...
		while (true) {
			...
			if (next == null) next = node.peers.closerPeer(
				pn,
				ctx.routedTo,
				target,
				true,
			...
			if (next != null) {
				...
				if (pn != null) try {
					...
					Logger.error(
						this,
						"Cannot send reject message back to source " + pn
					);
					return true;
				}
			}
	...
	private boolean dispatchRoutedMessage(Message m, PeerNode src, long id) {
		if (m.getSpec() == DMT.FNPRoutedPing) {
			if (logMINOR) Logger.minor(
				this,
				"RoutedPing reached other side! (" + id + ")"
			);
			int x = m.getInt(DMT.COUNTER);
			Message reply = DMT.createFNPRoutedPong(id, x);
			if (logMINOR) Logger.minor(
				this,
				"Replying - counter = " + x + " for " + id
			);
			try {
				...
				if (logMINOR) Logger.minor(
					this,
					"Lost connection replying to " +
					m +
					" in dispatchRoutedMessage"
				);
			}
			return true;
		}
	...
	public static String peersUIDsToString(long[] peerUIDs, double[] peerLocs) {
		StringBuilder sb = new StringBuilder(
			peerUIDs.length * 23 + peerLocs.length * 26
		);
