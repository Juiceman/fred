...
public class MemoryChecker implements Runnable {
  ...
  public void run() {
    ...
    if (totalMemory == maxMemory || maxMemory == Long.MAX_VALUE) {
      ...
      if (avgFreeMemory == null) avgFreeMemory = new SimpleRunningAverage(3, freeMemory);
      else avgFreeMemory.report(freeMemory);
    ...
    if (sleeptime <= 0) { // We are done
      ...
      return;
    } else ps.queueTimedJob(this, 120L * sleeptime);
    ...
    if (aggressiveGCModificator > 0) {
      ...
      if (logMINOR) {
        ...
        Logger.minor(
            this,
            "GC completed after "
                + (afterGCTime - beforeGCTime)
                + "ms and \"recovered\" "
                + (beforeGCUsedMemory - afterGCUsedMemory)
                + " bytes, leaving "
                + afterGCUsedMemory
                + " bytes used");
      }
