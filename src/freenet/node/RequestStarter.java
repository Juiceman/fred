...
public class RequestStarter implements Runnable, RandomGrabArrayItemExclusionList {
  ...
  void realRun() {
    ...
    while (true) {
      ...
      if (req != null) {
        ...
        //                  // Firstly, they use threads. This could be a serious problem for faster
        // nodes.
  ...
  private boolean startRequest(ChosenBlock req, boolean logMINOR) {
    ...
    if (req.key != null) {
      ...
      if (!sched.addRunningInsert(
          (SendableInsert) (((ChosenBlockImpl) req).request), req.token.getKey())) {
        req.onDumped();
        return false;
      }
    }
