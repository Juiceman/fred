...
public abstract class UIDTag {
	...
	public synchronized boolean addRoutedTo(PeerNode peer, boolean offeredKey) {
		if(logMINOR) {
			Logger.minor(this, "Routing to "+peer+" on "+this+(offeredKey ? " (offered)" : ""),
						 new Exception("debug"));
		}
		if(routedTo == null) {
	...
	public void removeRoutingTo(PeerNode next) {
		...
		synchronized(this) {
			...
			if(!currentlyRoutingTo.remove(next)) {
				if(logMINOR) {
					Logger.minor(this, "Removing wrong node or removing twice? on "+this+" : "+next,
								 new Exception("debug"));
				}
			}
			if(handlingTimeouts != null) {
	...
	protected synchronized boolean mustUnlock() {
		...
		if(currentlyRoutingTo != null && !currentlyRoutingTo.isEmpty()) {
			if(!(reassigned || wasLocal || sourceRestarted || timedOutButContinued)) {
				...
				if(handlingTimeouts != null) {
					...
					for(PeerNode pn : currentlyRoutingTo) {
						if(handlingTimeouts.contains(pn)) {
							if(logMINOR) {
								Logger.debug(this, "Still waiting for "+pn.shortToString()
				...
				if(!expected) {
					if(handlingTimeouts != null) {
						Logger.normal(this, "Unlocked handler but still routing to "+currentlyRoutingTo
									  +" - expected because have timed out so a fork might have succeeded and we might be waiting for the original");
					} else {
						Logger.error(this, "Unlocked handler but still routing to "+currentlyRoutingTo
									 +" yet not reassigned on "+this, new Exception("debug"));
					}
				} else {
					reassignToSelf();
				}
		...
		if(fetchingOfferedKeyFrom != null && !fetchingOfferedKeyFrom.isEmpty()) {
			if(!(reassigned || wasLocal)) {
				...
				{
					Logger.error(this, "Unlocked handler but still fetching offered keys from "+fetchingOfferedKeyFrom
								 +" yet not reassigned on "+this, new Exception("debug"));
				} else {
					reassignToSelf();
				}
