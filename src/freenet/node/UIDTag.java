...
public abstract class UIDTag {
	...
	public synchronized boolean addRoutedTo(PeerNode peer, boolean offeredKey) {
		...
		routedTo.add(peer);
		if (offeredKey) {
			if (fetchingOfferedKeyFrom == null) fetchingOfferedKeyFrom =
	...
	public void removeRoutingTo(PeerNode next) {
		if (logMINOR) {
			Logger.minor(
				this,
				"No longer routing to " + next + " on " + this,
				new Exception("debug")
			);
		}
		boolean noRecordUnlock;
		synchronized (this) {
			...
			if (!currentlyRoutingTo.remove(next)) {
				if (logMINOR) {
					Logger.minor(
						this,
						"Removing wrong node or removing twice? on " +
						this +
						" : " +
						next,
						new Exception("debug")
	...
	public void postUnlock() {
		PeerNode[] peers;
		synchronized (this) {
			if (routedTo != null) peers = routedTo.toArray(
				new PeerNode[routedTo.size()]
			);
			else peers = null;
		}
	...
	protected synchronized boolean mustUnlock() {
		...
		if (currentlyRoutingTo != null && !currentlyRoutingTo.isEmpty()) {
			if (
				...
				if (handlingTimeouts != null) {
					...
					for (PeerNode pn : currentlyRoutingTo) {
						if (handlingTimeouts.contains(pn)) {
							if (logMINOR) Logger.debug(
								this,
								"Still waiting for " +
								pn.shortToString() +
								" but expected because handling timeout in unlockHandler - will reassign to self to resolve timeouts"
							);
							break;
						}
						expected = false;
				...
				if (!expected) {
					if (handlingTimeouts != null) Logger.normal(
						this,
						"Unlocked handler but still routing to " +
						currentlyRoutingTo +
						" - expected because have timed out so a fork might have succeeded and we might be waiting for the original"
					);
					else Logger.error(
						this,
						"Unlocked handler but still routing to " +
		...
		if (
			fetchingOfferedKeyFrom != null && !fetchingOfferedKeyFrom.isEmpty()
		) {
			if (!(reassigned || wasLocal)) {
				...
				if (!expected) Logger.error( // Fork succeeds can't happen for fetch-offered-keys.
					this,
	...
	public void unlockHandler(boolean noRecord) {
		...
		if (canUnlock) innerUnlock(noRecordUnlock);
			...
			Logger.normal(
				this,
				"Cannot unlock yet in unlockHandler, still sending requests"
			);
		}
	}
	...
	public synchronized String toString() {
		...
		if (fetchingOfferedKeyFrom != null) sb
			.append(" (fetch offered keys from ")
			.append(fetchingOfferedKeyFrom.size())
			.append(")");
	...
	private static final long LOGGED_STILL_PRESENT_INTERVAL = SECONDS.toMillis(
		60
	);
	...
	public void maybeLogStillPresent(long now, Long uid) {
		if (now - createdTime > RequestTracker.TIMEOUT) {
			synchronized (this) {
				if (
					now - loggedStillPresent < LOGGED_STILL_PRESENT_INTERVAL
				) return;
