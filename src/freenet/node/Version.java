 ...
 *
 * <p>Note: when using the fields of this class, you must note that the Java compiler compiles
 * <b>constant final static</b> fields into the class definitions of classes that use them. This
 * might not be the most appropriate behaviour when, eg. creating a class that reports statistics.
 *
 * <p>A final static field can be made "non-constant" in the eyes of the compiler by initialising it
 * with the results of a method, eg <code>T identity(T o)
 * { T o; }</code>; however the "constant" behaviour might be required in some cases. A more
 * flexible solution is to add a method that returns the field, eg {@link #publicVersion()}, and
 * choose between the method and the field as necessary.
 *
 * <pre>$ grep -R "\WVersion\.\w*[^(a-zA-Z]" src</pre>
 *
 * can be used to find the places in the source code whose logic needs to be checked for this.
 */
public class Version {
  ...
  /**
   * The current tree version. FIXME: This is part of the node compatibility computations, so cannot
   * be safely changed!!! Hence publicVersion ...
   */
  public static final String nodeVersion = "0.7";

  /** The version for publicity purposes i.e. the version of the node that has been released. */
  ...
  /**
   * Use this method when you want the value returned to be the run-time version, not the build-time
   * version.
   *
   * <p>For example, if you compile your class, then modify this class and compile it (the build
   * script does this automatically), but don't re-compile your original class, then that will still
   * have the old version compiled into it, since it is a static constant.
   *
  ...
  /**
   * @return The lowest build number with which the node will connect and exchange data normally.
   */
  public static int lastGoodBuild() {
    if (System.currentTimeMillis() >= transitionTime) return newLastGoodBuild;
    else return oldLastGoodBuild;
  }
  ...
  /**
   * @return true if requests should be accepted from nodes brandishing this version string
   */
  public static boolean checkGoodVersion(String version) {
    if (version == null) {
      Logger.error(Version.class, "version == null!", new Exception("error"));
      return false;
    }
    ...
    if (sameVersion(v)) {
      try {
        ...
        if (logMINOR) Logger.minor(Version.class, "Not accepting (" + e + ") from " + version);
        return false;
      }
    }
    if (stableVersion(v)) {
    ...
    if (logDEBUG) Logger.minor(Version.class, "Accepting: " + version);
    return true;
  }

  /**
   * @return true if requests should be accepted from nodes brandishing this version string, given
   *     an arbitrary lastGoodVersion
   */
  public static boolean checkArbitraryGoodVersion(String version, String lastGoodVersion) {
    ...
    if (lastGoodVersion == null) {
      Logger.error(Version.class, "lastGoodVersion == null!", new Exception("error"));
      return false;
    }
    String[] v = Fields.commaList(version);
  ...
  public static String explainBadVersion(String version) {
    ...
    if (sameVersion(v)) {
      try {
        int build = Integer.parseInt(v[3]);
        int req = lastGoodBuild();
        if (build < req) return "Build older than last good build " + req;
      } catch (NumberFormatException e) {
    ...
    try {
      ...
      throw (VersionParseException)
          new VersionParseException(
                  "Got NumberFormatException on " + v[3] + " : " + e + " for " + version)
              .initCause(e);
    }
  }
  ...
  /**
   * Update static variable highestSeenBuild anytime we encounter a new node with a higher version
   * than we've seen before
   */
  public static void seenVersion(String version) {
    String[] v = Fields.commaList(version);

    if ((v == null) || (v.length < 3)) return; // bad, but that will be discovered elsewhere
  ...
  /**
   * @return true if the string describes the same node version as ours. Note that the build number
   *     may be different, and is ignored.
   */
  public static boolean sameVersion(String[] v) {
    return v[0].equals(nodeName) && v[1].equals(nodeVersion) && (v.length >= 4);
  }

  /**
   * @return true if the string describes the same node version as an arbitrary one. Note that the
   *     build number may be different, and is ignored.
   */
  ...
  private static boolean stableVersion(String[] v) {
    return v[0].equals(nodeName) && v[1].equals(stableNodeVersion) && (v.length >= 4);
  }
