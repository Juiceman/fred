...
public class FailureTable {
	...
	public void onFound(KeyBlock block) {
		...
		if (!(node.isEnableULPRDataPropagation() || node.isEnablePerNodeFailureTables())) {
			if (logMINOR) {
				Logger.minor(this, "Ignoring onFound because enable ULPR = " + node.isEnableULPRDataPropagation() + " and enable failure tables = " + node.isEnablePerNodeFailureTables());
			}
			return;
		}
		...
		if (!node.isEnableULPRDataPropagation()) {
			return;
		}
		entry.offer();
	}
