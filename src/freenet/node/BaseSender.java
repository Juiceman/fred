...
public abstract class BaseSender implements ByteCounter {
	...
	static public int calculateTimeout(boolean realTimeFlag, short htl, Node node) {
		...
		timeout = (timeout * ((double)htl + EXTRA_HOPS_AT_BOTTOM) / (EXTRA_HOPS_AT_BOTTOM +
				   (double) node.maxHTL()));
		return (int)timeout;
	}
		...
		while(true) {
			...
			if(expectedAcceptState == RequestLikelyAcceptedState.UNKNOWN) {
				...
				if(expectedAcceptState != null) {
					if(logMINOR) {
						Logger.minor(this, "Predicted accept state for "+this+" : "+expectedAcceptState+" realtime="
									 +realTimeFlag);
					}
					...
					if(lastNext == next && lastExpectedAcceptState == RequestLikelyAcceptedState.GUARANTEED &&
						...
						Logger.warning(this, "Rejected overload (last time) yet expected state was "+lastExpectedAcceptState
									   +" is now "+expectedAcceptState+" from "+next.shortToString()+" ("+next.getVersionNumber()+")");
				...
				if(expectedAcceptState == null) {
					...
					if(waited == null) {
						...
						if(logMINOR) {
							Logger.minor(this, "Sending to "+next+ " after waited for "+TimeUtil.formatTime(
											 endTime-startTime)+" realtime="+realTimeFlag);
						}
						expectedAcceptState = waiter.getAcceptedState();
					}
				...
				lastNext = next;
				if(logMINOR) {
					Logger.minor(this, "Leaving new load management big block: Predicted accept state for "+this+" : "
								 +expectedAcceptState+" realtime="+realTimeFlag+" for "+next);
				}
	...
	private PeerNode closerPeer(HashSet<PeerNode> exclude, long now, boolean newLoadManagement) {
		return node.peers.closerPeer(sourceForRouting(), exclude, target, true,
									 node.isAdvancedModeEnabled(), -1, null,
									 2.0, isInsert() ? null : key, htl, ignoreLowBackoff(), source == null, realTimeFlag, null, false,
									 now, newLoadManagement);
	}
	...
	private void logDelta(long delta, int tryCount, boolean waitedForLoadManagement,
						  boolean retriedForLoadManagement) {
		long longTimeout = getLongSlotWaiterTimeout();
		if((delta > longTimeout) || tryCount > 3) {
			Logger.error(this, "Took "+tryCount+" tries in "+TimeUtil.formatTime(delta, 2,
						 true)+" waited="+waitedForLoadManagement+" retried="+retriedForLoadManagement+
						 (realTimeFlag ? " (realtime)" : " (bulk)")+((source == null)?" (local)":" (remote)"));
		} else if((delta > longTimeout / 5) || tryCount > 1) {
			Logger.warning(this, "Took "+tryCount+" tries in "+TimeUtil.formatTime(delta, 2,
						   true)+" waited="+waitedForLoadManagement+" retried="+retriedForLoadManagement+
						   (realTimeFlag ? " (realtime)" : " (bulk)")+((source == null)?" (local)":" (remote)"));
		} else if(logMINOR && (waitedForLoadManagement || retriedForLoadManagement)) {
			Logger.minor(this, "Took "+tryCount+" tries in "+TimeUtil.formatTime(delta, 2,
						 true)+" waited="+waitedForLoadManagement+" retried="+retriedForLoadManagement+
						 (realTimeFlag ? " (realtime)" : " (bulk)")+((source == null)?" (local)":" (remote)"));
		}
			...
			if(msg.getSpec() == DMT.FNPRejectedOverload) {
				...
				if (msg.getBoolean(DMT.IS_LOCAL)) {
					...
					if(msg.getSubMessage(DMT.FNPRejectIsSoft) != null && expectedAcceptState != null) {
						...
						next.noLongerRoutingTo(origTag, false);
						if(softRejectCount == null) {
							softRejectCount = new HashMap<PeerNode, Integer>();
						}
						Integer i = softRejectCount.get(next);
						if(i == null) {
							softRejectCount.put(next, 1);
						} else {
