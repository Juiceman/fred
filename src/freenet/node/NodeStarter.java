...
public class NodeStarter implements WrapperListener {
	...
	public static synchronized boolean isTestingVM() {
		if (isStarted) return isTestingVM;
	...
	public Integer start(String[] args) {
		...
		String builtWithMessage =
			"freenet.jar built with freenet-ext.jar Build #" +
		...
		} else configFilename = new File(args[0]);
		...
		try {
			...
			cfg =
				FreenetFilePersistentConfig.constructFreenetFilePersistentConfig(
					configFilename
				);
		} catch (IOException e) {
		...
		try {
			...
			logConfigHandler = new LoggingConfigHandler(
				loggingConfig,
				executor
			);
		} catch (InvalidConfigValueException e) {
			System.err.println(
				"Error: could not set up logging: " + e.getMessage()
			);
			e.printStackTrace();
		...
		NativeThread plug = new NativeThread(
			useless,
			"Plug",
		...
		try {
			...
			System.err.println(
				"Failed to load node: " + e.exitCode + " : " + e.getMessage()
			);
			e.printStackTrace();
	...
	public static RandomSource globalTestInit(
		...
		// Setup RNG
		RandomSource random = randomSource != null
			? randomSource
			: new Yarrow();
	...
	public static final class TestNodeParameters {
		...
		public File baseDirectory = new File(
			"freenet-test-node-" + UUID.randomUUID().toString()
		);
	 ...
	 * likely a config problem.
	 */
	public static Node createTestNode(TestNodeParameters params)
		throws NodeInitException {
		synchronized (NodeStarter.class) {
			if ((!isStarted) || (!isTestingVM)) throw new IllegalStateException(
				"Call globalTestInit() first!"
			);
		}
		...
		if (
			(!portDir.mkdir()) &&
			((!portDir.exists()) || (!portDir.isDirectory()))
		) {
			System.err.println("Cannot create directory for test");
			System.exit(NodeInitException.EXIT_TEST_ERROR);
		}
		...
		if (params.outputBandwidthLimit > 0) {
			configFS.put(
				"node.outputBandwidthLimit",
				params.outputBandwidthLimit
			);
		...
		configFS.put(
			...
			params.disableProbabilisticHTLs
		);
		...
		configFS.putSingle(
			...
			new File(portDir, "temp").toString()
		);
		configFS.putSingle(
			"node.install.storeDir",
			new File(portDir, "store").toString()
		);
		configFS.put("fcp.persistentDownloadsEnabled", false);
		configFS.putSingle(
			"node.throttleFile",
			new File(portDir, "throttle.dat").toString()
		);
		...
		if (params.ramStore) configFS.putSingle("node.storeType", "ram");
		...
		configFS.put(
			...
			params.enablePerNodeFailureTables
		);
		configFS.put(
			"node.enablePacketCoalescing",
			params.enablePacketCoalescing
		);
		...
		configFS.put(
			"node.opennet.connectToSeednodes",
			params.connectToSeednodes
		);
		...
		if (params.ipAddressOverride != null) configFS.putSingle(
			"node.ipAddressOverride",
			params.ipAddressOverride
		);
	...
	public static long getMemoryLimitMB() {
		...
		limit /= (1024 * 1024);
		if (limit > Integer.MAX_VALUE) return -1; // Seems unlikely. FIXME 2TB limit!
		return limit;
	}
	...
	public static long getMemoryLimitBytes() {
		...
		if (maxMemory == Long.MAX_VALUE) return maxMemory;
		else if (maxMemory <= 0) return -1;
		else {
	...
	public static final boolean isSomething32bits() {
		...
		return (
			!JVMVersion.is32Bit() &&
			!wrapperProperties
				.getProperty("wrapper.java.additional.auto_bits")
				.startsWith("32")
		);
	}
