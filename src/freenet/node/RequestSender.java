...
public final class RequestSender extends BaseSender implements PrioRunnable {
	...
	protected void routeRequests() {
		...
		while(true) {
			...
			if(htl <= 0) {
				...
				node.failureTable.onFinalFailure(key, null, htl, origHTL, FailureTable.RECENTLY_FAILED_TIME,
			...
			if(next == null) {
				if (logMINOR && rejectOverloads>0) {
					Logger.minor(this, "no more peers, but overloads ("+rejectOverloads+"/"+routeAttempts
								 +" overloaded)");
				}
				// Backtrack
				finish(ROUTE_NOT_FOUND, null, false);
	...
	private synchronized long timeSinceSentForTimeout() {
		...
		if(time > FailureTable.REJECT_TIME) {
			if(time < searchTimeout + SECONDS.toMillis(10)) {
	...
	private class MainLoopCallback implements SlowAsyncMessageFilterCallback {
		...
		public void onTimeout() {
			...
			Logger.warning(this, "Timed out after waiting "+searchTimeout+" on "+uid+" from "+waitingFor+" ("
						   +gotMessages+" messages; last="+lastMessage+") for "+uid+" noReroute="+noReroute);
			if(noReroute) {
				...
				node.failureTable.onFinalFailure(key, waitingFor, htl, origHTL, FailureTable.RECENTLY_FAILED_TIME,
												 FailureTable.REJECT_TIME, source);
	...
	private OFFER_STATUS tryOffer(final BlockOffer offer, final PeerNode pn, final OfferList offers) {
		...
		try {
			node.usm.addAsyncFilter(getOfferedKeyReplyFilter(pn, getOfferedTimeout),
	...
	private MessageFilter getOfferedKeyReplyFilter(final PeerNode pn, long timeout) {
		MessageFilter mfRO = MessageFilter.create().setSource(pn).setField(DMT.UID,
							 uid).setTimeout(timeout).setType(DMT.FNPRejectedOverload);
		MessageFilter mfGetInvalid = MessageFilter.create().setSource(pn).setField(DMT.UID,
									 uid).setTimeout(timeout).setType(DMT.FNPGetOfferedKeyInvalid);
		if(isSSK) {
			MessageFilter mfAltDF = MessageFilter.create().setSource(pn).setField(DMT.UID,
									uid).setTimeout(timeout).setType(DMT.FNPSSKDataFoundHeaders);
			return mfAltDF.or(mfRO.or(mfGetInvalid));
		} else {
			MessageFilter mfDF = MessageFilter.create().setSource(pn).setField(DMT.UID,
								 uid).setTimeout(timeout).setType(DMT.FNPCHKDataFound);
	...
	private OFFER_STATUS handleOfferTimeout(final BlockOffer offer, final PeerNode pn,
											OfferList offers) {
		try {
			node.usm.addAsyncFilter(getOfferedKeyReplyFilter(pn, GET_OFFER_LONG_TIMEOUT),
			new SlowAsyncMessageFilterCallback() {
				...
				public void onMatched(Message m) {
					...
					if(logMINOR) {
						Logger.minor(this, "Forked get offered key due to two stage timeout completed with status "+status
									 +" from message "+m+" for "+RequestSender.this+" to "+pn);
					}
				}
	...
	private OFFER_STATUS handleSSKOfferReply(Message reply, PeerNode pn,
		...
		if(reply.getSpec() == DMT.FNPRejectedOverload) {
			// Non-fatal, keep it.
			if(logMINOR) {
				Logger.minor(this, "Node "+pn+" rejected FNPGetOfferedKey for "+key+" (expired="
							 +offer.isExpired());
			}
			...
			// Fatal, delete it.
			if(logMINOR) {
				Logger.minor(this, "Node "+pn+" rejected FNPGetOfferedKey as invalid with reason "+reply.getShort(
								 DMT.REASON));
			}
			return OFFER_STATUS.TRY_ANOTHER;
			...
			// Wait for the data
			MessageFilter mfData = MessageFilter.create().setSource(pn).setField(DMT.UID,
								   uid).setTimeout(getOfferedTimeout).setType(DMT.FNPSSKDataFoundData);
			Message dataMessage;
			try {
			...
			if(pubKey == null) {
				MessageFilter mfPK = MessageFilter.create().setSource(pn).setField(DMT.UID,
									 uid).setTimeout(getOfferedTimeout).setType(DMT.FNPSSKPubKey);
				Message pk;
				try {
			...
			if(finishSSKFromGetOffer(pn, headers, sskData)) {
				if(logMINOR) {
					Logger.minor(this, "Successfully fetched SSK from offer from "+pn+" for "+key);
				}
				return OFFER_STATUS.FETCHING;
			} else {
				return OFFER_STATUS.TRY_ANOTHER;
	 ...
	 * */
	private OFFER_STATUS handleCHKOfferReply(Message reply, final PeerNode pn, final BlockOffer offer,
		...
		if(reply.getSpec() == DMT.FNPRejectedOverload) {
			...
			try {
				...
				BlockReceiver br = new BlockReceiver(node.usm, pn, uid, prb, this, node.getTicker(), true,
													 realTimeFlag, myTimeoutHandler, true);
				...
				br.receive(new BlockReceiverCompletion() {
					...
					public void blockReceiveFailed(
						...
						try {
							...
							{
								Logger.normal(this, "Transfer for offer failed ("+e.getReason()+"/"+RetrievalException.getErrString(
												  e.getReason())+"): "+e+" from "+pn, e);
							}
							if(offers != null) {
	...
	protected MessageFilter makeAcceptedRejectedFilter(PeerNode next,
		...
		MessageFilter mfAccepted = MessageFilter.create().setSource(next).setField(DMT.UID,
								   uid).setTimeout(acceptedTimeout).setType(DMT.FNPAccepted);
		MessageFilter mfRejectedLoop = MessageFilter.create().setSource(next).setField(DMT.UID,
									   uid).setTimeout(acceptedTimeout).setType(DMT.FNPRejectedLoop);
		MessageFilter mfRejectedOverload = MessageFilter.create().setSource(next).setField(DMT.UID,
										   uid).setTimeout(acceptedTimeout).setType(DMT.FNPRejectedOverload);
	...
	private MessageFilter createMessageFilter(int timeout, PeerNode next) {
		MessageFilter mfDNF = MessageFilter.create().setSource(next).setField(DMT.UID,
							  uid).setTimeout(timeout).setType(DMT.FNPDataNotFound);
		MessageFilter mfRF = MessageFilter.create().setSource(next).setField(DMT.UID,
							 uid).setTimeout(timeout).setType(DMT.FNPRecentlyFailed);
		MessageFilter mfRouteNotFound = MessageFilter.create().setSource(next).setField(DMT.UID,
										uid).setTimeout(timeout).setType(DMT.FNPRouteNotFound);
		MessageFilter mfRejectedOverload = MessageFilter.create().setSource(next).setField(DMT.UID,
										   uid).setTimeout(timeout).setType(DMT.FNPRejectedOverload);

		if(!isSSK) {
			MessageFilter mfRealDFCHK = MessageFilter.create().setSource(next).setField(DMT.UID,
										uid).setTimeout(timeout).setType(DMT.FNPCHKDataFound);
			return mfDNF.or(mfRF.or(mfRouteNotFound.or(mfRejectedOverload.or(mfRealDFCHK))));
		} else {
			MessageFilter mfPubKey = MessageFilter.create().setSource(next).setField(DMT.UID,
									 uid).setTimeout(timeout).setType(DMT.FNPSSKPubKey);
			MessageFilter mfDFSSKHeaders = MessageFilter.create().setSource(next).setField(DMT.UID,
										   uid).setTimeout(timeout).setType(DMT.FNPSSKDataFoundHeaders);
			MessageFilter mfDFSSKData = MessageFilter.create().setSource(next).setField(DMT.UID,
										uid).setTimeout(timeout).setType(DMT.FNPSSKDataFoundData);
			return mfDNF.or(mfRF.or(mfRouteNotFound.or(mfRejectedOverload.or(mfPubKey.or(mfDFSSKHeaders.or(
										mfDFSSKData))))));
		}
	}
	...
	private DO handleMessage(Message msg, boolean wasFork, PeerNode source, MainLoopCallback waiter) {
		...
		if(msg.getSpec() == DMT.FNPRejectedOverload) {
			if(handleRejectedOverload(msg, wasFork, source)) {
				return DO.WAIT;
			} else {
				return DO.FINISHED;
			}
		...
		if(isSSK && msg.getSpec() == DMT.FNPSSKDataFoundHeaders) {
			...
			if(logMINOR) {
				Logger.minor(this, "Got headers on "+uid);
			}
	...
	private void handleCHKDataFound(Message msg, final boolean wasFork, final PeerNode next,
		...
		final PartiallyReceivedBlock prb = new PartiallyReceivedBlock(Node.PACKETS_IN_BLOCK,
				Node.PACKET_SIZE);
			...
			public void blockReceived(byte[] data) {
				try {
					...
					try {
						...
						Logger.normal(this, "Got data but verify failed: "+e1, e1);
						node.failureTable.onFinalFailure(key, next, htl, origHTL, FailureTable.RECENTLY_FAILED_TIME,
														 FailureTable.REJECT_TIME, source);
						if(!wasFork) {
			...
			public void blockReceiveFailed(
				...
				try {
					...
					if (e.getReason()==RetrievalException.SENDER_DISCONNECTED) {
						...
						// A certain number of these are normal, it's better to track them through statistics than call attention to them in the logs.
					{
						Logger.normal(this, "Transfer failed ("+e.getReason()+"/"+RetrievalException.getErrString(
										  e.getReason())+"): "+e+" from "+next, e);
					}
					if(RequestSender.this.source == null) {
						Logger.normal(this, "Local transfer failed: "+e.getReason()+" : "+RetrievalException.getErrString(
										  e.getReason())+"): "+e+" from "+next, e);
					}
					...
					boolean timeout = (!br.senderAborted()) &&
									  (reason == RetrievalException.SENDER_DIED || reason == RetrievalException.RECEIVER_DIED
	...
	private void finishSSK(PeerNode next, boolean wasFork, byte[] headers, byte[] sskData) {
		try {
			...
			block = node.fetch((NodeSSK)key, false, canWriteClientCache, canWriteClientCache, canWriteDatastore,
							   false, null);
			if(block != null) {
	...
	public synchronized short waitUntilStatusChange(short mask) {
		...
		while(true) {
			...
			while(true) {
				...
				try {
					if(now >= deadline) {
						Logger.error(this, "Waited more than 5 minutes for status change on " + this + " current = " +
									 current + " and there was no change.");
						break;
					}

					if(logMINOR) {
						Logger.minor(this, "Waiting for status change on "+this+" current is "+current+" status is "
									 +status);
					}
					wait(deadline - now);
					...
					if(now >= deadline) {
						Logger.error(this, "Waited more than 5 minutes for status change on " + this + " current = " +
									 current + ", maybe nobody called notify()");
	...
	private void finish(int code, PeerNode next, boolean fromOfferedKey) {
		...
		synchronized(this) {
			if(status != NOT_FINISHED) {
				if(logMINOR) {
					Logger.minor(this, "Status already set to "+status+" - returning on "+this+" would be setting "+code
								 +" from "+next);
				}
				if(next != null) {
					next.noLongerRoutingTo(origTag, fromOfferedKey);
				}
				return;
			}
		...
		if(status == SUCCESS) {
			if((!isSSK) && transferTime > 0 && logMINOR) {
				...
				synchronized(avgTimeTaken) {
					...
					if(logMINOR) {
						Logger.minor(this, "Successful CHK request transfer "+transferTime+" average "
									 +avgTimeTakenTransfer);
					}
					if(logMINOR) {
						Logger.minor(this, "Search phase: median "+(avgTimeTaken.currentValue() -
	...
	public byte[] waitForOpennetNoderef() throws WaitedTooLongForOpennetNoderefException {
		synchronized(this) {
			...
			while(true) {
				...
				try {
					int waitTime = (int) Math.min(Integer.MAX_VALUE,
	...
	public void sentBytes(int x) {
		...
		if(logMINOR) {
			Logger.minor(this, "Sent bytes: "+x+" for "+this+" isSSK="+isSSK, new Exception("debug"));
		}
	...
	public void addListener(RequestSenderListener l) {
		...
		if (sentFinished) {
			...
			if (status!=NOT_FINISHED) {
				...
				Logger.error(this, "sentFinished is true but status is still NOT_FINISHED?!?! on "+this,
							 new Exception("error"));
			}
		}
	}
	...
	private void fireRequestSenderFinished(int status, boolean fromOfferedKey) {
		...
		synchronized (listeners) {
			...
			if(logMINOR) {
				Logger.minor(this, "Notifying "+listeners.size()+" listeners of status "+status);
			}
			for (RequestSenderListener l : listeners) {
				try {
	...
	protected void handleAcceptedRejectedTimeout(final PeerNode next,
		...
		try {
			node.usm.addAsyncFilter(mf, new SlowAsyncMessageFilterCallback() {
				...
				@Override
				public void onTimeout() {
					Logger.error(this, "Fatal timeout waiting for Accepted/Rejected from "+next+" on "
								 +RequestSender.this);
					next.fatalTimeout(origTag, false);
				}
