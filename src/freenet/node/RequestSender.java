...
public final class RequestSender extends BaseSender implements PrioRunnable {
	...
	public void run() {
			...
			.queueTimedJob(
				new Runnable() {
					...
					public void run() {
						...
						Logger.normal(
							this,
							"Reassigning to self on timeout: " +
							RequestSender.this
						);
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Leaving RequestSender.run() for " + uid
			);
		}
	}
	...
	private void startRequests() {
		if (tryOffersOnly) {
			if (logMINOR) Logger.minor(
				this,
				"Tried all offers, not doing a regular request for key"
			);
	...
	protected void routeRequests() {
		...
		while (true) {
			...
			if (dontDecrementHTLThisTime) {
				...
				if ((!starting) && (!canWriteStorePrev)) {
					...
					if (highHTLFailureCount++ >= MAX_HIGH_HTL_FAILURES) {
						if (logMINOR) Logger.minor(
							this,
							"Too many failures at non-cacheable HTL"
						);
					...
					htl = node.decrementHTL(
						(hasForwarded ? next : source),
						htl
					);
					if (logMINOR) Logger.minor(
						this,
						"Decremented HTL to " + htl
					);
				}
			}
			starting = false;
			...
			if (htl <= 0) {
				...
				node.failureTable.onFinalFailure(
					key,
					null,
					htl,
					origHTL,
			...
			if (recentlyFailed > now) {
				synchronized (this) {
					recentlyFailedTimeLeft = (int) Math.min(
						Integer.MAX_VALUE,
						recentlyFailed - now
					);
				}
			...
			if (next == null) {
				if (logMINOR && rejectOverloads > 0) Logger.minor(
					this,
	...
	private synchronized long timeSinceSentForTimeout() {
		...
		if (time > FailureTable.REJECT_TIME) {
			if (
				time < searchTimeout + SECONDS.toMillis(10)
			) return FailureTable.REJECT_TIME;
			Logger.error(
				this,
				"Very long time since sent: " +
				time +
				" (" +
				TimeUtil.formatTime(time, 2, true) +
				")"
			);
			return FailureTable.REJECT_TIME;
		}
	...
	private class MainLoopCallback implements SlowAsyncMessageFilterCallback {
		...
		public void onTimeout() {
			...
			if (noReroute) {
				waitingFor.localRejectedOverload(
					"FatalTimeoutForked",
					realTimeFlag
				);
			} else {
			...
			while (true) {
				Message msg;
				try {
					int timeout = (int) (Math.min(
							Integer.MAX_VALUE,
							deadline - System.currentTimeMillis()
						));
					msg = node.usm.waitFor(
						createMessageFilter(timeout, waitingFor),
						RequestSender.this
					);
				} catch (DisconnectedException e) {
					Logger.normal(
				...
				if (msg == null) {
					...
					Logger.error(
						this,
						"Fatal timeout waiting for reply after Accepted on " +
						this +
						" from " +
						waitingFor
					);
	...
	private void tryOffers(
		final OfferList offers,
		PeerNode pn,
		OFFER_STATUS status
	) {
		while (true) {
	...
	private OFFER_STATUS tryOffer(
		final BlockOffer offer,
		final PeerNode pn,
		final OfferList offers
	) {
		...
		Message msg = DMT.createFNPGetOfferedKey(
			key,
			offer.authenticator,
			pubKey == null,
			uid
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Took too long sending offer get to " + pn + " for " + key
			);
			return OFFER_STATUS.TRY_ANOTHER;
		}
			...
			node.usm.addAsyncFilter(
				...
				new SlowAsyncMessageFilterCallback() {
					...
					public void onTimeout() {
						Logger.warning(
							this,
							"Timeout awaiting reply to offer request on " +
							this +
							" to " +
							pn
						);
						// Two stage timeout.
						OFFER_STATUS status = handleOfferTimeout(
							offer,
							pn,
							offers
						);
	...
	private MessageFilter getOfferedKeyReplyFilter(
		final PeerNode pn,
		long timeout
	) {
		MessageFilter mfRO = MessageFilter.create()
			.setSource(pn)
			.setField(DMT.UID, uid)
			.setTimeout(timeout)
			.setType(DMT.FNPRejectedOverload);
		MessageFilter mfGetInvalid = MessageFilter.create()
			.setSource(pn)
			.setField(DMT.UID, uid)
			.setTimeout(timeout)
			.setType(DMT.FNPGetOfferedKeyInvalid);
		if (isSSK) {
			MessageFilter mfAltDF = MessageFilter.create()
				.setSource(pn)
				.setField(DMT.UID, uid)
				.setTimeout(timeout)
			...
			MessageFilter mfDF = MessageFilter.create()
				.setSource(pn)
				.setField(DMT.UID, uid)
				.setTimeout(timeout)
	...
	private OFFER_STATUS handleOfferTimeout(
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Disconnected (2): " + pn + " getting offer for " + key
			);
			return OFFER_STATUS.TRY_ANOTHER;
		}
	}

	private OFFER_STATUS handleSSKOfferReply(
		Message reply,
		PeerNode pn,
		BlockOffer offer
	) {
		if (reply.getSpec() == DMT.FNPRejectedOverload) {
			...
			// Wait for the data
			MessageFilter mfData = MessageFilter.create()
				.setSource(pn)
				.setField(DMT.UID, uid)
			...
			byte[] sskData =
				((ShortBuffer) dataMessage.getObject(DMT.DATA)).getData();
			if (pubKey == null) {
				MessageFilter mfPK = MessageFilter.create()
					.setSource(pn)
					.setField(DMT.UID, uid)
					.setTimeout(getOfferedTimeout)
				...
				try {
					pubKey = DSAPublicKey.create(
						((ShortBuffer) pk.getObject(
								DMT.PUBKEY_AS_BYTES
							)).getData()
					);
			...
			if (finishSSKFromGetOffer(pn, headers, sskData)) {
				if (logMINOR) Logger.minor(
					this,
					"Successfully fetched SSK from offer from " +
					pn +
					" for " +
					key
				);
	...
	private OFFER_STATUS handleCHKOfferReply(
		...
		if (reply.getSpec() == DMT.FNPRejectedOverload) {
			...
			finalHeaders = ((ShortBuffer) reply.getObject(
					DMT.BLOCK_HEADERS
				)).getData();
			// Receive the data
			...
			try {
				prb = new PartiallyReceivedBlock(
				...
				if (logMINOR) Logger.minor(
					this,
					"Receiving data (for offer reply)"
				);
				receivingAsync = true;
				br.receive(
					new BlockReceiverCompletion() {
						...
						public void blockReceived(byte[] data) {
							...
							try {
								...
								if (logMINOR) Logger.minor(
									this,
									"Received data from offer reply"
								);
						...
						public void blockReceiveFailed(RetrievalException e) {
							...
							try {
								if (
									...
									e.getReason() +
									"/" +
									RetrievalException.getErrString(
										e.getReason()
									) +
									"): " +
									e +
								...
								pn.transferFailed(
									"RequestSenderGetOfferedTransferFailed",
									realTimeFlag
								);
								if (offers != null) {
									offers.deleteLastOffer();
	...
	private MessageFilter createMessageFilter(int timeout, PeerNode next) {
		MessageFilter mfDNF = MessageFilter.create()
			.setSource(next)
			.setField(DMT.UID, uid)
			.setTimeout(timeout)
			.setType(DMT.FNPDataNotFound);
		MessageFilter mfRF = MessageFilter.create()
			.setSource(next)
			.setField(DMT.UID, uid)
			.setTimeout(timeout)
		...
		if (!isSSK) {
			MessageFilter mfRealDFCHK = MessageFilter.create()
				.setSource(next)
				.setField(DMT.UID, uid)
				.setTimeout(timeout)
				.setType(DMT.FNPCHKDataFound);
			return mfDNF.or(
				mfRF.or(mfRouteNotFound.or(mfRejectedOverload.or(mfRealDFCHK)))
			);
		} else {
			MessageFilter mfPubKey = MessageFilter.create()
				.setSource(next)
				.setField(DMT.UID, uid)
				.setTimeout(timeout)
				.setType(DMT.FNPSSKPubKey);
			MessageFilter mfDFSSKHeaders = MessageFilter.create()
				.setSource(next)
				.setField(DMT.UID, uid)
				.setTimeout(timeout)
				.setType(DMT.FNPSSKDataFoundHeaders);
			MessageFilter mfDFSSKData = MessageFilter.create()
				.setSource(next)
				.setField(DMT.UID, uid)
				.setTimeout(timeout)
				.setType(DMT.FNPSSKDataFoundData);
				...
				mfRF.or(
					mfRouteNotFound.or(
						mfRejectedOverload.or(
							mfPubKey.or(mfDFSSKHeaders.or(mfDFSSKData))
						)
					)
				)
			);
	...
	private DO handleMessage(
		...
		if (isSSK && msg.getSpec() == DMT.FNPSSKDataFoundHeaders) {
			...
			waiter.headers = ((ShortBuffer) msg.getObject(
					DMT.BLOCK_HEADERS
				)).getData();
	...
	private boolean handleSSKPubKey(Message msg, PeerNode next) {
		...
		try {
			...
			Logger.error(
				this,
				"Invalid pubkey from " + source + " on " + uid + " (" + e + ')'
			);
			int t = timeSinceSent();
			node.failureTable.onFailed(key, next, htl, t, t);
			next.noLongerRoutingTo(origTag, false);
	...
	private void handleCHKDataFound(
		Message msg,
		final boolean wasFork,
		final PeerNode next,
		final MainLoopCallback waiter
	) {
		// Found data
		...
		final PartiallyReceivedBlock prb = new PartiallyReceivedBlock(
			Node.PACKETS_IN_BLOCK,
			Node.PACKET_SIZE
		);
		...
		synchronized (this) {
			...
			if (
				this.status == SUCCESS ||
				(this.prb != null && transferringFrom != null)
			) failNow = true;
			if (
				(!wasFork) &&
				(this.prb == null || !this.prb.allReceivedAndNotAborted())
			) this.prb = prb;
			notifyAll();
		}
		if (!wasFork) fireCHKTransferBegins(); // If we find the data we will offer it to the requester. // Don't fire transfer begins on a fork since we have not set headers or prb.
		...
		if (failNow) {
			if (logMINOR) Logger.minor(
				this,
				"Terminating forked transfer on " + this + " from " + next
			);
			prb.abort(
				RetrievalException.CANCELLED_BY_RECEIVER,
				"Cancelling fork",
				true
			);
			br.receive(
				new BlockReceiverCompletion() {
					...
					public void blockReceived(byte[] buf) {
						if (!wasFork) origTag.senderTransferEnds(
							(NodeCHK) key,
							RequestSender.this
						);
						next.noLongerRoutingTo(origTag, false);
		...
		} else if (logMINOR) Logger.minor(this, "Receiving data from fork");
			...
			new BlockReceiverCompletion() {
				...
				public void blockReceiveFailed(RetrievalException e) {
					try {
						...
						if (RequestSender.this.source == null) Logger.normal(
							this,
						...
						if (!prb.abortedLocally()) next.localRejectedOverload(
							"TransferFailedRequest" + e.getReason(),
							realTimeFlag
						);
						node.failureTable.onFinalFailure(
							key,
	...
	private boolean handleRejectedOverload(
		Message msg,
		boolean wasFork,
		PeerNode next
	) {
		...
		if (msg.getBoolean(DMT.IS_LOCAL)) {
			...
			Logger.normal(
				this,
				"Local RejectedOverload after Accepted, moving on to next peer"
			);
	...
	private void handleRecentlyFailed(
		Message msg,
		boolean wasFork,
		PeerNode next
	) {
		...
		if (timeLeft <= 0) {
			if (timeLeft == 0) {
				if (logMINOR) Logger.minor(
					this,
					"RecentlyFailed: timeout already consumed on " + this
				);
			} else {
	...
	private void finishSSK(
		...
		try {
			...
			if (
				node.random.nextInt(RANDOM_REINSERT_INTERVAL) == 0
	 ...
	 */
	private boolean finishSSKFromGetOffer(
		PeerNode next,
		byte[] headers,
		byte[] sskData
	) {
		try {
			...
			Logger.error(
				this,
				"Failed to verify (from get offer): " + e + " from " + next,
				e
			);
			return false;
		} catch (KeyCollisionException e) {
	...
	protected Message createDataRequest() {
		...
		else req = DMT.createFNPSSKDataRequest( // if(key instanceof NodeSSK)
			uid,
			htl,
			(NodeSSK) key,
			pubKey == null
	...
	private void verifyAndCommit(byte[] headers, byte[] data)
	...
	public synchronized short waitUntilStatusChange(short mask) {
		if (mask == WAIT_ALL) throw new IllegalArgumentException(
			"Cannot ignore all!"
		);
		while (true) {
			long now = System.currentTimeMillis();
			long deadline =
				now +
				(realTimeFlag ? MINUTES.toMillis(5) : MINUTES.toMillis(21));
			while (true) {
				short current = mask; // If any bits are set already, we ignore those states.

				if (hasForwardedRejectedOverload) current |=
				WAIT_REJECTED_OVERLOAD;
	...
	private void finish(int code, PeerNode next, boolean fromOfferedKey) {
		...
		synchronized (this) {
			if (status != NOT_FINISHED) {
				...
				if (next != null) next.noLongerRoutingTo(
					origTag,
					fromOfferedKey
				);
				return;
			...
			if ((!isSSK) && transferTime > 0 && logMINOR) {
				...
				synchronized (avgTimeTaken) {
					...
					if (logMINOR) Logger.minor(
						...
						"ms, mean " +
						(avgTimeTaken.meanValue() -
							avgTimeTakenTransfer.meanValue()) +
						"ms"
					);
				}
			}
	...
	private boolean finishOpennet(final PeerNode next) {
		...
		try {
			byte[] noderef = OpennetManager.waitForOpennetNoderef(
				false,
				next,
				uid,
				this,
			...
			SimpleFieldSet ref = OpennetManager.validateNoderef(
				noderef,
				0,
				noderef.length,
				next,
			...
			if (
				node.addNewOpennetNode(ref, ConnectionType.PATH_FOLDING) == null
			) {
				if (logMINOR) Logger.minor(
					this,
					"Don't want noderef on " + this
				);
				...
				if (source == null) {
					...
					if (logMINOR) {
						Logger.minor(
							this,
							"Delaying opennet completion for " +
							TimeUtil.formatTime(delay, 2, true)
						);
					}
				...
				Logger.normal(
					this,
					"Added opennet noderef in " + this + " from " + next
				);
			}
			...
			Logger.error(
				this,
				"Could not parse opennet noderef for " + this + " from " + next,
				e
			);
			ackOpennet(next);
			return false;
			...
			Logger.error(
				this,
				"Bad signature on opennet noderef for " +
				this +
				" from " +
				next +
				" : " +
			...
			if (logMINOR) Logger.minor(
				this,
				"Not connected sending ConnectReply on " + this + " to " + next
			);
			origTag.finishedWaitingForOpennet(next);
		} catch (WaitedTooLongForOpennetNoderefException e) {
			Logger.error(
				this,
				"RequestSender timed out waiting for noderef from " +
				next +
				" for " +
				this
			);
			...
			synchronized (this) {
				...
				try {
					...
					if (logMINOR) {
						Logger.minor(
							this,
							"Not connected sending ConnectReply on " +
							this +
							" to " +
							next
						);
			...
			try {
				...
				Logger.error(
					this,
					"RequestSender FATAL TIMEOUT out waiting for noderef from " +
					next +
					" for " +
					this
				);
		...
		synchronized (this) {
			...
			while (true) {
				if (opennetFinished) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Grabbing opennet noderef on " + this,
						new Exception("debug")
					);
				...
				try {
					int waitTime = (int) Math.min(
						Integer.MAX_VALUE,
						OPENNET_TIMEOUT + startTime - System.currentTimeMillis()
					);
					if (waitTime > 0) {
						wait(waitTime);
						continue;
				...
				if (logMINOR) Logger.minor(
					this,
					"Took too long waiting for opennet ref on " + this
				);
				return null;
			}
		}
	...
	public void sentBytes(int x) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Sent bytes: " + x + " for " + this + " isSSK=" + isSSK,
			new Exception("debug")
		);
	...
	public void addListener(RequestSenderListener l) {
		...
		if (sentFinished) {
			...
			if (status != NOT_FINISHED) l.onRequestSenderFinished(
				status,
				sentFinishedFromOfferedKey,
				this
			);
			else Logger.error(
				this,
	...
	private void reassignToSelfOnTimeout(boolean fromOfferedKey) {
		...
		synchronized (listeners) {
			if (sentCHKTransferBegins) {
				Logger.error(
					this,
					"Transfer started, not dumping listeners when reassigning to self on timeout (race condition?) on " +
					this
				);
				return;
			}
			list = listeners.toArray(
				new RequestSenderListener[listeners.size()]
			);
			listeners.clear();
		}
	...
	BlockReceiverTimeoutHandler myTimeoutHandler =
		new BlockReceiverTimeoutHandler() {
			...
			public void onFatalTimeout(PeerContext receivingFrom) {
				Logger.error(
					this,
					"Fatal timeout receiving requested block on " +
					this +
					" from " +
					receivingFrom
				);
	...
	protected void timedOutWhileWaiting(double load) {
		...
		if (logMINOR) {
			if (source != null) Logger.minor(
				this,
				"Timed out while waiting for a slot on " + this
			);
			else Logger.minor(
				this,
				"Local request timed out while waiting for a slot on " + this
			);
		}
		finish(ROUTE_NOT_FOUND, null, false);
		node.failureTable.onFinalFailure(
	...
	@Override
	protected void handleAcceptedRejectedTimeout(
		final PeerNode next,
