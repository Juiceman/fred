...
public class Node implements TimeSkewDetectorCallback {
  ...
  public boolean isUsingWrapper() {
    if (nodeStarter != null && WrapperManager.isControlledByNativeWrapper()) return true;
    else return false;
  }
  ...
  Node(
    ...
    // Easy stuff
    String tmp =
        "Initializing Node using Freenet Build #"
            + Version.buildNumber()
            + " r"
            + Version.cvsRevision()
    ...
    } else this.fastWeakRandom = weakRandom;
    ...
    try {
      ...
      Logger.error(
          this,
          "Invalid trafficClass:" + trafficClassValue + " resetting the value to default.",
          e);
    ...
    nodeConfig.register(
        "maxTimeForSingleCompressor",
        "20m",
        sortOrder++,
        true,
        true,
        "Node.maxTimeForSingleCompressor",
        "Node.maxTimeForSingleCompressorLong",
        new IntCallback() {
          @Override
          public Integer get() {
          ...
          new SecurityLevelListener<SecurityLevels.PHYSICAL_THREAT_LEVEL>() {
            ...
            public void onChange(PHYSICAL_THREAT_LEVEL oldLevel, PHYSICAL_THREAT_LEVEL newLevel) {
              try {
                ...
                else nodeConfig.set("storePreallocate", true);
    ...
    nodeConfig.register(
        ...
        true,
        false,
        "Node.cachingFreenetStoreMaxSize",
        "Node.cachingFreenetStoreMaxSizeLong",
        new LongCallback() {
          @Override
          public Long get() {
            synchronized (Node.this) {
    ...
    nodeConfig.register(
        ...
        true,
        false,
        "Node.cachingFreenetStorePeriod",
        "Node.cachingFreenetStorePeriod",
        new LongCallback() {
          @Override
          public Long get() {
          ...
          public void set(Boolean val) {
            if (val) {
              for (UserAlert alert : clientCore.alerts.getAlerts())
                if (alert instanceof PeersOffersUserAlert) clientCore.alerts.unregister(alert);
  ...
  private <T extends StorableBlock> FreenetStore<T> makeStore(
    ...
    SaltedHashFreenetStore<T> fs =
        SaltedHashFreenetStore.<T>construct(
            getStoreDir(),
            type + "-" + store,
            cb,
            random,
  ...
  public void start(boolean noSwaps) throws NodeInitException {
    ...
    Logger.normal(
        this, "Freenet 0.7.5 Build #" + Version.buildNumber() + " r" + Version.cvsRevision());
    System.out.println(
        "Freenet 0.7.5 Build #" + Version.buildNumber() + " r" + Version.cvsRevision());
    Logger.normal(
  ...
  private void checkForEvilJVMBugs() {
    ...
    // If we are running a Sun/Oracle or Blackdown JVM, on Linux, and LD_ASSUME_KERNEL is not set,
    // then we are.
    ...
    // Should have no effect because if a user has downloaded a new enough file for Oracle to have
    // changed the name these bugs shouldn't apply.
    ...
    if ((!isOpenJDK) && (jvmVendor.startsWith("Sun ") || jvmVendor.startsWith("Oracle "))
      ...
      if (is150 || is160) {
        ...
        Logger.minor(
            this, "JVM version: " + javaVersion + " subver: " + subver + " from " + secondPart);
      }

    } else if (jvmVendor.startsWith("Apple ") || jvmVendor.startsWith("\"Apple ")) {
      // Note that Sun/Oracle does not produce VMs for the Macintosh operating system, dont ask the
      // user to find one...
    } else if (!isOpenJDK) {
    ...
    //          if(!(FileUtil.detectedOS == FileUtil.OperatingSystem.Windows || FileUtil.detectedOS
    // == FileUtil.OperatingSystem.MacOS))
  ...
  public ClientKeyBlock fetch(
    ...
    SSKBlock block =
        fetch(
            ...
            null);
    if (block == null) {
      if (logMINOR) Logger.minor(this, "Could not find key for " + clientSSK);
      return null;
    }
  ...
  public void handleNodeToNodeTextMessageSimpleFieldSet(
    ...
    if (logMINOR) Logger.minor(this, "Got node to node message: \n" + fs);
  ...
  public void connect(Node node, FRIEND_TRUST trust, FRIEND_VISIBILITY visibility)
      throws FSParseException,
          ...
          PeerTooOldException {
    peers.connect(
        node.darknetCrypto.exportPublicFieldSet(), darknetCrypto.packetMangler, trust, visibility);
  }
  ...
  public Set<ForwardPort> getPublicInterfacePorts() {
    ...
    // FIXME IPv6 support
    set.add(
        new ForwardPort("darknet", false, ForwardPort.PROTOCOL_UDP_IPV4, darknetCrypto.portNumber));
    if (opennet != null) {
      NodeCrypto crypto = opennet.crypto;
      if (crypto != null) {
        set.add(
            new ForwardPort("opennet", false, ForwardPort.PROTOCOL_UDP_IPV4, crypto.portNumber));
      }
    }
    return set;
  }
  ...
  public boolean shouldStoreDeep(Key key, PeerNode source, PeerNode[] routedTo) {
    ...
    if (source != null && !source.isLowUptime()) {
      if (Location.distance(source, target) < myDist) {
        if (logMINOR)
          Logger.minor(
              this, "Not storing because source is closer to target for " + key + " : " + source);
        return false;
      }
    }
  ...
  public boolean updateIsUrgent() {
    ...
    if (om != null) {
      if (om.announcer != null && om.announcer.isWaitingForUpdater()) return true;
    }
    if (peers.getPeerNodeStatusSize(PeerManager.PEER_NODE_STATUS_TOO_NEW, true)
        > PeerManager.OUTDATED_MIN_TOO_NEW_DARKNET) return true;
    return false;
  }
