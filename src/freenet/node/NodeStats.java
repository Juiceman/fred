...
public class NodeStats implements Persistable, BlockTimeCallback {
  ...
  private static class RequestsByLocation {
    ...
    /**
     * Get the request count bins. The total request count is placed at position zero of the given
     * array.
     */
  ...
  NodeStats(
    ...
    if (memoryLimit > 0 && memoryLimit < 100) {
      ...
      Logger.minor(
          this, "Severe memory pressure, setting 200 thread limit. Freenet may not work well!");
    ...
    statsConfig.register(
        ...
        new BooleanCallback() {
          ...
          public void set(Boolean val) throws InvalidConfigValueException {
            ...
            if (val) myMemoryChecker.start();
            else myMemoryChecker.terminate();
          }
        });
    if (statsConfig.getBoolean("memoryChecker")) myMemoryChecker.start();
    ...
    node.securityLevels.addNetworkThreatLevelListener(
   ...
   * (If no packets have been sent, the throttledPacketSendAverage should decrease; if it doesn't,
   * it may go high, and then no requests will be accepted, and it will stay high forever.
   */
  ...
  //  /** Proportion of the various bandwidth limits we can use without triggering slow-down
  // messages */
  ...
  public class PeerLoadStats {
    ...
    public PeerLoadStats(PeerNode peer, int transfersPerInsert, boolean realTimeFlag) {
      ...
      RunningRequestsSnapshot runningLocal =
          new RunningRequestsSnapshot(
              node.tracker, peer, false, ignoreLocalVsRemote, transfersPerInsert, realTimeFlag);

      int peers = node.peers.countConnectedPeers();

      // Peer limits are adjusted to deduct any requests already allocated by sourceRestarted()
      // requests.
      // I.e. requests which were sent before the peer restarted, which it doesn't know about or
      // failed for when the bootID changed.
      // We try to complete these quickly, but they can stick around for a while sometimes.

      // This implies that the sourceRestarted() requests are not counted when checking whether the
      // peer is over the limit.
          ...
          (int)
              Math.max(
                  ...
                  getPeerLimit(
                      ...
                      realTimeFlag,
                      peers,
                      runningLocal.expectedTransfersOutCHKSR
                          + runningLocal.expectedTransfersOutSSKSR));
      maxTransfersOut =
          calculateMaxTransfersOut(peer, realTime, nonOverheadFraction, maxTransfersOutUpperLimit);

      outputBandwidthPeerLimit =
          getPeerLimit(
              peer,
              outputBandwidthUpperLimit - outputBandwidthLowerLimit,
              false,
              transfersPerInsert,
              realTimeFlag,
              peers,
              runningLocal.calculateSR(ignoreLocalVsRemote, false));
      inputBandwidthPeerLimit =
          getPeerLimit(
              peer,
              inputBandwidthUpperLimit - inputBandwidthLowerLimit,
              true,
              transfersPerInsert,
              realTimeFlag,
              peers,
              runningLocal.calculateSR(ignoreLocalVsRemote, true));

      this.averageTransfersOutPerInsert = transfersPerInsert;

      RunningRequestsSnapshot runningGlobal =
          new RunningRequestsSnapshot(
              node.tracker, ignoreLocalVsRemote, transfersPerInsert, realTimeFlag);
      expectedTransfersInCHK =
          runningGlobal.expectedTransfersInCHK - runningLocal.expectedTransfersInCHK;
      expectedTransfersInSSK =
          runningGlobal.expectedTransfersInSSK - runningLocal.expectedTransfersInSSK;
      expectedTransfersOutCHK =
          runningGlobal.expectedTransfersOutCHK - runningLocal.expectedTransfersOutCHK;
      expectedTransfersOutSSK =
          runningGlobal.expectedTransfersOutSSK - runningLocal.expectedTransfersOutSSK;
  ...
  class RunningRequestsSnapshot {
    ...
    public double calculateMinimum(
      ...
      return new RunningRequestsSnapshot(averageTransfersPerInsert)
          .calculate(ignoreLocalVsRemoteBandwidthLiability, input);
    }
  }
   ...
   */
  public RejectReason shouldRejectRequest(
    ...
    synchronized (serializeShouldRejectRequest) {
      ...
      //          slowDown(">softThreadLimit", isLocal, isInsert, isSSK, isOfferReply, realTimeFlag,
      // tag);
  ...
  //      if(logMINOR) Logger.minor(this, "Slow down: "+reason+" insert="+isInsert+" SSK="+isSSK+"
   ...
   * @param hasInStore True if we have the data in the store, and can return it, so won't need to
   *     route it onwards.
  ...
  private String checkBandwidthLiability(
      ...
      boolean realTimeFlag,
      int maxOutputTransfers,
      int maxOutputTransfersPeerLimit,
      UIDTag tag) {
    ...
    if (SEND_LOAD_STATS_NOTICES && source != null) {
      ...
      source.onSetPeerAllocation(
          input, (int) thisAllocation, transfersPerInsert, maxOutputTransfers, realTimeFlag);
    }
    ...
    if (bandwidthLiabilityOutput > bandwidthAvailableOutputLowerLimit) {
      ...
      double peerUsedBytes =
          getPeerBandwidthLiability(peerRequestsSnapshot, source, isSSK, transfersPerInsert, input);
      if (peerUsedBytes > thisAllocation) {
        ...
        //              // Allow any node to use one request of each type. They'll just have to get
        // backed off if necessary.
        //              slowDownLimit = Math.max(slowDownLimit,
        // requestsSnapshot.calculateMinimum(input, ignoreLocalVsRemoteBandwidthLiability,
        // transfersPerInsert));
        //              if(peerUsedBytes > slowDownLimit) {
        //                  // Sender should slow down if we are using more than 80% of our fair
        // share of capacity.
        //                  slowDown(name+" bandwidth liability: fairness between peers", isLocal,
        // isInsert, isSSK, isOfferReply, realTimeFlag, tag);
        //              }
      }
  ...
  private String checkMaxOutputTransfers(
    ...
    if (peerOutTransfers <= maxTransfersOutPeerLimit) {
      ...
      // FIXME slowdown
      //          if(peerOutTransfers > Math.max(1, maxTransfersOutPeerLimit *
      // SOFT_REJECT_MAX_BANDWIDTH_USAGE))
      //              // Send slow-down's if we are using more than 80% of our peer limit.
      //              slowDown("TooManyTransfers: Fair sharing between peers", isLocal, isInsert,
      // isSSK, isOfferReply, realTime, tag);
      return null;
    }
    rejected(
  ...
  private double getPeerLimit(
      PeerNode source,
      double totalGuaranteedBandwidth,
      boolean input,
      int transfersPerInsert,
      boolean realTimeFlag,
      int peers,
      double sourceRestarted) {
  ...
  private RunningAverage getThrottle(
      boolean isLocal, boolean isInsert, boolean isSSK, boolean isSent) {
    if (isLocal) {
      if (isInsert) {
        if (isSSK) {
          return isSent
  ...
  public TimedStats[] getMandatoryBackoffStatistics(boolean realtime) {
    ...
    if (realtime) {
      ...
      synchronized (avgMandatoryBackoffTimesRT) {
        for (Map.Entry<String, TrivialRunningAverage> entry :
            avgMandatoryBackoffTimesRT.entrySet()) {
          TrivialRunningAverage avg = entry.getValue();
          entries[i++] =
              new TimedStats(
                  entry.getKey(),
                  avg.countReports(),
                  (long) avg.currentValue(),
                  (long) avg.totalValue());
        }
      }
      ...
      synchronized (avgMandatoryBackoffTimesBulk) {
        for (Map.Entry<String, TrivialRunningAverage> entry :
            avgMandatoryBackoffTimesBulk.entrySet()) {
          TrivialRunningAverage avg = entry.getValue();
          entries[i++] =
              new TimedStats(
                  entry.getKey(),
  ...
  public TimedStats[] getRoutingBackoffStatistics(boolean realtime) {
    if (realtime) {
      ...
      synchronized (avgRoutingBackoffTimesBulk) {
        for (Map.Entry<String, TrivialRunningAverage> entry :
            avgRoutingBackoffTimesBulk.entrySet()) {
          TrivialRunningAverage avg = entry.getValue();
          entries[i++] =
              new TimedStats(
                  entry.getKey(),
  ...
  public TimedStats[] getTransferBackoffStatistics(boolean realtime) {
    if (realtime) {
      ...
      synchronized (avgTransferBackoffTimesRT) {
        for (Map.Entry<String, TrivialRunningAverage> entry :
            avgTransferBackoffTimesRT.entrySet()) {
          TrivialRunningAverage avg = entry.getValue();
          entries[i++] =
              new TimedStats(
                  entry.getKey(),
      ...
      synchronized (avgTransferBackoffTimesBulk) {
        for (Map.Entry<String, TrivialRunningAverage> entry :
            avgTransferBackoffTimesBulk.entrySet()) {
          TrivialRunningAverage avg = entry.getValue();
          entries[i++] =
              new TimedStats(
                  entry.getKey(),
  ...
  public TimedStats[] getDatabaseJobExecutionStatistics() {
    ...
    synchronized (avgDatabaseJobExecutionTimes) {
      for (Map.Entry<String, TrivialRunningAverage> entry :
          avgDatabaseJobExecutionTimes.entrySet()) {
        TrivialRunningAverage avg = entry.getValue();
        entries[i++] =
            new TimedStats(
                entry.getKey(),
  ...
  public PeerLoadStats createPeerLoadStats(
      PeerNode peer, int transfersPerInsert, boolean realTimeFlag) {
  ...
  public RunningRequestsSnapshot getRunningRequestsTo(
    ...
    return new RunningRequestsSnapshot(
        node.tracker, peerNode, true, false, outwardTransfersPerInsert(), realTimeFlag);
  }
  ...
  public final double getBandwidthLiabilityUsage() {
    ...
    RunningRequestsSnapshot requestsSnapshot =
        new RunningRequestsSnapshot(
            node.tracker, ignoreLocalVsRemoteBandwidthLiability, transfersPerInsert, false);
