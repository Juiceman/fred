...
public class IPDetectorPluginManager implements ForwardPortCallback {
	...
	public class PortForwardAlert extends AbstractUserAlert {
		...
		public HTMLNode getHTMLText() {
			...
			String url = ExternalLinkToadlet.escape(
			...
			String keySuffix = maybeForwarded
				? "MaybeForwarded"
				: "NotForwarded";
			if (portsNotForwarded.length == 1) {
				...
				Logger.error(
					this,
					"Unknown number of ports to forward: " +
	...
	public class MyUserAlert extends AbstractUserAlert {
		...
		public String getText() {
			...
			if (portsNotForwarded.length == 1) {
				...
				if (portsNotForwarded.length > 2) Logger.error(
					this,
					"Not able to tell user about more than 2 ports to forward! (" +
					portsNotForwarded.length +
					")"
				);
			}
	...
	IPDetectorPluginManager(Node node, NodeIPDetector detector) {
		...
		symmetricAlert = new MyUserAlert(
			l10n("symmetricTitle"),
			l10n("symmetric"),
			true,
			UserAlert.ERROR
		);
		portRestrictedAlert = new MyUserAlert(
			l10n("portRestrictedTitle"),
			l10n("portRestricted"),
			true,
			UserAlert.WARNING
		);
		restrictedAlert = new MyUserAlert(
			l10n("restrictedTitle"),
			l10n("restricted"),
			false,
			UserAlert.MINOR
		);
	...
	public int[] getUDPPortsNotForwarded() {
		...
		Status darknetStatus =
			(node.peers.anyDarknetPeers()
					? node.darknetCrypto.getDetectedConnectivityStatus()
					: AddressTracker.Status.DONT_KNOW);
		Status opennetStatus = om == null
			? Status.DONT_KNOW
			: om.crypto.getDetectedConnectivityStatus();
		if (
			om == null ||
			opennetStatus.ordinal() >= AddressTracker.Status.DONT_KNOW.ordinal()
		) {
			if (
				darknetStatus.ordinal() >=
				AddressTracker.Status.DONT_KNOW.ordinal()
			) {
				return new int[] {};
			} else {
				return new int[] {
					(darknetStatus.ordinal() <
							AddressTracker.Status.MAYBE_NATED.ordinal()
							? -1
							: 1) *
	...
	public String l10n(String key, String[] patterns, String[] values) {
		return NodeL10n.getBase()
			.getString("IPDetectorPluginManager." + key, patterns, values);
	}
	...
	public void unregisterDetectorPlugin(FredPluginIPDetector d) {
		...
		synchronized (this) {
			...
			if (count == 0) return;
			FredPluginIPDetector[] newPlugins =
				new FredPluginIPDetector[plugins.length - count];
			int x = 0;
			for (FredPluginIPDetector plugin : plugins) {
	...
	public void maybeRun() {
		...
		if (logMINOR) Logger.minor(
			this,
			"Maybe running IP detection plugins",
			new Exception("debug")
		);
		...
		synchronized (this) {
			...
			if (runners.size() == plugins.length) {
				if (logMINOR) Logger.minor(
					this,
					"Already running all IP detection plugins"
				);
				return;
			}
			...
			if (failedRunners.size() == plugins.length) {
				if (now - lastDetectAttemptEndedTime < MINUTES.toMillis(5)) {
					if (logMINOR) Logger.minor(
						this,
						"Last detect failed less than 5 minutes ago"
					);
					return;
				} else {
					if (logMINOR) Logger.minor(
						this,
						"Last detect failed, redetecting"
					);
					startDetect();
					return;
				}
			...
			if (peerCount == 0) {
				...
				if (
					shouldDetectWithPeers(now, peers, conns, nodeAddrs)
				) startDetect();
			}
		}
	}
	...
	private boolean shouldDetectNoPeers(long now) {
		if (now - lastDetectAttemptEndedTime < HOURS.toMillis(6)) {
			...
			if (logMINOR) Logger.minor(
				this,
				"No peers but detected less than 6 hours ago"
			);
			return false;
		} else {
			// Must try once!
	...
	private boolean shouldDetectWithPeers(
		...
		if (logMINOR) Logger.minor(
			this,
			"Checking whether should detect with " +
			peers.length +
			" peers and " +
			conns.length +
			" conns, counting peers..."
		);
		for (PeerNode p : peers) {
			...
			if (skip) continue;
			if (p.isConnected()) realConnections++;
			else {
				realDisconnected++;
				if (
					now - p.lastReceivedPacketTime() < MINUTES.toMillis(5)
				) recentlyConnected++;
			}
		}
		...
		if (realConnections == 0 && realDisconnected > 0) {
			...
			if (
				...
				if (now - firstTimeUrgent > MINUTES.toMillis(2)) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Detecting now as 2 minutes are up (have oldIPAddress)"
					);
				}
			} else {
				if (logMINOR) Logger.minor(
					this,
					"Detecting now (no oldIPAddress)"
				);
				// Detect immediately
				detect = true;
			}
			...
			if (logDEBUG) Logger.minor(
				this,
				"Not urgent; conns=" + conns.length + ", peers=" + peers.length
			);
			firstTimeUrgent = 0;
		}
	...
	private boolean shouldDetectDespiteRealIP(
		...
		if (now - lastDetectAttemptEndedTime < HOURS.toMillis(12)) {
			if (logMINOR) Logger.minor(
				this,
				"Node has directly detected IP and we have checked less than 12 hours ago"
			);
			return false;
		}
		...
		if (logMINOR) Logger.minor(
			this,
			"Checking whether should detect despite real IP..."
		);
		...
		for (PeerNode p : peers) {
			if (
				p.isConnected() ||
				(now - p.lastReceivedPacketTime() < HOURS.toMillis(24))
			) {
				...
				if (peer != null) {
					...
					if (
						p.isConnected() &&
						(addr != null) &&
						IPUtil.isValidAddress(peer.getAddress(), false)
					) {
						...
						if (!internal) {
							// Real IP address
							if (addressesConnected == null) addressesConnected =
							...
							if (addressesConnected.size() > 2) {
								...
								if (logMINOR) Logger.minor(
									this,
									"Node has directly detected IP and has connected to 3 real IPs"
								);
								return false;
							}
						}
		...
		if (!hasOldPeers) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Not detecting as less than 30 minutes old"
			);
			return false;
		}
		return true;
		...
		synchronized (this) {
			...
			for (FredPluginIPDetector plugin : plugins) {
				...
				node.executor.execute(
					d,
					"Plugin detector runner for " + plugin.getClass()
				);
			}
		}
	}
	...
	public class DetectorRunner implements Runnable {
		...
		public void realRun() {
			...
			try {
				...
				if (detected != null) {
					for (DetectedIP d : detected) v.add(d);
				}
				synchronized (IPDetectorPluginManager.this) {
					...
					if (v.isEmpty()) {
						...
						for (DetectedIP ip : v) {
							if (logMINOR) Logger.minor(
								this,
								"Detected IP: " + ip + " for " + plugin
							);
							if (
								!((ip.publicAddress == null) ||
									!IPUtil.isValidAddress(
										ip.publicAddress,
										false
									))
							) {
				...
				if (
					...
					countPortRestricted > 0 &&
					(countOpen + countFullCone + countRestricted == 0)
				) {
					proxyAlert.setAlert(portRestrictedAlert);
					proxyAlert.isValid(true);
					connectionType = DetectedIP.PORT_RESTRICTED_NAT;
				} else if (
					countRestricted > 0 && (countOpen + countFullCone == 0)
				) {
				...
				if (connectionType == DetectedIP.NO_UDP) {
					...
					synchronized (this) {
						if (noConnectivityAlert == null) noConnectivityAlert =
							toRegister = new SimpleUserAlert(
								false,
								l10n("noConnectivityTitle"),
								l10n("noConnectivity"),
					...
					if (toRegister != null) node.clientCore.alerts.register(
						toRegister
					);
				} else {
					UserAlert toKill;
					...
					if (toKill != null) node.clientCore.alerts.unregister(
						toKill
					);
				}
			} finally {
	...
	public void registerPortForwardPlugin(FredPluginPortForward forward) {
		...
		synchronized (this) {
			portForwardPlugins = Arrays.copyOf(
				portForwardPlugins,
				portForwardPlugins.length + 1
			);
			portForwardPlugins[portForwardPlugins.length - 1] = forward;
		}
		if (logMINOR) Logger.minor(
			this,
			"Registering a new port forward plugin : " + forward
		);
	...
	public void unregisterPortForwardPlugin(FredPluginPortForward forward) {
		synchronized (this) {
			...
			int x = 0;
			for (FredPluginPortForward portForwardPlugin : portForwardPlugins) {
				if (portForwardPlugin != forward) newPlugins[x++] =
					portForwardPlugin;
			}
		...
		for (final FredPluginPortForward plugin : plugins) {
			node.executor.execute(
				new Runnable() {
					...
					public void run() {
						try {
							plugin.onChangePublicPorts(
								newPorts,
								IPDetectorPluginManager.this
							);
						} catch (Throwable t) {
							Logger.error(
								this,
								"Changing public ports list on " +
								plugin +
								" threw: " +
								t,
								t
	...
	@Override
	public void portForwardStatus(
		Map<ForwardPort, ForwardPortStatus> statuses
	) {
		...
		for (ForwardPort p : currentPorts) {
			...
			if (status.status == ForwardPortStatus.DEFINITE_SUCCESS) {
				...
				Logger.normal(
					...
					" for " +
					p.protocol +
					" - port forward may have succeeded but strongly recommend out of band verification " +
					status.reasonString
				);
	...
	public void addConnectionTypeBox(HTMLNode contentNode) {
		...
		if (proxyAlert.isValid()) contentNode.addChild(
			node.clientCore.alerts.renderAlert(proxyAlert)
		);
	}
