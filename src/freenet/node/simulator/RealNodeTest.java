...
public class RealNodeTest {
  ...
  */
  static void makeKleinbergNetwork(
      ...
      RandomSource random) {
    if (idealLocations) {
      // First set the locations up so we don't spend a long time swapping just to stabilise each
      // network.
      ...
      if (countFullyConnected == nodes.length
        ...
        System.err.println(
            "Waiting for nodes to be fully connected: "
                ...
                + " / "
                + totalPeers
                + " connections total partial "
                + totalPartialConnections
                + " compatible "
                + totalCompatibleConnections
                + ") - backed off "
                + totalBackedOff
                + " ping min/avg/max "
                + (int) minPingTime
