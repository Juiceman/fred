 ...
 * the key from each node. Then insert it to one of them. Expected results: fast propagation via
 * ULPRs of the data to every node.
 *
 * <p>Note that this relies as much on per-node failure tables causing the request to go to every
 * node as it does on ULPRs to propagate the data to every node that it's been requested by.
 *
 * <p>This should be transformed into a Heavy Unit Test.
 *
 * @author toad
 */
public class RealNodeULPRTest extends RealNodeTest {
  ...
  static final boolean ENABLE_ULPRS =
      true; // This is the point of the test, but it's probably a good idea to be able to do a
            // comparison if we want to
  ...
  public static void main(String[] args)
    ...
    NodeStarter.globalTestInit(
        ...
        false,
        LogLevel.ERROR,
        "freenet.node.Location:normal,freenet.node.simulator.RealNodeRoutingTest:normal,freenet.node.NodeDispatcher:NORMAL,freenet.node.FailureTable:MINOR,freenet.node.Node:MINOR,freenet.node.Request:MINOR,freenet.io.comm.MessageCore:MINOR,freenet.node.PeerNode:MINOR,freenet.node.DarknetPeerNode:MINOR,freenet.io.xfer.PacketThrottle:MINOR,freenet.node.PeerManager:MINOR,freenet.client.async:MINOR",
        true);
    // For testing low-level stuff (connection bugs)
    // NodeStarter.globalTestInit(testName, false, LogLevel.ERROR,
    // "freenet.node.Location:normal,freenet.node.simulator.RealNodeRoutingTest:normal,freenet.node.Node:MINOR,freenet.io.comm.MessageCore:MINOR,freenet.node.PeerNode:MINOR,freenet.node.DarknetPeerNode:MINOR,freenet.node.FNP:MINOR,freenet.io.xfer.PacketThrottle:MINOR,freenet.node.PeerManager:MINOR", true);
    ...
    for (int i = 0; i < NUMBER_OF_NODES; i++) refs[i] = nodes[i].exportDarknetPublicFieldSet();
    ...
    for (int i = 0; i < NUMBER_OF_NODES * 5; i++) {
      if (i % NUMBER_OF_NODES == 0) Logger.normal(RealNodeRoutingTest.class, String.valueOf(i));
    ...
    for (Node node : nodes) node.start(false);
    ...
    for (int totalCount = 0; totalCount < NUMBER_OF_TESTS; totalCount++) {
      ...
      // Only those nodes which have been asked by another node (not directly here) for the key will
      // want it.
      ...
      for (int i = 0; i < nodes.length; i++) {
        ...
        try {
          nodes[i % nodes.length].clientCore.realGetKey(
              fetchKey, false, false, false, REAL_TIME_FLAG);
          ...
          switch (e.code) {
            ...
            default:
              System.err.println(
                  "Node " + i % nodes.length + " : UNEXPECTED ERROR: " + e.toString());
      ...
      System.err.println(
          "Nodes which were asked for the key by another node: "
              + visitedCount
              + " : "
              + sb.toString());
      ...
      while (true) {
        ...
        int count = 0;
        for (Node node : nodes) {
          if (node.hasKey(fetchKey.getNodeKey(false), true, true)) count++;
        }
        System.err.println(
            "T="
                + x
