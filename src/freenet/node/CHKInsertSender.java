...
public final class CHKInsertSender
	...
	private class BackgroundTransfer
		implements PrioRunnable, SlowAsyncMessageFilterCallback {
				...
				new BlockTransmitterCompletion() {
					...
					public void blockTransferFinished(boolean success) {
						if (logMINOR) Logger.minor(
							this,
							"Transfer completed: " + success + " for " + this
						);
						...
						if (pn.isConnected() && success) {
							...
							BackgroundTransfer.this.receivedNotice(
									false,
									false,
									false
								);
							pn.localRejectedOverload(
								"TransferFailedInsert",
								realTimeFlag
							);
						}
					}
		...
		private void startWait() {
			if (logMINOR) Logger.minor(
				this,
				"Waiting for completion notification from " + this
			);
			...
			try {
				node.usm.addAsyncFilter(
					getNotificationMessageFilter(false),
					BackgroundTransfer.this,
					null
				);
		...
		void start() {
			node.executor.execute(
				this,
				"CHKInsert-BackgroundTransfer for " +
				uid +
				" to " +
				pn.getPeer()
			);
		...
		private boolean receivedNotice(
			boolean success,
			boolean timeout,
			boolean kill
		) {
			...
			if (!noUnlockPeer) pn.noLongerRoutingTo(thisTag, false); // LOCKING: Do this outside the lock as pn can do heavy stuff in response (new load management). // Upstream (towards originator), of course, we can unlockHandler() as soon as all the transfers are finished. // Downstream (away from originator), we need to stay locked on the peer until the fatal timeout / the delayed notice.
			synchronized (backgroundTransfers) {
			...
			if (timeout && gotFatalTimeout) {
				Logger.error(
					this,
					"Second timeout waiting for final ack from " +
					pn +
					" on " +
					this
				);
		...
		public void onMatched(Message m) {
			...
			if (this.pn.equals(pn)) {
				...
				Logger.error(
					this,
					"received completion notice for wrong node: " +
					pn +
					" != " +
					this.pn
				);
		...
		private MessageFilter getNotificationMessageFilter(
			...
			return MessageFilter.create()
				...
				.setTimeout(
					longTimeoutAnyway
						? TRANSFER_COMPLETION_ACK_TIMEOUT_BULK
		...
		public void onTimeout() {
			...
			Logger.normal(
				this,
				"Timed out waiting for a final ack from: " + pn + " on " + this,
				new Exception("debug")
			);
			if (receivedNotice(false, true, false)) {
				pn.localRejectedOverload(
					"InsertTimeoutNoFinalAck",
					realTimeFlag
				);
				// First timeout. Wait for second timeout.
				try {
					node.usm.addAsyncFilter(
						getNotificationMessageFilter(true),
						this,
						CHKInsertSender.this
					);
				} catch (DisconnectedException e) {
					...
					if (logMINOR) Logger.minor(
						this,
						"Disconnected while adding filter after first timeout"
					);
	...
	CHKInsertSender(
		...
		if (realTimeFlag) {
			transferCompletionTimeout =
				TRANSFER_COMPLETION_ACK_TIMEOUT_REALTIME;
		} else {
	...
	static final long TRANSFER_COMPLETION_ACK_TIMEOUT_REALTIME =
		MINUTES.toMillis(1);
	static final long TRANSFER_COMPLETION_ACK_TIMEOUT_BULK = MINUTES.toMillis(
		5
	);
	...
	protected void routeRequests() {
		...
		while (true) {
			...
			if (
				...
				forkedRequestTag = new InsertTag(
					false,
				...
				Logger.normal(
					this,
					"FORKING CHK INSERT " + origUID + " to " + uid
				);
				nodesRoutedTo.clear();
	...
	private void handleRejectedTimeout(Message msg, PeerNode next) {
		...
		next.localRejectedOverload(
			"AfterInsertAcceptedRejectedTimeout",
			realTimeFlag
		);
	...
	private void handleDataInsertRejected(
		Message msg,
		PeerNode next,
		InsertTag thisTag
	) {
		...
		if (reason == DMT.DATA_INSERT_REJECTED_VERIFY_FAILED) {
			if (fromStore) {
				...
				try {
					if (!prb.allReceived()) Logger.error(
						this,
						...
						Logger.error(
							this,
							"Verify failed on " + next + " but data was valid!"
						);
					}
			...
			if (recvFailed) {
				...
				try {
					if (prb.allReceived()) {
						...
						Logger.warning(
							this,
							"Received all data but send failed to " + next
						);
					} else {
						if (prb.isAborted()) {
							Logger.normal(
							...
							this,
							"Send failed; have not yet received all data but not aborted: " +
							next
						);
					}
				} catch (AbortedException e) {
	...
	protected void handleAcceptedRejectedTimeout(
		...
		Logger.warning(
			this,
			"Timeout awaiting Accepted/Rejected " + this + " to " + next
		);
		...
		MessageFilter mf = makeAcceptedRejectedFilter(
			next,
			TIMEOUT_AFTER_ACCEPTEDREJECTED_TIMEOUT,
			tag
		);
		try {
			node.usm.addAsyncFilter(
				...
				new SlowAsyncMessageFilterCallback() {
						...
						if (
							...
							try {
								next.sendAsync(
									DMT.createFNPDataInsertRejected(
										uid,
										DMT.DATA_INSERT_REJECTED_TIMEOUT_WAITING_FOR_ACCEPTED
									),
					...
					public void onTimeout() {
						Logger.error(
							this,
							"Fatal: No Accepted/Rejected for " +
							CHKInsertSender.this
						);
	...
	private void finish(int code, PeerNode next) {
		...
		synchronized (this) {
			...
			if ((code == ROUTE_NOT_FOUND) && !hasForwarded) code =
				ROUTE_REALLY_NOT_FOUND;

			if (status != NOT_FINISHED) {
				if (status == RECEIVE_FAILED) {
					if (code == SUCCESS) Logger.error(
						this,
						"Request succeeded despite receive failed?! on " + this
					);
				} else if (status != TIMED_OUT) throw new IllegalStateException(
					"finish() called with " +
					code +
					" when was already " +
					status
			...
			if (logMINOR) Logger.minor(
				this,
				"Set status code: " + getStatusString() + " on " + uid
			);
		}
	...
	public boolean failIfReceiveFailed(InsertTag tag, PeerNode next) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Failing because receive failed on " + this
		);
	...
	public void onReceiveFailed() {
		...
		synchronized (backgroundTransfers) {
			...
			for (BackgroundTransfer t : backgroundTransfers) t.thisTag.handlingTimeout(
				t.pn
			);
		}
	...
	public synchronized String getStatusString() {
		...
		if (
			status == GENERATED_REJECTED_OVERLOAD
	...
	private void waitForBackgroundTransferCompletions() {
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Waiting for background transfer completions: " + this
			);
			...
			synchronized (backgroundTransfers) {
				...
				for (BackgroundTransfer transfer : transfers) {
					...
					if (!transfer.completedTransfer) {
						if (logMINOR) Logger.minor(
							this,
							"Waiting for transfer completion to " +
							transfer.pn +
							" : " +
							transfer
						);
					...
					if (!transfer.receivedCompletionNotice) {
						if (logMINOR) Logger.minor(
							this,
							"Waiting for completion notice from " +
							transfer.pn +
							" : " +
							transfer
						);
				...
				if (
					completedTransfers && completedNotifications
				) return !someFailed;
	...
	protected Message createDataRequest() {
		...
		if (forkOnCacheable != Node.FORK_ON_CACHEABLE_DEFAULT) {
			req.addSubMessage(
				DMT.createFNPSubInsertForkControl(forkOnCacheable)
			);
		}
		if (ignoreLowBackoff != Node.IGNORE_LOW_BACKOFF_DEFAULT) {
			req.addSubMessage(
				DMT.createFNPSubInsertIgnoreLowBackoff(ignoreLowBackoff)
			);
		}
	...
	protected void onAccepted(PeerNode next) {
		...
		MessageFilter mfInsertReply = MessageFilter.create()
			.setSource(next)
			.setField(DMT.UID, uid)
		...
		MessageFilter mfDataInsertRejected = MessageFilter.create()
			.setSource(next)
			.setField(DMT.UID, uid)
			.setTimeout(searchTimeout)
			.setType(DMT.FNPDataInsertRejected);
		MessageFilter mfTimeout = MessageFilter.create()
			.setSource(next)
			.setField(DMT.UID, uid)
			.setTimeout(searchTimeout)
			.setType(DMT.FNPRejectedTimeout);

		MessageFilter mf = mfInsertReply.or(
			mfRouteNotFound.or(
		...
		try {
			...
			if (logMINOR) Logger.minor(
				this,
				"Not connected sending DataInsert: " + next + " for " + uid
			);
		...
		final BackgroundTransfer transfer = startBackgroundTransfer(
			next,
			prb,
			thisTag
		);
			...
			if (msg == null) {
				...
				Runnable r = new Runnable() {
					...
					public void run() {
						...
						MessageFilter mf = mfInsertReply.or(
							mfRouteNotFound.or(
								mfDataInsertRejected.or(
									mfTimeout.or(mfRejectedOverload)
								)
							)
						);
						...
						while (true) {
							...
							if (msg.getSpec() == DMT.FNPRejectedOverload) {
								if (
									handleRejectedOverload(msg, waitingFor, tag)
								) {
