...
public class MainJarUpdater extends NodeUpdater implements Deployer {
  ...
  private class DependencyJarFetcher
      implements JarFetcher, ClientGetCallback, RequestClient, ClientEventListener {
    ...
    DependencyJarFetcher(
      ...
      try {
        tempFile =
            File.createTempFile(filename.getName(), NodeUpdateManager.TEMP_FILE_SUFFIX, parent);
      ...
      getter =
          new ClientGetter(
              ...
              new FileBucket(tempFile, false, false, false, false),
              null,
              null);
    ...
    public void onSuccess(FetchResult result, ClientGetter state) {
      ...
      if (!MainJarDependenciesChecker.validFile(
          tempFile, expectedHash, expectedLength, executable)) {
        Logger.error(
            this,
            "Unable to download dependency " + filename + " : not the expected size or hash!");
        System.err.println(
            "Download of "
                + filename
        ...
        if (cb != null)
          cb.onFailure(
              new FetchException(
                  FetchExceptionMode.BUCKET_ERROR,
                  "Downloaded jar from Freenet but failed consistency check: "
                      + tempFile
                      + " length "
                      + tempFile.length()
                      + " "));
      ...
      if (!FileUtil.renameTo(tempFile, filename)) {
        ...
        System.err.println(
            "Download of "
                + filename
                + " for update failed because cannot rename from "
                + tempFile);
        if (cb != null)
          cb.onFailure(
              new FetchException(
    ...
    public synchronized void receive(ClientEvent ce, ClientContext context) {
      if (ce instanceof SplitfileProgressEvent) lastProgress = (SplitfileProgressEvent) ce;
    }
    ...
    public synchronized HTMLNode renderRow() {
      ...
      if (uomFetcher != null) row.addChild("td").addChild("#", l10n("fetchingFromUOM"));
      else if (lastProgress == null)
        row.addChild(
            QueueToadlet.createProgressCell(
                false, true, COMPRESS_STATE.WORKING, 0, 0, 0, 0, 0, false, false));
      else
        row.addChild(
            QueueToadlet.createProgressCell(
                false,
                ...
                lastProgress.succeedBlocks,
                lastProgress.failedBlocks,
                lastProgress.fatallyFailedBlocks,
                lastProgress.minSuccessfulBlocks,
                lastProgress.totalBlocks,
                lastProgress.finalizedTotal,
                false));
      return row;
    }
  ...
  public JarFetcher fetch(
    ...
    DependencyJarFetcher fetcher =
        new DependencyJarFetcher(
            downloadTo, uri, expectedLength, expectedHash, cb, essential, executable);
    synchronized (fetchers) {
      fetchers.add(fetcher);
      if (essential) essentialFetchers.add(fetcher);
    }
    fetcher.start();
    if (manager.uom.fetchingUOM()) {
      if (essential) fetcher.fetchFromUOM();
    }
    return fetcher;
  }
  ...
  public void onStartFetchingUOM() {
    ...
    for (DependencyJarFetcher fetcher : f) fetcher.fetchFromUOM();
  }
  ...
  @Override
  public void multiFileReplaceReadyToDeploy(
      final MainJarDependenciesChecker.AtomicDeployer atomicDeployer) {
    if (this.manager.isAutoUpdateAllowed()) {
      atomicDeployer.deployMultiFileUpdateOffThread();
    } else {
      final long now = System.currentTimeMillis();
      System.err.println(
          "Not deploying multi-file update for "
              + atomicDeployer.name
      ...
      node.clientCore.alerts.register(
          new UserAlert() {
            ...
            public HTMLNode getHTMLText() {
              ...
              // FIXME separate button, then the alert could be dismissable? Only useful if it's
              // permanently dismissable though, which means a config setting as well...
            }
            ...
            public String dismissButtonText() {
              return NodeL10n.getBase()
                  .getString("UpdatedVersionAvailableUserAlert.updateNowButton");
            }
