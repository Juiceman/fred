...
class LegacyJarFetcher implements ClientGetCallback {
  ...
  public LegacyJarFetcher(
      FreenetURI uri, File saveTo, NodeClientCore core, LegacyFetchCallback cb) {
    this.uri = uri;
    this.saveTo = saveTo;
    ...
    if (blobBucket.size() > 0) {
      ...
      try {
        tmp =
            File.createTempFile(
                saveTo.getName(), NodeUpdateManager.TEMP_BLOB_SUFFIX, saveTo.getParentFile());
        tmp
            .deleteOnExit(); // To be used sparingly, as it leaks, but safe enough here as it should
      ...
      cg =
          new ClientGetter(
              ...
              null,
              new BinaryBlobWriter(new FileBucket(tempFile, false, false, false, false)));
      fetched = false;
    }
  }
