...
public class UpdateOverMandatoryManager implements RequestClient {
	...
	private static final Pattern mainBuildNumberPattern = Pattern.compile(
		"^main(?:-jar)?-(\\d+)\\.fblob$"
	);
	private static final Pattern mainTempBuildNumberPattern = Pattern.compile(
		"^main(?:-jar)?-(\\d+-)?(\\d+)\\.fblob\\.tmp*$"
	);
	...
	public boolean handleAnnounce(Message m, final PeerNode source) {
		...
		long revocationKeyLastTried = m.getLong(
			DMT.REVOCATION_KEY_TIME_LAST_TRIED
		);
		int revocationKeyDNFs = m.getInt(DMT.REVOCATION_KEY_DNF_COUNT);
		long revocationKeyFileLength = m.getLong(
			DMT.REVOCATION_KEY_FILE_LENGTH
		);
	...
	private void tellFetchers(PeerNode source) {
		...
		synchronized (dependencyFetchers) {
			fetchList = new HashSet<UOMDependencyFetcher>(
				dependencyFetchers.values()
			);
		}
	...
	private void tryFetchRevocation(final PeerNode source)
		throws NotConnectedException {
		// Try to transfer it.

		Message msg = DMT.createUOMRequestRevocation(
			updateManager.node.random.nextLong()
		);
		source.sendAsync(
			msg,
			new AsyncMessageCallback() {
				...
				public void disconnected() {
					// :(
					System.err.println(
						"Failed to send request for revocation key to " +
						source.userToString() +
						" (build #" +
						source.getSimpleVersion() +
			...
			.queueTimedJob(
				new Runnable() {
					...
					public void run() {
						...
						synchronized (UpdateOverMandatoryManager.this) {
							if (
								nodesSayKeyRevokedFailedTransfer.contains(
									source
								)
							) return;
							if (
	...
	private void handleMainJarOffer(
		...
		else whenToTakeOverTheNormalUpdater = System.currentTimeMillis() +
		GRACE_TIME;
		...
		Logger.normal(
			...
			TimeUtil.formatTime(whenToTakeOverTheNormalUpdater - now) +
			") file length " +
			mainJarFileLength +
		...
		if (
			mainJarVersion > Version.buildNumber() &&
			mainJarFileLength > 0 &&
			...
			if ((isOutdated) || whenToTakeOverTheNormalUpdater < now) {
				...
				// Fetch it
				try {
					FreenetURI mainJarURI = new FreenetURI(
						jarKey
	...
	private void sendUOMRequest(final PeerNode source, boolean addOnFail) {
		...
		if (logMINOR) Logger.minor(
			this,
			"sendUOMRequest" + name + "(" + source + "," + addOnFail + ")"
		);
		...
		synchronized (this) {
			...
			if (offeredVersion < updateVersion) {
				if (offeredVersion <= 0) Logger.error(
					this,
					"Not sending UOM " +
					lname +
					" request to " +
					...
					" request to " +
					source +
					" because we already have its offered version " +
					offeredVersion
				);
				return;
			}
			...
			if (askedSendJar.contains(source)) {
				if (logMINOR) Logger.minor(
					...
					" (" +
					lname +
					") so not re-asking yet."
				);
				return;
			}
			if (
				addOnFail &&
				askedSendJar.size() + sendingJar.size() >= MAX_NODES_SENDING_JAR
			) {
				HashSet<PeerNode> offeredJar = nodesOfferedMainJar;
				if (offeredJar.add(source)) System.err.println(
					"Offered " +
					lname +
					" jar by " +
					source.userToString() +
					" (already fetching from " +
					sendingJar.size() +
		...
		if (!wasFetchingUOM) this.updateManager.onStartFetchingUOM();

		Message msg = DMT.createUOMRequestMainJar(
			updateManager.node.random.nextLong()
		);

		try {
			System.err.println(
				"Fetching " + lname + " jar from " + source.userToString()
			);
			source.sendAsync(
				msg,
				new AsyncMessageCallback() {
					...
					public void sent() {
						// Timeout...
						updateManager.node.ticker.queueTimedJob(
							new Runnable() {
								@Override
								public void run() {
									synchronized (
										UpdateOverMandatoryManager.this
									) {
										// free up a slot
										if (
											!askedSendJar.remove(source)
	...
	protected void maybeRequestMainJar() {
		...
		synchronized (this) {
			if (
				nodesAskedSendMainJar.size() + nodesSendingMainJar.size() >=
				MAX_NODES_SENDING_JAR
			) return;
			if (nodesOfferedMainJar.isEmpty()) return;
			offers = nodesOfferedMainJar.toArray(
				new PeerNode[nodesOfferedMainJar.size()]
			);
		}
		for (PeerNode offer : offers) {
			...
			synchronized (this) {
				...
				if (nodesSendingMainJar.contains(offer)) continue;
				if (nodesAskedSendMainJar.contains(offer)) continue;
			}
	...
	private class PeersSayKeyBlownAlert extends AbstractUserAlert {
		...
		public HTMLNode getHTMLText() {
			...
			if (nodesSayBlownConnected.length > 0) div
				.addChild("p")
				.addChild("#", l10n("fetching"));
			...
			if (nodesSayBlownDisconnected.length > 0) {
				div
					.addChild("p")
					.addChild("#", l10n("disconnectedSayBlownLabel"));
				HTMLNode list = div.addChild("ul");
			...
			if (nodesSayBlownFailedTransfer.length > 0) {
				div
					.addChild("p")
					.addChild("#", l10n("failedTransferSayBlownLabel"));
				HTMLNode list = div.addChild("ul");
		...
		private String l10n(String key, String pattern, String value) {
			return NodeL10n.getBase()
				.getString("PeersSayKeyBlownAlert." + key, pattern, value);
		}
		...
		public String getText() {
			...
			if (nodesSayBlownConnected.length > 0) sb
				.append(l10n("fetching"))
				.append("\n\n");
			...
			if (nodesSayBlownConnected.length > 0) {
				...
				for (PeerNode pn : nodesSayBlownConnected) {
					sb
						.append(pn.userToString() + " (" + pn.getPeer() + ")")
						.append("\n");
				}
				sb.append("\n");
			}
	...
	public PeerNode[][] getNodesSayBlown() {
		...
		synchronized (this) {
			PeerNode[] nodesSayRevoked = nodesSayKeyRevoked.toArray(
				new PeerNode[nodesSayKeyRevoked.size()]
			);
			for (PeerNode pn : nodesSayRevoked) {
				if (
					nodesSayKeyRevokedFailedTransfer.contains(pn)
		...
		return new PeerNode[][] {
			nodesConnectedSayRevoked.toArray(
				new PeerNode[nodesConnectedSayRevoked.size()]
			),
			nodesDisconnectedSayRevoked.toArray(
				new PeerNode[nodesDisconnectedSayRevoked.size()]
			),
			nodesFailedSayRevoked.toArray(
				new PeerNode[nodesFailedSayRevoked.size()]
			),
		};
	}
	...
	public boolean handleRequestRevocation(Message m, final PeerNode source) {
		...
		final RandomAccessBuffer data =
			updateManager.revocationChecker.getBlobBuffer();
		...
		long length;
		length = data.size();
		prb = new PartiallyReceivedBulk(
			updateManager.node.getUSM(),
			length,
			Node.PACKET_SIZE,
			data,
			true
			...
			public void run() {
				try {
					if (!bt.send()) Logger.error(
						...
						this,
						"Sent revocation key blob to " + source.userToString()
					);
				} catch (DisconnectedException e) {
		...
		Message msg = DMT.createUOMSendingRevocation(
			uid,
			length,
			updateManager.getRevocationURI().toString()
		);
			...
			source.sendAsync(
				...
				new AsyncMessageCallback() {
					...
					public void acknowledged() {
						...
						// Send the data
						updateManager.node.executor.execute(
							r,
							"Revocation key send for " +
							uid +
							" to " +
							source.userToString()
						);
	...
	public boolean handleSendingRevocation(Message m, final PeerNode source) {
		...
		try {
			...
			Logger.error(
				this,
				"Failed receiving recovation because URI not parsable: " +
				e +
				" for " +
				key,
				e
		...
		if (updateManager.isBlown()) {
			if (logMINOR) Logger.minor(
				this,
				"Already blown, so not receiving from " +
				source +
				"(" +
				uid +
				")"
		...
		final BulkReceiver br = new BulkReceiver(
			prb,
			source,
			uid,
			updateManager.ctr
				...
				public void run() {
					try {
						if (br.receive()) processRevocationBlob(temp, source); // Success!
							...
							Logger.error(
								this,
								"Failed to transfer revocation certificate from " +
								source
							);
							System.err.println(
								"Failed to transfer revocation certificate from " +
	...
	private boolean mightBeRevoked() {
		...
		synchronized (this) {
			started = nodesSayKeyRevoked.toArray(
				new PeerNode[nodesSayKeyRevoked.size()]
			);
			transferring = nodesSayKeyRevokedTransferring.toArray(
	...
	void processRevocationBlob(final File temp, PeerNode source) {
		processRevocationBlob(
			new FileBucket(temp, true, false, false, true),
			source.userToString(),
			false
		);
	}
	...
	void processRevocationBlob(
		...
		try {
			...
			if (!fromDisk) updateManager.blow(
				"Somebody deleted " +
				temp +
				" ? We lost the revocation certificate from " +
				source +
		...
		FetchContext seedContext = updateManager.node.clientCore
			.makeClient((short) 0, true, false)
			.getFetchContext();
		FetchContext tempContext = new FetchContext(
			seedContext,
		...
		tempContext.maxOutputLength =
			NodeUpdateManager.MAX_REVOCATION_KEY_LENGTH;
		tempContext.maxTempLength =
			NodeUpdateManager.MAX_REVOCATION_KEY_TEMP_LENGTH;
		...
		ClientGetCallback myCallback = new ClientGetCallback() {
			...
			public void onFailure(FetchException e, ClientGetter state) {
				if (e.mode == FetchExceptionMode.CANCELLED) {
					...
					System.err.println(
						"Got revocation certificate from " +
						source +
						" (fatal error i.e. someone with the key inserted bad data) : " +
						e
					);
					// Blow the update, and propagate the revocation certificate.
					updateManager.revocationChecker.onFailure(
						e,
						state,
						cleanedBlob
					);
					...
					insertBlob(
						updateManager.revocationChecker.getBlobBucket(),
						"revocation",
			...
			public void onSuccess(FetchResult result, ClientGetter state) {
				...
				updateManager.revocationChecker.onSuccess(
					result,
					state,
					cleanedBlob
				);
		...
		ClientGetter cg = new ClientGetter(
			myCallback,
			updateManager.getRevocationURI(),
			tempContext,
			(short) 0,
	...
	protected void insertBlob(
		final RandomAccessBucket bucket,
		final String type,
		short priority
	) {
		ClientPutCallback callback = new ClientPutCallback() {
			...
			public void onFailure(InsertException e, BaseClientPutter state) {
				Logger.error(
					this,
					"Failed to insert " + type + " binary blob: " + e,
					e
				);
			}
		...
		ClientPutter putter = new ClientPutter(
			...
			null,
			true,
			updateManager.node.clientCore.clientContext,
			null,
			-1
		);
		try {
			...
			Logger.error(
				this,
				"Failed to start insert of " + type + " binary blob: " + e1,
				e1
			);
		} catch (PersistenceDisabledException e) {
			// Impossible
	...
	public void handleRequestJar(Message m, final PeerNode source) {
		...
		if (!source.sendingUOMJar(false)) {
			Logger.error(
				this,
				"Peer " + source + " asked for UOM main jar twice"
			);
			return;
		}
		...
		try {
			...
			msg = DMT.createUOMSendingMainJar(
				uid,
				length,
				uri.toString(),
				version
			...
			public void run() {
				try {
					if (!bt.send()) Logger.error(
						...
						this,
						"Sent " + name + " jar blob to " + source.userToString()
					);
					raf.close();
				} catch (DisconnectedException e) {
					// Not much we can do.
	...
	public boolean handleSendingMain(Message m, final PeerNode source) {
		...
		if (
			!jarURI.equals(updateManager.getURI().setSuggestedEdition(version))
		) {
			System.err.println(
				"Node sending us a main jar update (" +
				version +
				") from the wrong URI:\n" +
				"Node: " +
				source.userToString() +
		...
		if (updateManager.isBlown()) {
			if (logMINOR) Logger.minor(
				this,
				"Key blown, so not receiving main jar from " +
				source +
				"(" +
				uid +
				")"
		...
		// Okay, we can receive it
		System.out.println(
			"Receiving main jar " + version + " from " + source.userToString()
		);
		...
		try {
			...
			System.err.println(
				"Cannot save new main jar to disk and therefore cannot fetch it from our peer!: " +
				e
			);
			e.printStackTrace();
			cancelSend(source, uid);
	...
	protected void processMainJarBlob(
		...
		SimpleBlockSet blocks = new SimpleBlockSet();
		final String toString = source == null
			? "(local)"
			: source.userToString();
		...
		ClientGetCallback myCallback = new ClientGetCallback() {
			...
			public void onSuccess(FetchResult result, ClientGetter state) {
				System.err.println(
					"Got main jar version " + version + " from " + toString
				);
				if (result.size() == 0) {
				...
				if (mainUpdater == null) {
					System.err.println(
						"Not updating because updater is disabled!"
					);
					return;
				}
	...
	protected void maybeInsertMainJar(
		...
		if (source != null) {
			...
			updateManager.node.lastVersion > 0 &&
			updateManager.node.lastVersion != version
		) {
			// We just restarted after updating.
			priority = RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS;
		} else if (
			updateManager.node.fastWeakRandom.nextInt(RANDOM_INSERT_BLOB) != 0
		) {
	...
	protected boolean removeOldTempFiles() {
		...
		File[] oldTempFiles = oldTempFilesPeerDir.listFiles(
			new FileFilter() {
				...
				public boolean accept(File file) {
					...
					if (
						fileName.startsWith("revocation-") &&
					...
					if (mainBuildNumberMatcher.matches()) {
						try {
							...
							if (
								buildNumber < lastGoodMainBuildNumber
							) return true;
						} catch (NumberFormatException e) {
							Logger.error(
								this,
								"Wierd file in persistent temp: " + fileName
							);
							return false;
						}
					} else if (
						mainTempBuildNumberMatcher.matches() ||
		...
		for (File fileToDelete : oldTempFiles) {
			...
			if (!fileToDelete.delete()) {
				if (fileToDelete.exists()) Logger.error(
					this,
					...
					this,
					"Temporary persistent file does not exist when deleting: " +
					fileToDeleteName
				);
				gotError = true;
			}
		...
		try {
			...
			Logger.error(
				...
				(e instanceof FileNotFoundException
						? "don't have the file"
						: "can't read the file") +
				" even though we did have it when we checked!: " +
				e,
				e
			);
			raf = null;
	...
	private void decrementDependencies(PeerNode source) {
		synchronized (peersFetchingDependencies) {
			...
			if (x == null) {
				Logger.error(
					this,
					"Inconsistent dependency counting? Should not be null for " +
					source
				);
			} else if (x == 1) {
				...
				Logger.error(
					this,
					"Inconsistent dependency counting? Counter is " +
					x +
					" for " +
					source
				);
	...
	private boolean incrementDependencies(PeerNode source) {
		synchronized (peersFetchingDependencies) {
			...
			if (x > MAX_TRANSFERS_PER_PEER) {
				Logger.normal(
					this,
					"Too many dependency transfers for peer " +
					source +
					" - rejecting"
				);
				return false;
	 ...
	 */
	public UOMDependencyFetcher fetchDependency(
		byte[] expectedHash,
		long size,
		File saveTo,
		boolean executable,
		...
		final UOMDependencyFetcher f = new UOMDependencyFetcher(
			expectedHash,
			size,
			saveTo,
			executable,
		...
		this.updateManager.node.executor.execute(
				new Runnable() {
					@Override
					public void run() {
						f.start();
	...
	protected void startSomeDependencyFetchers() {
		...
		synchronized (this) {
			fetchers = dependencyFetchers
				.values()
				.toArray(new UOMDependencyFetcher[dependencyFetchers.size()]);
		}
	...
	class UOMDependencyFetcher {
			...
			while (true) {
				synchronized (this) {
					if (peersFetching.size() >= MAX_NODES_SENDING_JAR) {
						if (logMINOR) Logger.minor(
							this,
							"Already fetching jar from 2 peers " + peersFetching
						);
						return false;
					}
				...
				if (tryEverything) {
					Logger.minor(
						this,
						"Could not find a peer to send request to for " + saveTo
					);
					return false;
				}
				synchronized (this) {
					if (peersFailed.size() != 0) {
						System.out.println(
							"UOM trying peers which have failed downloads for " +
							saveTo.getName() +
			...
			updateManager.node.executor.execute(
				new Runnable() {
					...
					public void run() {
						...
						try {
							System.out.println(
								"Fetching " + saveTo + " from " + fetchFrom
							);
							long uid =
								updateManager.node.fastWeakRandom.nextLong();
							fetchFrom.sendAsync(
								DMT.createUOMFetchDependency(
									uid,
									expectedHash,
									size
								),
							...
							BulkReceiver br = new BulkReceiver(
								prb,
								fetchFrom,
								uid,
								updateManager.ctr
							...
							if (!failed) {
								// Check the hash.
								if (
									MainJarDependenciesChecker.validFile(
										tmp,
										expectedHash,
										size,
										executable
									...
									if (FileUtil.renameTo(tmp, saveTo)) {
										...
										failed = true;
										System.err.println(
											"Update failing: Saved dependency to " +
											tmp +
											" for " +
											saveTo +
									...
									failed = true;
									System.err.println(
										"Update failing: Downloaded file " +
										saveTo +
										" from " +
										fetchFrom +
										" but file does not match expected hash."
									);
							...
							System.err.println(
								"Fetch failed due to internal error (bug or severe local problem?): " +
								e
							);
							e.printStackTrace();
						} catch (Error e) {
		...
		private synchronized PeerNode chooseRandomPeer(
			HashSet<PeerNode> uomPeers
		) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Trying to choose peer from " + uomPeers.size()
			);
			...
			for (PeerNode pn : uomPeers) {
				...
				if (peersFailed.contains(pn)) {
					if (logMINOR) Logger.minor(
						this,
						"Peer already failed for " + saveTo + " : " + pn
					);
					continue;
				}
				if (!pn.isConnected()) {
			...
			if (notTried == null) {
				if (logMINOR) Logger.minor(
					this,
					"No peers to ask for " + saveTo
				);
				return null;
			}
			PeerNode fetchFrom = notTried.get(
				updateManager.node.fastWeakRandom.nextInt(notTried.size())
			);
