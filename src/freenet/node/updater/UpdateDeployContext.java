...
public class UpdateDeployContext {
  ...
  UpdateDeployContext(MainJarDependencies deps) throws UpdaterParserException {
    ...
    for (int propNo = 1; true; propNo++) {
      ...
      if (mainJar == null) {
        if (name.equals("freenet-ext.jar")
    ...
    if (mainJar == null) throw new UpdaterParserException(l10n("cannotUpdateNoMainJar"));
  ...
  void rewriteWrapperConf(boolean writtenNewJar)
      throws IOException, UpdateCatastropheException, UpdaterParserException {

    // Rewrite wrapper.conf
    // Don't just write it out from properties; we want to keep it as close to what it was as
    // possible.
    ...
    // Therefore, on Windows, if we update freenet-ext.jar, it will use freenet.jar and
    // freenet-ext.jar
    // and freenet-ext.jar.new as well. The old freenet-ext.jar will take precedence, and we won't
    // be
    ...
    while ((line = br.readLine()) != null) {
      ...
      if (lowcaseLine.startsWith("wrapper.java.classpath.")) {
        ...
        if (idx != -1) {
          ...
          dontWrite = true;
          if (rhs.equals("freenet.jar")
              || rhs.equals("freenet.jar.new")
              || rhs.equals("freenet-stable-latest.jar")
              || rhs.equals("freenet-stable-latest.jar.new")
              || rhs.equals("freenet-testing-latest.jar")
              || rhs.equals("freenet-testing-latest.jar.new")) {
            if (writtenNewJar) mainRHS = newMain;
            else mainRHS = rhs;
          } else {
            ...
            if (dep != null) {
              ...
              else System.out.println("Found new dependency " + dep.newFilename());
            } else { // dep == null
      ...
      if (!dontWrite) otherLines.add(line);
    }
    br.close();
    ...
    if (mainRHS == null) {
      throw new UpdaterParserException(
          l10n(
              "updateFailedNonStandardConfig",
              new String[] {"main"},
              new String[] {Boolean.toString(mainRHS != null)}));
    }
    ...
    if (!writtenPrivateModulesOpens && JVMVersion.supportsModules()) {
      ...
      bw.write(
          "wrapper.java.additional." + count + "=--add-opens=java.base/java.io=ALL-UNNAMED" + '\n');
      count++;
    }
    ...
    if (!newConfig.renameTo(oldConfig)) {
      if (!oldConfig.delete()) {
        throw new UpdaterParserException(
            l10n("updateFailedCannotDeleteOldConfig", "old", oldConfig.toString()));
      }
      if (!newConfig.renameTo(oldConfig)) {
    ...
    System.err.println(
        "Rewritten wrapper.conf for build "
            + deps.build
            + " and "
  ...
  public static CHANGED tryIncreaseMemoryLimit(int extraMemoryMB, String markerComment) {
    ...
    try {
      ...
      while ((line = br.readLine()) != null) {
        ...
        if (line.equals("#" + markerComment)) return CHANGED.ALREADY;
        ...
        if (line.startsWith("wrapper.java.maxmemory=")) {
          try {
            ...
            // There have been some cases where really high limits have caused the JVM to do bad
            // things.
            if (NodeStarter.isSomething32bits() && newMemoryLimit > 1408) {
              Logger.error(
                  UpdateDeployContext.class,
                  "We've detected a 32bit JVM so we're refusing to set maxmemory to "
                      + newMemoryLimit);
              newMemoryLimit = 1408;
            }
    ...
    if (success) {
      if (!newConfig.renameTo(oldConfig)) {
        if (!oldConfig.delete()) {
          System.err.println(
              "Unable to move rewritten wrapper.conf with new memory limit "
                  + newConfig
                  + " over old config "
                  + oldConfig
                  + " : unable to delete old config");
          return CHANGED.FAIL;
        }
        if (!newConfig.renameTo(oldConfig)) {
          System.err.println("Old wrapper.conf deleted but new wrapper.conf cannot be renamed!");
          System.err.println(
              "FREENET WILL NOT START UNTIL YOU RENAME " + newConfig + " to " + oldConfig);
