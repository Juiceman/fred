...
public class NodeUpdateManager {
	...
	public final static String UPDATE_URI =
	...
	public final static String LEGACY_UPDATE_URI =
	...
	throws InvalidConfigValueException {
		...
		LegacyJarFetcher.LegacyFetchCallback legacyFetcherCallback = new
		LegacyJarFetcher.LegacyFetchCallback() {
			...
			try {
				...
				for (int i = 0; i < 10; i++) {
					...
					if (FileUtil.renameTo(temp, directory.file(filename))) {
						...
						try {
							Thread.sleep(SECONDS.toMillis(1) + node.fastWeakRandom.nextInt((int) SECONDS.toMillis((
	...
	public synchronized void addChangelogLinks(long version, HTMLNode node) {
		...
		String developerDetailsUri = getDeveloperChangelogURI().setSuggestedEdition(
										 version).sskForUSK().toASCIIString();
	...
	private boolean innerDeployUpdate(MainJarDependencies deps) {
		System.err.println("Deploying update "+deps.build+" with "+deps.dependencies.size()
						   +" dependencies...");
