...
public class NodeUpdateManager {
  ...
  public static final String UPDATE_URI =
      "USK@vCKGjQtKuticcaZ-dwOgmkYPVLj~N1dm9mb3j3Smg4Y,-wz5IYtd7PlhI2Kx4cAwpUu13fW~XBglPyOn8wABn60,AQACAAE/jar/"
          + Version.buildNumber();

  /** The URI for post-TRANSITION_VERSION builds' freenet.jar on EoL JVMs. */
  public static final String LEGACY_UPDATE_URI =
      "SSK@ugWS2VICgMcQ5ptmEE1mAvHgUn2OSCOogJIUAvbL090,ZKO1pZRI9oaBuBQuWFL4bK3K0blvmEdqYgiIJF5GcjQ,AQACAAE/jar-"
          + TRANSITION_VERSION;

  /**
   * The URI for freenet.jar before the updater was rekeyed. Unless both the EoL and modern keys
   * rekey this is LEGACY_UPDATE_URI.
   */
  public static final String PREVIOUS_UPDATE_URI =
      "SSK@O~UmMwTeDcyDIW-NsobFBoEicdQcogw7yrLO2H-sJ5Y,JVU4L7m9mNppkd21UNOCzRHKuiTucd6Ldw8vylBOe5o,AQACAAE/jar-"
          + TRANSITION_VERSION;

  public static final String REVOCATION_URI =
      "SSK@tHlY8BK2KFB7JiO2bgeAw~e4sWU43YdJ6kmn73gjrIw,DnQzl0BYed15V8WQn~eRJxxIA-yADuI8XW7mnzEbut8,AQACAAE/revoked";
  ...
  public static final String transitionMainJarFilename =
      "legacy-freenet-jar-" + TRANSITION_VERSION + ".fblob";
  ...
  public NodeUpdateManager(Node node, Config config) throws InvalidConfigValueException {
    ...
    updaterConfig.register(
        "URI",
        JVMVersion.needsLegacyUpdater() ? legacyMainJarUSK.toString() : UPDATE_URI,
        3,
        true,
        true,
    ...
    if (!updateURI.isUSK()) throw new InvalidConfigValueException(l10n("updateURIMustBeAUSK"));
    ...
    LegacyJarFetcher.LegacyFetchCallback legacyFetcherCallback =
        new LegacyJarFetcher.LegacyFetchCallback() {
    ...
    transitionMainJarFile =
        new File(node.clientCore.getPersistentTempDir(), transitionMainJarFilename);
    transitionMainJarFetcher =
        new LegacyJarFetcher(
            previousMainJarSSK, transitionMainJarFile, node.clientCore, legacyFetcherCallback);
    ...
    updaterConfig.register(
        ...
        new BooleanCallback() {
          ...
          public void set(Boolean val)
            ...
            updateSeednodes = val;
            if (val) throw new NodeNeedRestartException("Must restart to fetch the seednodes");
            else
    ...
    updaterConfig.register(
        ...
        new BooleanCallback() {
          ...
          public void set(Boolean val)
            ...
            if (val) throw new NodeNeedRestartException("Must restart to fetch the installers");
            else
              throw new NodeNeedRestartException(
    ...
    this.revocationChecker =
        new RevocationChecker(
            this, new File(node.clientCore.getPersistentTempDir(), "revocation-key.fblob"));
      ...
      try {
        ...
        for (int i = 0; i < 10; i++) {
          ...
          if (FileUtil.renameTo(temp, directory.file(filename))) {
            System.out.println(
                "Successfully fetched " + filename + " for version " + Version.buildNumber());
            break;
          } else {
            System.out.println(
                "Failed to rename "
  ...
  void broadcastUOMAnnouncesOld() {
    boolean mainJarAvailable =
        transitionMainJarFetcher == null ? false : transitionMainJarFetcher.fetched();
    Message msg;
  ...
  public void maybeSendUOMAnnounce(PeerNode peer) {
    ...
    try {
      if (peer.getVersionNumber() < TRANSITION_VERSION) {
        if (sendOld || hasBeenBlown) peer.sendAsync(getOldUOMAnnouncement(), null, ctr);
      } else {
        if (sendNew || hasBeenBlown) peer.sendAsync(getNewUOMAnnouncement(size), null, ctr);
      }
  ...
  void enable(boolean enable) throws InvalidConfigValueException {
    ...
    synchronized (this) {
      ...
      if (!enable) {
        ...
        mainUpdater =
            new MainJarUpdater(
                this, updateURI, Version.buildNumber(), -1, Integer.MAX_VALUE, "main-jar-");
  ...
  void startPluginUpdater(OfficialPluginDescription plugin) {
    ...
    if (info != null) minVer = Math.max(minVer, info.getPluginLongVersion());
  ...
  private boolean isReadyToDeployUpdate(boolean ignoreRevocation) {
    ...
    synchronized (this) {
      ...
      if (startedFetchingNextMainJar > 0) {
        waitForNextJar =
            (int) (startedFetchingNextMainJar + WAIT_FOR_SECOND_FETCH_TO_COMPLETE - now);
        if (waitForNextJar > 0) {
      ...
      if (this.latestMainJarDependencies == null) {
        if (logMINOR) Logger.minor(this, "Dependencies not available");
        return false;
      }
    ...
    try {
      ...
      synchronized (this) {
        if (disabledThisSession) {
          String msg =
              "Not deploying update because disabled for this session (bad java version??)";
        ...
        if (hasBeenBlown) {
          String msg =
              "Trying to update but key has been blown! Not updating, message was "
                  + revocationMessage;
          Logger.error(this, msg);
          System.err.println(msg);
          return;
        ...
        if (peersSayBlown) {
          String msg =
              "Trying to update but at least one peer says the key has been blown! Not updating.";
          Logger.error(this, msg);
          System.err.println(msg);
          return;
        }
      ...
      node.clientCore.alerts.register(
          new SimpleUserAlert(false, error, error, error, UserAlert.CRITICAL_ERROR));
    } finally {
      if (started && !success) {
        Bucket toFree = null;
        synchronized (this) {
  ...
  private boolean writeJars(UpdateDeployContext ctx, MainJarDependencies deps) {
    ...
    if (hasNewMainJar) {
      ...
      try {
        if (writeJar(mainJar, newMainJar, backupJar, mainUpdater, "main", tryEasyWay))
          writtenNewJar = true;
    ...
    if (!(writtenNewJar || deps.mustRewriteWrapperConf)) return true;
    try {
  ...
  void onDownloadedNewJar(Bucket result, int fetched, File savedBlob) {
    ...
    synchronized (this) {
      if (fetched > Version.buildNumber()) {
        ...
        if (logMINOR) Logger.minor(this, "Got main jar: " + fetched);
      }
      if (!isDeployingUpdate) {
        ...
        if (fetched == Version.buildNumber()) {
          if (savedBlob != null) currentVersionBlobFile = savedBlob;
          else Logger.error(this, "No blob file for latest version?!", new Exception("error"));
        }
      } else {
    ...
    if (delete1 != null) delete1.free();
    if (delete2 != null) delete2.free();
  ...
  public void blow(String msg, boolean disabledNotBlown) {
    ...
    synchronized (this) {
      if (hasBeenBlown) {
        if (this.disabledNotBlown && !disabledNotBlown) disabledNotBlown = true;
        Logger.error(
            this,
  ...
  private void deployPluginUpdates() {
    ...
    if (updaters != null) {
      for (PluginJarUpdater u : updaters) {
        if (u.onNoRevocation()) restartRevocationFetcher = true;
      }
    }
    if (restartRevocationFetcher) revocationChecker.start(true, true);
  }
    ...
    if (om != null) {
      if (om.waitingForUpdater()) {
        synchronized (this) {
          ...
          if (gotJarTime > 0) gotJarTime = System.currentTimeMillis();
        }
        om.reannounce();
      }
    }
  ...
  void deployOffThread(long delay, final boolean announce) {
    node.ticker.queueTimedJob(
        new Runnable() {
          @Override
          public void run() {
            if (announce) maybeBroadcastUOMAnnouncesNew();
  ...
  class UpdateURICallback extends StringCallback {
    ...
    public void set(String val) throws InvalidConfigValueException {
      ...
      if (uri.hasMetaStrings())
        throw new InvalidConfigValueException(l10n("updateURIMustHaveNoMetaStrings"));
      if (!uri.isUSK()) throw new InvalidConfigValueException(l10n("updateURIMustBeAUSK"));
      setURI(uri);
    }
  }
  ...
  public void notPeerClaimsKeyBlown() {
    ...
    node.executor.execute(
        new Runnable() {
          ...
          @Override
          public void run() {
            if (isReadyToDeployUpdate(false)) deployUpdate();
          }
        },
        "Check for updates");
    node.getTicker()
  ...
  public boolean dontAllowUOM() {
    if (node.isOpennetEnabled() && node.wantAnonAuth(true)) {
      ...
      if (node.getUptime() > MINUTES.toMillis(5) && node.peers.countCompatibleRealPeers() == 0)
        return false;
      return true;
    }
    return false;
