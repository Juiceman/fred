...
public class PeerMessageQueue {
  ...
  private class PrioQueue {
    ...
    private MessageItem addNonUrgentMessages(
        long now, MutableBoolean addPeerLoadStatsRT, MutableBoolean addPeerLoadStatsBulk) {
      if (logMINOR) checkOrder();
     ...
     * @param addPeerLoadStatsBulk Will be set if the caller needs to include a load stats message
     *     for bulk (i.e. a bulk request completes etc).
  ...
  public synchronized MessageItem grabQueuedMessageItem(int minPriority) {
    ...
    for (int i = 0; i < DMT.PRIORITY_REALTIME_DATA; i++) {
      ...
      MessageItem ret =
          queuesByPriority[i].addPriorityMessages(now, addPeerLoadStatsRT, addPeerLoadStatsBulk);
      if (ret != null) return ret;
    }
