...
public class NodeIPDetector {
  ...
  FreenetInetAddress[] detectPrimaryIPAddress(boolean dumpLocalAddresses) {
    ...
    if (overrideIPAddress != null) {
      ...
      if (overrideIPAddress.isRealInternetAddress(false, true, false)) addedValidIP = true;
    }
    ...
    } else if (logMINOR) Logger.minor(this, "Client core not loaded");
    synchronized (this) {
      hasValidIP = addedValidIP;
    }
  ...
  private boolean innerDetect(List<FreenetInetAddress> addresses) {
    ...
    if (node.peers != null) {
      ...
      for (PeerNode pn : peerList) {
        ...
        if (logMINOR) Logger.minor(this, "Peer " + pn.getPeer() + " thinks we are " + addr);
  ...
  public boolean hasDirectlyDetectedIP() {
    ...
    for (InetAddress addr : addrs) {
      if (IPUtil.isValidAddress(addr, false)) {
        if (logMINOR) Logger.minor(this, "Has a directly detected IP: " + addr);
        return true;
      }
    }
    return false;
  ...
  public int registerConfigs(SubConfig nodeConfig, int sortOrder) {
    ...
    if (overrideIPAddressString.length() == 0) overrideIPAddress = null;
    else {
      try {
  ...
  void hasDetectedPM() {
    if (logMINOR) Logger.minor(this, "hasDetectedPM() called", new Exception("debug"));
    synchronized (this) {
      hasDetectedPM = true;
    }
  }
  ...
  public void setMaybeSymmetric() {
    if (ipDetectorManager != null && ipDetectorManager.isEmpty()) {
      ...
      if (maybeSymmetricAlert != null) node.clientCore.alerts.unregister(maybeSymmetricAlert);
    }
  }
