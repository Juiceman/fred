...
public class NodeClientCore implements Persistable {
	...
	NodeClientCore(
		...
		lazyStartDatastoreChecker = nodeConfig.getBoolean(
			"lazyStartDatastoreChecker"
		);

		storeChecker = new DatastoreChecker(
			node,
			lazyStartDatastoreChecker,
			node.executor,
			"Datastore checker"
		...
		persister = new ConfigurablePersister(
			this,
			nodeConfig,
			"clientThrottleFile",
			"client-throttle.dat",
		...
		if (logMINOR) Logger.minor(
			this,
			"Serializing RequestStarterGroup from:\n" + throttleFS
		);

		// Temp files

		this.tempDir = node.setupProgramDir(
			installConfig,
			"tempDir",
		...
		File oldTemp = node
			.runDir()
			.file("temp-" + node.getDarknetPortNumber());
		if (
			oldTemp.exists() &&
			oldTemp.isDirectory() &&
			!FileUtil.equals(tempDir.dir, oldTemp)
		) {
		...
		try {
			tempFilenameGenerator = new FilenameGenerator(
				random,
				true,
				getTempDir(),
				"temp-"
		...
		this.persistentTempDir = node.setupProgramDir(
			installConfig,
			"persistentTempDir",
		...
		try {
			this.persistentTempBucketFactory = new PersistentTempBucketFactory(
			...
			String msg =
				"Could not find or create persistent temporary directory: " + e;
			e.printStackTrace();
			throw new NodeInitException(NodeInitException.EXIT_BAD_DIR, msg);
		}
		...
		if (oldBlobFile.exists()) {
			...
			if (persistentTempBucketFactory.isEncrypting()) {
				try {
					...
					System.err.println(
						"Unable to delete old blob file " +
						oldBlobFile +
						" : error: " +
						e
					);
					System.err.println(
						"Please delete " + oldBlobFile + " yourself."
					);
				}
			} else {
				oldBlobFile.delete();
		...
		if (maxMemory < 0) defaultRamBucketPoolSize = 10;
			...
			defaultRamBucketPoolSize = (int) Math.min(
				Integer.MAX_VALUE,
				((maxMemory - 64) / 10)
			);
		...
		long maxBucketSize = Math.max(
			32768,
			(defaultRamBucketPoolSize * 1024 * 1024) / 20
		);
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				public void set(Long val) throws InvalidConfigValueException {
					if (
						get().equals(val) || (tempBucketFactory == null)
					) return;
		...
		tempBucketFactory = new TempBucketFactory(
			node.executor,
		...
		clientLayerPersister = new ClientLayerPersister(
			node.executor,
			node.ticker,
			node,
			this,
		...
		persistentDiskChecker = new DiskSpaceCheckingRandomAccessBufferFactory(
			raff,
			persistentTempDir.dir(),
			minDiskFreeLongTerm + tempBucketFactory.getMaxRamUsed()
		);
		persistentRAFFactory = new MaybeEncryptedRandomAccessBufferFactory(
		...
		maxMemoryLimitedJobThreads = Math.min(
		...
		nodeConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer val)
					throws InvalidConfigValueException, NodeNeedRestartException {
					if (val < 1) throw new InvalidConfigValueException(
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				public void set(Long val)
					throws InvalidConfigValueException, NodeNeedRestartException {
					if (
						val < FECCodec.MIN_MEMORY_ALLOCATION
		...
		memoryLimitedJobRunner = new MemoryLimitedJobRunner(
		...
		try {
			requestStarters = new RequestStarterGroup(
				node,
				this,
				portNumber,
				random,
			...
			throw new NodeInitException(
				NodeInitException.EXIT_BAD_CONFIG,
				e1.toString()
			);
		}
				...
				public void onChange(
					...
					if (newLevel == PHYSICAL_THREAT_LEVEL.LOW) {
						...
						if (persistentTempBucketFactory != null) {
							if (persistentTempBucketFactory.isEncrypting()) {
								persistentTempBucketFactory.setEncryption(
									false
								);
							}
						}
					...
					if (clientLayerPersister.hasLoaded()) {
						...
						try {
							initStorage(
								NodeClientCore.this.node.getDatabaseKey()
							);
						} catch (MasterKeysWrongPasswordException e) {
		...
		this.downloadsDir = node.setupProgramDir(
			nodeConfig,
			"downloadsDir",
		...
		nodeConfig.register(
			...
			new StringArrCallback() {
				...
				public String[] get() {
					synchronized (NodeClientCore.this) {
						if (downloadAllowedEverywhere) return new String[] {
							"all",
						};
						String[] dirs = new String[downloadAllowedDirs.length +
						(includeDownloadDir ? 1 : 0)];
						for (
							int i = 0;
		...
		nodeConfig.register(
			...
			new StringArrCallback() {
				...
				public String[] get() {
					synchronized (NodeClientCore.this) {
						if (uploadAllowedEverywhere) return new String[] {
							"all",
						};
						String[] dirs = new String[uploadAllowedDirs.length];
						for (
							int i = 0;
							i < uploadAllowedDirs.length;
							i++
		...
		nodeConfig.register(
			...
			new IntCallback() {
				...
				public void set(Integer uskFetch)
					throws InvalidConfigValueException {
					if (uskFetch <= 0) throw new InvalidConfigValueException(
		...
		maxBackgroundUSKFetchers = nodeConfig.getInt(
			"maxBackgroundUSKFetchers"
		);
		...
		// TMCI
		try {
			tmci = TextModeClientInterfaceServer.maybeCreate(
				node,
				this,
				config
			);
			...
			e.printStackTrace();
			throw new NodeInitException(
				NodeInitException.EXIT_COULD_NOT_START_TMCI,
		...
		try {
			fcpServer = FCPServer.maybeCreate(
				node,
				this,
				node.config,
				fcpPersistentRoot
			...
			throw new NodeInitException(
				NodeInitException.EXIT_COULD_NOT_START_FCP,
		...
		this.alerts.register(
				startingUpAlert = new SimpleUserAlert(
					true,
		...
		this.alerts.register(
				new SimpleUserAlert(
					...
					public boolean isValid() {
						...
						if (
							NodeClientCore.this.node.awaitingPassword()
						) return false;
		...
		nodeConfig.register(
			...
			new LongCallback() {
				...
				public void set(Long val)
					throws InvalidConfigValueException, NodeNeedRestartException {
					synchronized (NodeClientCore.this) {
						if (val < 0) throw new InvalidConfigValueException(
							l10n("minDiskFreeMustBePositive")
						);
	...
	boolean lateInitDatabase(DatabaseKey databaseKey) throws NodeInitException {
		System.out.println(
			"Late database initialisation: starting middle phase"
		);
		try {
			initStorage(databaseKey);
		} catch (MasterKeysWrongPasswordException e) {
			Logger.error(
				this,
				"Impossible: can't load even though have key? " +
				(databaseKey != null)
			);
			return true;
		}
	 ...
	 */
	private void initStorage(DatabaseKey databaseKey)
	...
	private static String l10n(String key, String pattern, String value) {
		return NodeL10n.getBase()
			.getString("NodeClientCore." + key, pattern, value);
	}
				...
				public void run() {
					...
					if (node.getDatabaseKey() != null) {
						try {
							...
							Logger.error(
								this,
								"Failed to migrate and/or cleanup persistent temp buckets: " +
								t,
								t
							);
							System.err.println(
	...
	public void asyncGet(
		...
		final RequestTag tag = new RequestTag(
			isSSK,
		...
		if (
			!tracker.lockUID(uid, isSSK, false, false, true, realTimeFlag, tag)
		) {
			Logger.error(
				this,
		...
		if (offersOnly) {
			...
			if (logMINOR) Logger.minor(
				this,
				"Using old HTL for GetOfferedKey: " + htl
			);
		}
		...
		asyncGet(
			...
			new RequestSenderListener() {
				...
				public void onReceivedRejectOverload() {
					...
					requestStarters.rejectedOverload(
						isSSK,
						false,
						realTimeFlag
					);
				...
				public void onRequestSenderFinished(
					...
					if (
						...
						(isSSK
								...
								: nodeStats.localChkFetchBytesSentAverage).report(
								rs.getTotalSentBytes()
							);
						(isSSK
					...
					if (
						...
						if (!rejectedOverload) {
							...
							if (isSSK) {
								node.nodeStats.reportSSKOutcome(
									rtt,
									false,
									realTimeFlag
								);
							} else {
								node.nodeStats.reportCHKOutcome(
									rtt,
									false,
									targetLocation,
									realTimeFlag
						...
						long rtt = System.currentTimeMillis() - startTime;
						double targetLocation = key.toNormalizedDouble();
						if (!rejectedOverload) requestStarters.requestCompleted(
							isSSK,
							false,
							key,
							realTimeFlag
						...
						requestStarters
							.getThrottle(isSSK, false, realTimeFlag)
							.successfulCompletion(rtt);
						if (isSSK) {
							node.nodeStats.reportSSKOutcome(
								rtt,
								status == RequestSender.SUCCESS,
					...
					if (status == RequestSender.SUCCESS) listener.onSucceeded(); // FIXME how to identify failed to decode and report it back to the client layer??? do we even need to???
					else {
						switch (status) {
							case RequestSender.NOT_FINISHED:
								Logger.error(
									this,
									"RS still running in get" +
									(isSSK ? "SSK" : "CHK") +
									"!: " +
									rs
								);
				...
				public void onNotStarted(boolean internalError) {
					if (internalError) listener.onFailed(
						new LowLevelGetException(
							LowLevelGetException.INTERNAL_ERROR
						)
					);
	...
	void asyncGet(
		...
		try {
			Object o = node.makeRequestSender(
				key,
				htl,
				uid,
				tag,
	...
	public ClientKeyBlock realGetKey(
		...
		if (key instanceof ClientCHK) return realGetCHK(
			(ClientCHK) key,
			localOnly,
			ignoreStore,
			canWriteClientCache,
		...
		else if (key instanceof ClientSSK) return realGetSSK(
			(ClientSSK) key,
			localOnly,
			ignoreStore,
			canWriteClientCache,
	...
	ClientCHKBlock realGetCHK(
		...
		if (
			!tracker.lockUID(uid, false, false, false, true, realTimeFlag, tag)
		) {
			Logger.error(
				this,
				"Could not lock UID just randomly generated: " +
		...
		try {
			...
			if (o instanceof CHKBlock) try {
				tag.setServedFromDatastore();
				return new ClientCHKBlock((CHKBlock) o, key);
			} catch (CHKVerifyException e) {
				Logger.error(this, "Does not verify: " + e, e);
				throw new LowLevelGetException(
					LowLevelGetException.DECODE_FAILED
				);
			}
			if (o == null) throw new LowLevelGetException(
				LowLevelGetException.DATA_NOT_FOUND_IN_STORE
			);
			rs = (RequestSender) o;
			...
			while (true) {
				...
				if (
					(!rejectedOverload) &&
					(waitStatus & RequestSender.WAIT_REJECTED_OVERLOAD) != 0
				) {
				...
				if (
					...
					if (
						status == RequestSender.SUCCESS
					) nodeStats.successfulChkFetchBytesReceivedAverage.report( //nodeStats.successfulChkFetchBytesSentAverage.report(rs.getTotalSentBytes()); // See comments above declaration of successful* : We don't report sent bytes here.
						rs.getTotalReceivedBytes()
					);
				}
				...
				if (
					...
					// Count towards RTT even if got a RejectedOverload - but not if timed out.
					requestStarters
						.getThrottle(false, false, realTimeFlag)
						.successfulCompletion(rtt);
					node.nodeStats.reportCHKOutcome(
						rtt,
						status == RequestSender.SUCCESS,
				...
				if (status == RequestSender.SUCCESS) try {
					...
					switch (status) {
						...
						default:
							Logger.error(
								this,
								"Unknown RequestSender code in getCHK: " +
								status +
								" on " +
								rs
							);
	...
	ClientSSKBlock realGetSSK(
		...
		if (
			!tracker.lockUID(uid, true, false, false, true, realTimeFlag, tag)
		) {
			Logger.error(
				this,
				"Could not lock UID just randomly generated: " +
		...
		try {
			...
			while (true) {
				...
				if (
					...
					nodeStats.localSskFetchBytesSentAverage.report(
						rs.getTotalSentBytes()
					);
					nodeStats.localSskFetchBytesReceivedAverage.report(
						rs.getTotalReceivedBytes()
					);
					if (
						status == RequestSender.SUCCESS
				...
				if (
					...
					// Count towards RTT even if got a RejectedOverload - but not if timed out.
					requestStarters
						.getThrottle(true, false, realTimeFlag)
						.successfulCompletion(rtt);
					node.nodeStats.reportSSKOutcome(
						rtt,
						status == RequestSender.SUCCESS,
				...
				if (rs.getStatus() == RequestSender.SUCCESS) try {
	...
	public void realPut(
		...
		if (block instanceof CHKBlock) realPutCHK(
			(CHKBlock) block,
			canWriteClientCache,
			forkOnCacheable,
			preferInsert,
		...
		else if (block instanceof SSKBlock) realPutSSK(
			(SSKBlock) block,
			canWriteClientCache,
			forkOnCacheable,
			preferInsert,
			...
			"Unknown put type " + block.getClass()
		);
	}
	...
	public void realPutCHK(
		...
		PartiallyReceivedBlock prb = new PartiallyReceivedBlock(
			Node.PACKETS_IN_BLOCK,
			Node.PACKET_SIZE,
			data
		);
		CHKInsertSender is;
		...
		if (
			!tracker.lockUID(uid, false, true, false, true, realTimeFlag, tag)
		) {
			Logger.error(
				this,
				"Could not lock UID just randomly generated: " +
		...
		try {
			...
			while (true) {
				synchronized (is) {
					if (is.getStatus() == CHKInsertSender.NOT_FINISHED) try {
			...
			// Finished?
			if (!hasReceivedRejectedOverload) if ( // Is it ours? Did we send a request?
				is.sentRequest() &&
				(is.uid == uid) &&
				...
				requestStarters
					.getThrottle(false, true, realTimeFlag)
					.successfulCompletion(len);
				requestStarters.requestCompleted(
					false,
					true,
					block.getKey(),
					realTimeFlag
			...
			if (
				status != CHKInsertSender.TIMED_OUT &&
			...
			boolean deep = node.shouldStoreDeep(
				block.getKey(),
				null,
				is == null ? new PeerNode[0] : is.getRoutedTo()
			);
			try {
			...
			if (status == CHKInsertSender.SUCCESS) {
				...
				String msg =
					"Failed inserting " + block + " : " + is.getStatusString();
				if (status == CHKInsertSender.ROUTE_NOT_FOUND) msg +=
				" - this is normal on small networks; the data will still be propagated, but it can't find the 20+ nodes needed for full success";
				if (
					is.getStatus() != CHKInsertSender.ROUTE_NOT_FOUND
				...
				switch (is.getStatus()) {
					...
					case CHKInsertSender.TIMED_OUT:
						throw new LowLevelPutException(
							LowLevelPutException.REJECTED_OVERLOAD
						);
					case CHKInsertSender.ROUTE_NOT_FOUND:
						throw new LowLevelPutException(
							LowLevelPutException.ROUTE_NOT_FOUND
						);
					...
					default:
						Logger.error(
							this,
							"Unknown CHKInsertSender code in putCHK: " +
							is.getStatus() +
							" on " +
							is
						);
	...
	public void realPutSSK(
		...
		try {
			...
			if (
				altBlock != null && !altBlock.equals(block)
			...
			while (true) {
				synchronized (is) {
					if (is.getStatus() == SSKInsertSender.NOT_FINISHED) try {
						is.wait(SECONDS.toMillis(5));
					} catch (InterruptedException e) {
						// Ignore
					}
			...
			if (!hasReceivedRejectedOverload) if ( // Is it ours? Did we send a request?
				...
				requestStarters
					.getThrottle(true, true, realTimeFlag)
					.successfulCompletion(rtt);
			}
			...
			if (is.hasCollided()) {
				...
				LowLevelPutException failed = new LowLevelPutException(
					LowLevelPutException.COLLISION
				);
				NodeSSK key = block.getKey();
				KeyBlock collided = node.fetch(
					key,
					true,
					canWriteClientCache,
					false,
				...
				if (collided == null) {
					...
					try {
						...
						Logger.error(
							this,
							"Collided but no key and still collided!"
						);
						throw new LowLevelPutException(
							LowLevelPutException.INTERNAL_ERROR,
			...
			if (status == SSKInsertSender.SUCCESS) {
				...
				switch (is.getStatus()) {
					...
					default:
						Logger.error(
							this,
							"Unknown CHKInsertSender code in putSSK: " +
							is.getStatus() +
							" on " +
							is
						);
	...
	public void queueRandomReinsert(KeyBlock block) {
		SimpleSendableInsert ssi = new SimpleSendableInsert(
			this,
			block,
		...
		if (logMINOR) Logger.minor(
			this,
			"Queueing random reinsert for " + block + " : " + ssi
		);
		ssi.schedule();
	}
	...
	public void storeConfig() {
		Logger.normal(
			this,
			"Trying to write config to disk",
			new Exception("debug")
		);
		node.config.store();
	}
	...
	public FilterCallback createFilterCallback(URI uri, FoundURICallback cb) {
		if (logMINOR) Logger.minor(
			this,
			"Creating filter callback: " + uri + ", " + cb
		);
	...
	public boolean allowDownloadTo(File filename) {
		...
		synchronized (this) {
			...
			if (
				includeDownloadDir &&
				FileUtil.isParent(getDownloadsDir(), filename)
			) return true;
	...
	public void queueOfferedKey(Key key, boolean realTime) {
		ClientRequestScheduler sched = requestStarters.getScheduler(
	...
	public void dequeueOfferedKey(Key key) {
		...
		sched = requestStarters.getScheduler(
			key instanceof NodeSSK,
			false,
			true
		);
	...
	public boolean wantKey(Key key) {
		...
		if (
			this.clientContext.getFetchScheduler(isSSK, true).wantKey(key)
		) return true;
		if (
			this.clientContext.getFetchScheduler(isSSK, false).wantKey(key)
		) return true;
		return false;
	}
	...
	public void setupMasterSecret(MasterSecret persistentSecret) {
		if (
			clientContext.getPersistentMasterSecret() == null
