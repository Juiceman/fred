...
public class NodeClientCore implements Persistable {
   ...
   * PluginRespirator#addFormChild(freenet.support.HTMLNode, String, String)}.
   *
  ...
  NodeClientCore(
    ...
    storeChecker =
        new DatastoreChecker(node, lazyStartDatastoreChecker, node.executor, "Datastore checker");
    ...
    shutdownHook.addEarlyJob(
        new NativeThread("Shutdown RealCompressor", NativeThread.HIGH_PRIORITY, true) {
          @Override
          public void realRun() {
            compressor.shutdown();
          }
    ...
    shutdownHook.addEarlyJob(
        new NativeThread("Shutdown database", NativeThread.HIGH_PRIORITY, true) {
    ...
    shutdownHook.addLateJob(
        new NativeThread("Close database", NativeThread.HIGH_PRIORITY, true) {
    ...
    maxMemoryLimitedJobThreads =
        Math.min(maxMemoryLimitedJobThreads, node.nodeStats.getThreadLimit() / 20);
