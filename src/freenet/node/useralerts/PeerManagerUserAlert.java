...
public class PeerManagerUserAlert extends AbstractUserAlert {
	...
	public static final long MAX_OLDEST_NEVER_CONNECTED_PEER_AGE_ALERT_THRESHOLD = DAYS.toMillis(
				14); // 2 weeks
	...
	public String getTitle() {
		synchronized(this) {
			...
			if(disconnDarknetPeers > MAX_DISCONN_PEER_ALERT_THRESHOLD && !darknetDefinitelyPortForwarded
					&& !darknetAssumeNAT) {
	...
	public String getText() {
		...
		synchronized(this) {
			...
			if(peers == 0 && !isOpennetEnabled) {
				...
				s = l10n("connError", "count", Integer.toString(connError));
			} else if(disconnDarknetPeers > MAX_DISCONN_PEER_ALERT_THRESHOLD && !darknetDefinitelyPortForwarded
					  && !darknetAssumeNAT) {
	...
	static public String replaceCareful(String text, String find, String replace) {
		...
		StringBuilder sb = new StringBuilder(text.length() + (split.length-1)*
	...
	public HTMLNode getHTMLText() {
		...
		synchronized(this) {
				...
				// This is useful to not lose those who do however.
			{
				...
				NodeL10n.getBase().addL10nSubstitution(alertNode,
	...
	public boolean isValid() {
		...
		synchronized(this) {
			ret = ((peers == 0 && !isOpennetEnabled) ||
				   ...
				   (disconnDarknetPeers > MAX_DISCONN_PEER_ALERT_THRESHOLD && !darknetDefinitelyPortForwarded
					&& !darknetAssumeNAT) ||
