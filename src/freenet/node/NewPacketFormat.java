...
public class NewPacketFormat implements PacketFormat {
  ...
  private NPFPacket tryDecipherPacket(byte[] buf, int offset, int length, SessionKey sessionKey) {
    ...
    int oldHighestReceived =
        (int)
            ((0l + keyContext.watchListOffset + (keyContext.seqNumWatchList.length / 2))
                % NUM_SEQNUMS);
    if (seqNumGreaterThan(highestReceivedSeqNum, oldHighestReceived, 31)) {
      ...
      int seqNum =
          (int)
              ((0l + keyContext.watchListOffset + keyContext.seqNumWatchList.length) % NUM_SEQNUMS);
  ...
  NPFPacket createPacket(
            ...
            while (it.hasNext() && packet.getLength() < maxPacketSize) {
              ...
              while (packet.getLength() < maxPacketSize) {
                MessageFragment frag =
                    wrapper.getMessageFragment(maxPacketSize - packet.getLength());
                if (frag == null) break;
                mustSend = true;
                addedFragments = true;
    ...
    if (!ackOnly) {
      ...
      if (sendStatsBulk || sendStatsRT) {
        ...
        checkedCanSend = true;
        if (cantSend) {
          if (sendStatsBulk) pn.setSendLoadStatsASAP(false);
          if (sendStatsRT) pn.setSendLoadStatsASAP(true);
        } else {
          mustSend = true;
        }
      }
        ...
        while (true) {
          ...
          while ((packet.getLength() + 10)
            ...
            if (messageID == -1) {
              ...
              // different threads.
              // This doesn't happen at the moment because we use a single PacketSender for all
              // ports and all peers.
              // We might in future split it across multiple threads but it'd be best to keep the
              // same peer on the same thread.
              Logger.error(
                  this,
