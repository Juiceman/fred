...
public class FNPPacketMangler implements OutgoingPacketMangler {
  ...
  public DECODED process(byte[] buf, int offset, int length, Peer peer, PeerNode opn, long now) {
    ...
    if (length > Node.SYMMETRIC_KEY_LENGTH /* iv */ + HASH_LENGTH + 2) {
      for (PeerNode pn : peers) {
        if (pn == opn) continue;
        if (logDEBUG) Logger.debug(this, "Trying auth with " + pn);
    ...
    if (wantAnonAuth && !wantAnonAuthChangeIP) {
      if (checkAnonAuthChangeIP(opn, buf, offset, length, peer, now)) {
        ...
        // Logger.error(this, "Last resort match anon-auth against all anon setup peernodes
    ...
    if (logMINOR && crypto.isOpennet && wantAnonAuth) {
      if (!didntTryOldOpennetPeers) Logger.minor(this, "Unmatchable packet from " + peer);
    } else Logger.normal(this, "Unmatchable packet from " + peer);
  ...
  private boolean tryProcessAuthAnon(byte[] buf, int offset, int length, Peer peer) {
    ...
    if (length < digestLength + ivLength + 5) {
      if (logMINOR)
        Logger.minor(
            this, "Too short: " + length + " should be at least " + (digestLength + ivLength + 5));
      return false;
    }
    // IV at the beginning
    PCFBMode pcfb = PCFBMode.create(authKey, buf, offset);
  ...
  private void processDecryptedAuthAnon(final byte[] payload, final Peer replyTo) {
    if (logMINOR)
      Logger.minor(
          this, "Processing decrypted auth packet from " + replyTo + " length " + payload.length);
    ...
    if (!(negType == 10)) {
      if (negType > 10) Logger.error(this, "Unknown neg type: " + negType);
      else
        Logger.warning(
            this, "Received a setup packet with unsupported obsolete neg type: " + negType);
      return;
    }
  ...
  private final SerialExecutor authHandlingThread =
      new SerialExecutor(NativeThread.HIGH_PRIORITY, 1000);
      ...
      if (!MessageDigest.isEqual(expectedIdentityHash, crypto.identityHash)) {
        Logger.error(
            ...
            "Invalid unknown-initiator JFK(1), IDr' is "
                ...
                + " should be "
                + HexUtil.bytesToHex(crypto.identityHash));
        return;
      }
    }
  ...
  private void handleNoContextsException(
      NoContextsException e, freenet.node.FNPPacketMangler.NoContextsException.CONTEXT context) {
  ...
  private void logLoudErrorNoContexts() {
    ...
    System.err.println(
        "FREENET IS HAVING PROBLEMS CONNECTING: Either your CPU is overloaded or it is having trouble reading from the random number generator");
    System.err.println(
    ...
    if (FileUtil.detectedOS.isUnix) {
      ...
      System.err.println(
          "The best solution is to install a hardware random number generator, or use turbid or similar software to take random data from an unconnected sound card.");
      System.err.println(
          "The quick workaround is to add \"wrapper.java.additional.4=-Djava.security.egd=file:///dev/urandom\" to your wrapper.conf.");
    }
  }
  ...
  private boolean throttleRekey(PeerNode pn, Peer replyTo) {
    ...
    synchronized (throttleRekeysByIP) {
      ...
      if (l != null && now - l < PeerNode.THROTTLE_REKEY) {
        Logger.error(
            this, "Two JFK(1)'s initiated by same IP within " + PeerNode.THROTTLE_REKEY + "ms");
        return true;
      }
    }
    return false;
  ...
  private void processJFKMessage2(
    ...
    if (message3 != null) {
      Logger.normal(
          this, "We replayed a message from the cache (shouldn't happen often) - " + pn.getPeer());
  ...
  private void processJFKMessage3(
    ...
    if (!HMAC.verifyWithSHA256(
      ...
      if (shouldLogErrorInHandshake(t1)) {
        if (logDEBUG)
          Logger.debug(
              this, "We received the following HMAC : " + HexUtil.bytesToHex(authenticator));
        if (logDEBUG)
    ...
    if (message4 != null) {
      ...
      if (unknownInitiator) {
        sendAnonAuthPacket(
            1, negType, 3, setupType, (byte[]) message4, null, replyTo, crypto.anonSetupCipher);
      } else {
    ...
    if (ctx == null) {
      Logger.error(
          this,
          "WTF? the HMAC verified but we don't know about that exponential! SHOULDN'T HAPPEN! - JFK3 - "
              + pn);
    ...
    theirInitialMsgID =
        unknownInitiator
            ? getInitialMessageID(crypto.myIdentity)
    ...
    if (logMINOR)
      Logger.minor(
          this, "Their initial message ID: " + theirInitialMsgID + " ours " + ourInitialMsgID);
    ...
    if (oldOpennetPeer && pn instanceof OpennetPeerNode /* true */) {
      ...
      if (opennet == null) {
        Logger.normal(
            this, "Dumping incoming old-opennet peer as opennet just turned off: " + pn + ".");
        return;
      }
    ...
    if (newTrackerID > 0) {
      ...
      if (dontWant) {
        node.peers.disconnectAndRemove(
            pn, true, true, true); // Let it connect then tell it to remove it.
      } else {
  ...
  private boolean processJFKMessage4(
    ...
    if (jfkBuffer == null) {
      Logger.normal(
          this, "We have already handled this message... might be a replay or a bug - " + pn);
      return false;
    }
    ...
    if (!HMAC.verifyWithSHA256(pn.jfkKa, decypheredPayload, hmac)) {
      Logger.normal(
          this, "The digest-HMAC doesn't match; let's discard the packet - " + pn.getPeer());
      return false;
    }
            ...
            new Runnable() {
              ...
              public void run() {
                if (pn.timeLastConnectionCompleted() < timeSent) {
                  if (logMINOR)
                    Logger.minor(
                        this, "Resending JFK(3) to " + pn + " for " + node.getDarknetPortNumber());
                  if (unknownInitiator) {
                    sendAnonAuthPacket(
                        1,
                        negType,
  ...
  private void sendJFKMessage4(
    ...
    if (logMINOR) Logger.minor(this, "Sending a JFK(4) message to " + pn.getPeer());
    long t1 = System.currentTimeMillis();
  ...
  private void sendAuthPacket(
    ...
    Util.randomBytes(
        node.fastWeakRandom, data, hash.length + iv.length + 2 + output.length, paddingLength);
    try {
  ...
  private boolean shouldLogErrorInHandshake(long now) {
    if (now - node.startupTime < Node.HANDSHAKE_TIMEOUT * 2) return false;
    return true;
  }
  ...
  public void sendHandshake(PeerNode pn, boolean notRegistered) {
    ...
    if (logMINOR) Logger.minor(this, "Sending handshake to " + peer + " for " + pn);
  ...
  public Status getConnectivityStatus() {
    ...
    Status value;
    if (crypto.config.alwaysHandshakeAggressively()) value = AddressTracker.Status.DEFINITELY_NATED;
