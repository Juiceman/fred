...
public class MessageWrapper {
		...
		synchronized(acks) {
			...
			if(acks.contains(0, item.buf.length - 1)) {
				if(!alreadyAcked) {
					...
					alreadyAcked = true;
					if(logMINOR) {
						Logger.minor(this, "Total round trip time for message "+messageID+" : "+item+" : "+
									 (System.currentTimeMillis() - created)+" in "+resends+" resends"+(pn == null ? "" : " for "
											 +pn.shortToString()));
					}
				}
				return true;
	...
	public MessageFragment getMessageFragment(int maxLength) {
		...
		synchronized(sent) {
			...
			if(logDEBUG) {
				Logger.debug(this, "Using range "+start+" to "+(start+dataLength-1)+" gives "+sent+" on "
							 +messageID);
			}
		}
