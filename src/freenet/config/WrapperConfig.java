    ...
    if (!newConfig.renameTo(oldConfig)) {
      ...
      if (!newConfig.renameTo(oldConfig)) {
        ...
        if (!oldOldConfig.renameTo(oldConfig)) {
          System.err.println(
              "CATASTROPHIC UPDATE ERROR: Unable to rename backup copy of config file over the current config file, after failing to update config file! The node will not boot until you get a new wrapper.conf!\n"
