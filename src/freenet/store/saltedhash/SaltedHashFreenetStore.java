...
public class SaltedHashFreenetStore<T extends StorableBlock> implements FreenetStore<T> {
	...
	public static <T extends StorableBlock> SaltedHashFreenetStore<T> construct(File baseDir,
			String name, StoreCallback<T> callback, Random random,
			long maxKeys, boolean useSlotFilter, SemiOrderedShutdownHook shutdownHook, boolean preallocate,
			boolean resizeOnStart, Ticker exec, byte[] masterKey)
	throws IOException {
	...
	private SaltedHashFreenetStore(File baseDir, String name, StoreCallback<T> callback, Random random,
								   long maxKeys,
								   boolean enableSlotFilters, SemiOrderedShutdownHook shutdownHook, boolean preallocate,
								   boolean resizeOnStart, byte[] masterKey) throws IOException {
		...
		if(!slotFilterDisabled) {
			...
			System.err.println("Slot filter (" + slotFilterFile + ") for " + name + " is loaded (new="
							   +slotFilter.isNew()+").");
			...
			if(slotFilterFile.exists()) {
				if(slotFilterFile.delete()) {
					...
					System.err.println("Old slot filter file "+slotFilterFile
		...
		shutdownHook.addEarlyJob(new NativeThread(new ShutdownDB(), "Shutdown salted hash store",
	...
	public boolean start(Ticker ticker, boolean longStart) throws IOException {
		...
		storeFileOffsetReady = Math.min(curStoreFileSize / (headerBlockLength + dataBlockLength +
										hdPadding), curMetaFileSize / Entry.METADATA_LENGTH);
	...
	public T fetch(byte[] routingKey, byte[] fullKey, boolean dontPromote, boolean canReadClientCache,
		...
		try {
			...
			try {
				...
				try {
					T block = entry.getStorableBlock(routingKey, fullKey, canReadClientCache, canReadSlashdotCache,
													 meta, null);
					if (block == null) {
						misses.incrementAndGet();
						return null;
	 ...
	 * @throws IOException
	 */
	private Entry probeEntry(byte[] digestedKey, byte[] routingKey,
	...
	private Entry probeEntry0(byte[] digestedKey, byte[] routingKey, long probeStoreSize,
							  boolean withData) throws IOException {
		Entry entry = null;
	...
	public boolean put(T block, byte[] data, byte[] header, boolean overwrite, boolean isOldBlock,
					   boolean wrongStore) throws IOException, KeyCollisionException {
			...
			try {
				...
				if (oldEntry != null && !oldEntry.isFree()) {
					...
					try {
						...
						T oldBlock = oldEntry.getStorableBlock(routingKey, fullKey, false, false, null,
															   (block instanceof SSKBlock) ? ((SSKBlock)block).getPubKey() : null);
				...
				if((!wrongStore) && altStore != null) {
					if(altStore.put(block, data, header, overwrite, isOldBlock, true)) {
						if(logMINOR) {
							Logger.minor(this, "Successfully wrote block to wrong store "+altStore+" on "+this);
						}
						return true;
					} else {
						if(logMINOR) {
				...
				// Do the overwriting.
				if (logDEBUG) {
					Logger.debug(this, "collision, write to i=" + indexToOverwrite + ", offset=" +
	...
	class Entry {
		...
		private T getStorableBlock(byte[] routingKey, byte[] fullKey, boolean canReadClientCache,
								   boolean canReadSlashdotCache, BlockMetadata meta, DSAPublicKey knownKey) throws KeyVerifyException {
			...
			T block = callback.construct(data, header, routingKey, fullKey, canReadClientCache,
										 canReadSlashdotCache, meta, knownKey);
	...
	public boolean slotCacheLikelyMatch(int value, byte[] digestedRoutingKey) {
		if((value & (SLOT_CHECKED)) == 0) {
			return false;
		}
		if((value & (SLOT_OCCUPIED)) == 0) {
			return false;
		}
	...
	private boolean openStoreFiles(File baseDir, String name) throws IOException {
		...
		try {
			...
			throw new Error("Could not aquire lock for file " + baseDir.toPath().resolve(name + ".metadata"),
							ex);
		}
	 ...
	 */
	private Entry readEntry(long offset, byte[] digestedRoutingKey, byte[] routingKey,
							boolean withData) throws IOException {
		...
		if(digestedRoutingKey != null && !slotFilterDisabled) {
			...
			if(USE_SLOT_FILTER && validCache && !likelyMatch) {
				return null;
			}
		}
	...
	private boolean loadConfigFile(byte[] masterKey) throws IOException {
		...
		if (!configFile.exists()) {
			...
			if(masterKey != null) {
				...
				if(logDEBUG) {
					Logger.debug(this, "Encrypting with "+HexUtil.bytesToHex(newsalt)+" from "+HexUtil.bytesToHex(
									 diskSalt));
				}
			}
			...
			try {
				...
				try {
					...
					if(masterKey != null) {
						...
						if(logDEBUG) {
							Logger.debug(this, "Encrypting (new) with "+HexUtil.bytesToHex(salt)+" from "+HexUtil.bytesToHex(
											 diskSalt));
						}
					}
	...
	private class Cleaner extends NativeThread {
		...
		private void resizeStore(final long _prevStoreSize, final boolean sleep) {
			...
			BatchProcessor<T> resizeProcesser = new BatchProcessor<T>() {
				...
				public void init() {
					...
					WrapperManager.signalStarting((int) (RESIZE_MEMORY_ENTRIES * SECONDS.toMillis(
							30) + SECONDS.toMillis(1)));
				}
		...
		private void rebuildBloom(boolean sleep) {
			...
			BatchProcessor<T> rebuildBloomProcessor = new BatchProcessor<T>() {
				...
				public void init() {
					...
					WrapperManager.signalStarting((int) (RESIZE_MEMORY_ENTRIES * SECONDS.toMillis(5) + SECONDS.toMillis(
							1)));
				}
		...
		private void batchProcessEntries(BatchProcessor<T> processor, long storeSize, boolean reverse,
										 boolean sleep) {
	...
	public void setMaxKeys(long newStoreSize, boolean shrinkNow) throws IOException {
		Logger.normal(this, "[" + name + "] Resize newStoreSize=" + newStoreSize + ", shinkNow=" +
					  shrinkNow);
		...
		if(shrinkNow) {
			...
			try {
				...
				System.err.println("Completed shrink, old size was "+old+" new size was "+newStoreSize
								   +" size is now "+storeSize+" (prev="+prevStoreSize+")");
			} finally {
				configLock.writeLock().unlock();
			}
		}
	...
	private long[] getOffsetFromDigestedKey(byte[] digestedKey, long storeSize) {
		...
		for (int i = 0; i < OPTION_MAX_PROBE; i++) {
			...
			while(true) {
				...
				if(clear || OPTION_MAX_PROBE > storeSize) {
					break;
				}
			}
		}
