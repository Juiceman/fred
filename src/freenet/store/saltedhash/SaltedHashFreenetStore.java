...
public class SaltedHashFreenetStore<T extends StorableBlock>
	...
	public void setAltStore(SaltedHashFreenetStore<T> store) {
		if (store.altStore != null) throw new IllegalArgumentException(
			"Target must not have an altStore - deadlock can result"
		);
		altStore = store;
	}
	...
	private SaltedHashFreenetStore(
		...
		hdPadding = ((headerBlockLength + dataBlockLength + 512 - 1) &
			~(512 - 1)) -
		...
		if (bloomFile.exists()) {
			bloomFile.delete();
			System.err.println(
				"Deleted old bloom filter for " +
				name +
				" - obsoleted by slot filter"
			);
			System.err.println(
				"We will need to rebuild the slot filters, it will take a while and there will be a lot of disk access, but once it's done there should be a lot less disk access."
			);
		}
		...
		if (!slotFilterDisabled) {
			...
			if (slotFilterFile.exists()) {
				if (slotFilterFile.delete()) {
					System.err.println(
						"Old slot filter file deleted as slot filters are disabled, keeping it might cause data loss when they are turned back on."
					);
				} else {
					System.err.println(
						"Old slot filter file " +
						slotFilterFile +
						" could not be deleted. If you turn on slot filters later you might lose data from your datastore. Please delete it manually."
					);
				}
			}
			slotFilter = null;
		...
		if ((flags & FLAG_DIRTY) != 0) System.err.println(
			"Datastore(" + name + ") is dirty."
		);
		...
		if (
			resizeOnStart && prevStoreSize != 0 && cleanerGlobalLock.tryLock()
		) {
		...
		} else if ((flags & FLAG_REBUILD_BLOOM) != 0) System.out.println(
			"Slot filter still needs rebuilding"
		);
	}
	...
	public boolean start(Ticker ticker, boolean longStart) throws IOException {
		...
		if (prevStoreSize < storeSize && prevStoreSize > 0) smallerSize =
			prevStoreSize;

		if (
			(smallerSize * (headerBlockLength + dataBlockLength + hdPadding) >
				curStoreFileSize) ||
	...
	public T fetch(
		...
		if (logMINOR) Logger.minor(
			this,
			"Fetch " + HexUtil.bytesToHex(routingKey) + " for " + callback
		);
		...
		try {
			...
			try {
				...
				try {
					T block = entry.getStorableBlock(
						routingKey,
						fullKey,
						canReadClientCache,
	...
	private Entry probeEntry(
		...
		if (entry == null && prevStoreSize != 0) entry = probeEntry0(
			digestedKey,
			routingKey,
			prevStoreSize,
			withData
	...
	private Entry probeEntry0(
		...
		for (int i = 0; i < offset.length; i++) {
			if (logDEBUG) Logger.debug(
				this,
				"probing for i=" + i + ", offset=" + offset[i]
			);

			try {
				if (
					storeFileOffsetReady == -1 ||
					offset[i] < this.storeFileOffsetReady
				) {
					entry = readEntry(
						offset[i],
						digestedKey,
	...
	public boolean put(
		...
		if (logMINOR) Logger.minor(
			this,
			"Putting " + HexUtil.bytesToHex(routingKey) + " (" + name + ")"
		);
			...
			try {
				...
				if (oldEntry != null && !oldEntry.isFree()) {
					...
					try {
						if (!collisionPossible) {
							if (
								(oldEntry.flag & Entry.ENTRY_NEW_BLOCK) == 0 &&
								!isOldBlock
							) {
								oldEntry = readEntry(
									oldEntry.curOffset,
								...
								if (logMINOR) Logger.minor(
									this,
									"Setting old block to new block"
								);
						...
						T oldBlock = oldEntry.getStorableBlock(
							routingKey,
							fullKey,
							false,
							false,
							null,
							(block instanceof SSKBlock)
								? ((SSKBlock) block).getPubKey()
								: null
						);
				...
				for (int i = 0; i < offset.length; i++) {
					if (offset[i] < storeFileOffsetReady) {
						...
						if ((flag & Entry.ENTRY_FLAG_OCCUPIED) == 0) {
							...
							((flag & Entry.ENTRY_WRONG_STORE) ==
								Entry.ENTRY_WRONG_STORE)
						) {
				...
				if ((!wrongStore) && altStore != null) {
					if (
						...
						if (logMINOR) Logger.minor(
							this,
							"Successfully wrote block to wrong store " +
							altStore +
							" on " +
							this
						);
	...
	class Entry {
		...
		private long[] getOffset() {
			if (digestedRoutingKey != null) return getOffsetFromDigestedKey(
				digestedRoutingKey,
				storeSize
			);
		...
		byte[] getDigestedRoutingKey() {
			if (digestedRoutingKey == null) if (
				plainRoutingKey == null
			) return null;
			else digestedRoutingKey = cipherManager.getDigestedKey(
				plainRoutingKey
			);
	 ...
	 */
	private boolean openStoreFiles(File baseDir, String name)
		throws IOException {
		...
		try {
			...
			throw new Error(
				"Could not aquire lock for file " +
				baseDir.toPath().resolve(name + ".metadata"),
				ex
			);
		}
		...
		try {
			...
			throw new Error(
				"Could not aquire lock for file " +
				baseDir.toPath().resolve(name + ".hd"),
				ex
			);
		}
	...
	private Entry readEntry(
		...
		do {
			int status = metaFC.read(
				mbf,
				Entry.METADATA_LENGTH * offset + mbf.position()
			);
			if (status == -1) {
				Logger.error(
					this,
					"Failed to access offset " + offset,
					new Exception("error")
				);
		...
		if (trueCache != cache && !slotFilterDisabled) {
			if (validCache) Logger.error(
				this,
				"Slot cache has changed for slot " +
				offset +
				" from " +
				cache +
				" to " +
		...
		if (routingKey != null) {
			if (entry.isFree()) {
				if (validCache && !likelyMatch && !slotCacheIsFree(cache)) {
					Logger.error(
						this,
						"Slot falsely identified as non-free on slot " +
						offset +
						" cache was " +
						cache
					);
			...
			if (!Arrays.equals(digestedRoutingKey, slotDigestedRoutingKey)) {
				if (validCache && likelyMatch) {
					Logger.normal(
						this,
						"False positive from slot cache on slot " +
						offset +
						" cache was " +
						cache
					);
					bloomFalsePos.incrementAndGet();
				} else if (logMINOR && validCache && !likelyMatch) Logger.minor(
					this,
					"True negative!"
				);
				return null;
			...
			if (validCache && !likelyMatch) {
				Logger.error(
					this,
					"False NEGATIVE from slot cache on slot " +
					offset +
					" cache was " +
					cache
				);
			...
			if (withData) {
				...
				if (!decrypted) {
					if (logMINOR && validCache && likelyMatch) Logger.minor(
						this,
						"True positive but decrypt failed on slot " +
						offset +
						" cache was " +
						cache
					);
	 ...
	 * @throws IOException
	 */
	private long getFlag(long offset, boolean forceReadEntry)
		throws IOException {
	...
	private void writeEntry(
		...
		if (!slotFilterDisabled) slotFilter.put(
			(int) offset,
			entry.getSlotFilterEntry(digestedRoutingKey, entry.flag)
		);
		...
		do {
			int status = metaFC.write(
				bf,
				Entry.METADATA_LENGTH * offset + bf.position()
			);
			if (status == -1) throw new EOFException();
		...
		if (bf != null) {
			long pos =
				(headerBlockLength + dataBlockLength + hdPadding) * offset;
			do {
			...
			if (preallocate) {
				try (
					...
					if (currentHdLen < newHdLen) {
						Fallocate.forChannel(hdFC, hdRAF.getFD(), newHdLen)
							.fromOffset(currentHdLen)
							.execute();
					}
				}
			}
		...
		if (!configFile.exists()) {
			...
			if (masterKey != null) {
				...
				try {
					...
					throw new Error(
						"Impossible: no Rijndael(256,128): " + e,
						e
					);
				}
			...
			try {
				...
				try {
					...
					if (storeSize <= 0) throw new IOException(
						"Bogus datastore size"
					);
					...
					if (
						...
						// FIXME figure out a way to do this consistently!
							...
							ResizablePersistentIntBuffer.getPersistenceTime() !=
							-1
					) flags |= FLAG_REBUILD_BLOOM;
				...
				Logger.error(
					this,
					"config file corrupted, trying to create a new store: " +
					name,
					e
				);
				System.err.println(
				...
				Logger.error(
					this,
					"can't delete config file, please delete the store manually: " +
					name,
					e
				);
				System.err.println(
	...
	private interface BatchProcessor<T extends StorableBlock> {
		...
		SaltedHashFreenetStore<T>.Entry process(
			SaltedHashFreenetStore<T>.Entry entry
		);
	...
	private class Cleaner extends NativeThread {
		...
		public void realRun() {
			if (!NO_CLEANER_SLEEP) {
				try {
					Thread.sleep(
						(int) (CLEANER_PERIOD / 2 +
							CLEANER_PERIOD * random.nextDouble())
					);
				} catch (InterruptedException e) {}
			}
			...
			BatchProcessor<T> resizeProcesser = new BatchProcessor<T>() {
				...
				public void init() {
					...
					WrapperManager.signalStarting(
						(int) (RESIZE_MEMORY_ENTRIES * SECONDS.toMillis(30) +
							SECONDS.toMillis(1))
					);
				}
				...
				public Entry process(Entry entry) {
					...
					try {
						...
						if (
							oldEntryList.size() > RESIZE_MEMORY_ENTRIES
						) oldEntryList.poll();
					} catch (IOException e) {
						Logger.error(
							this,
							"error reading entry (offset=" +
							entry.curOffset +
							")",
							e
						);
				...
				public boolean batch(long entriesLeft) {
					...
					if (storeSize < _prevStoreSize) setStoreFileSize(
						Math.max(storeSize, entriesLeft)
					);

					// try to resolve the list
					Iterator<Entry> it = oldEntryList.iterator();
					while (it.hasNext()) if (
						resolveOldEntry(it.next())
					) it.remove();
		...
		private void rebuildBloom(boolean sleep) {
			...
			BatchProcessor<T> rebuildBloomProcessor = new BatchProcessor<T>() {
				...
				public void init() {
					...
					WrapperManager.signalStarting(
						(int) (RESIZE_MEMORY_ENTRIES * SECONDS.toMillis(5) +
							SECONDS.toMillis(1))
					);
				}
				...
				public Entry process(Entry entry) {
					if (!slotFilterDisabled) {
						...
						try {
							...
							Logger.error(
								this,
								"Unable to update slot filter in bloom rebuild: " +
								e,
								e
							);
						}
				...
				public void finish() {
					...
					System.out.println(
						name + " cleaner finished successfully."
					);
					Logger.normal(
						this,
						"Finish rebuilding bloom filter (" + name + ")"
					);
				}
		...
		private void batchProcessEntries(
			...
			if (!reverse) {
				...
				startOffset = ((storeSize - 1) / RESIZE_MEMORY_ENTRIES) *
			...
			try {
				for (
					...
					entriesLeft = reverse
						? curOffset
						: Math.max(
							storeSize - curOffset - RESIZE_MEMORY_ENTRIES,
							0
						);
		 ...
		 */
		private boolean batchProcessEntries(
			long offset,
			int length,
			BatchProcessor<T> processor
		) {
			...
			try {
				...
				try {
					for (
						int j = 0;
						!shutdown && buf.limit() > j * Entry.METADATA_LENGTH;
						j++
					) {
						...
						if (newEntry == null) { // free the offset
							...
							if (!slotFilterDisabled) try {
								...
								Logger.error(
									this,
									"Unable to update slot filter: " + e,
									e
								);
							}

							dirty = true;
						} else if (newEntry == NOT_MODIFIED) {} else {
							// write back
							buf.position(j * Entry.METADATA_LENGTH);
							...
							if (!slotFilterDisabled) {
								...
								if (
									slotFilter.get((int) (offset + j)) != newVal
								) {
									try {
										slotFilter.put(
											(int) (offset + j),
		...
		private boolean resolveOldEntry(Entry entry) {
			Map<Long, Condition> lockMap = lockDigestedKey(
				...
				for (long offset : offsets) {
					try {
						if (
							!isFree(offset) &&
							Arrays.equals(
								getDigestedKeyFromOffset(offset),
								entry.getDigestedRoutingKey()
							)
						) {
							// do nothing
						...
						Logger.debug(
							this,
							"IOExcception on resolveOldEntry",
							e
						);
					}
				}
	...
	private final class CleanerStatusUserAlert extends AbstractUserAlert {
		...
		public String getShortText() {
			if (cleaner.isResizing) return NodeL10n.getBase()
				.getString(
					"SaltedHashFreenetStore.shortResizeProgress", //
				...
				.getString(
					"SaltedHashFreenetStore.shortRebuildProgress" +
					(slotFilter.isNew() ? "New" : ""),
					new String[] { "name", "processed", "total" }, //
					new String[] {
						name,
						String.valueOf(
		...
		public String getText() {
			if (cleaner.isResizing) return NodeL10n.getBase()
				.getString(
					"SaltedHashFreenetStore.longResizeProgress", //
					new String[] { "name", "processed", "total" }, //
					new String[] {
						name,
						String.valueOf(
				...
				.getString(
					"SaltedHashFreenetStore.longRebuildProgress" +
					(slotFilter.isNew() ? "New" : ""),
					new String[] { "name", "processed", "total" },
					new String[] {
						name,
	...
	public void setUserAlertManager(UserAlertManager userAlertManager) {
		if (cleanerStatusUserAlert != null) userAlertManager.register(
			cleanerStatusUserAlert
		);
	}
	...
	public void setMaxKeys(long newStoreSize, boolean shrinkNow)
		...
		if (
			newStoreSize > Integer.MAX_VALUE
		...
		try {
			...
			if (!slotFilterDisabled) slotFilter.resize(
				(int) Math.max(storeSize, prevStoreSize)
			);
			writeConfigFile();
		} finally {
			configLock.writeLock().unlock();
	...
	private Condition resizeCompleteCondition = configLock
		.writeLock()
		.newCondition();
	 ...
	 */
	private Map<Long, Condition> lockDigestedKey(
		byte[] digestedKey,
		...
		if (locked.size() == offsets.size()) {
			...
			for (Map.Entry<
				Long,
				Condition
			> e : locked.entrySet()) lockManager.unlockEntry(
				e.getKey(),
				e.getValue()
			);
			return null;
	...
	private long[] getOffsetFromPlainKey(byte[] plainKey, long storeSize) {
		return getOffsetFromDigestedKey(
			cipherManager.getDigestedKey(plainKey),
			storeSize
		);
	}
	 ...
	 * @return
	 */
	private long[] getOffsetFromDigestedKey(
		byte[] digestedKey,
		long storeSize
	) {
	...
	public boolean probablyInStore(byte[] routingKey) {
		...
		try {
			...
			if (prevStoreSize != 0) offsets = getOffsetFromDigestedKey(
				digestedKey,
				prevStoreSize
			);
	...
	void testingWaitForCleanerDone(int delay, int count)
		throws InterruptedException {
