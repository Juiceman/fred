...
final class Rijndael_Algorithm // implicit no-argument constructor
 {
  ...
  private static void generateSBoxes() {
    ...
    for (int i = 0; i < 256; i++) {
      S[i] = (byte) (cox[i][0] << 7);
      for (int t = 1; t < 8; t++) S[i] ^= cox[i][t] << (7 - t);
      Si[S[i] & 0xFF] = (byte) i;
    }
  }
  ...
  private static byte[][] generateInvertedGMatrix(byte[][] gMatrix) {
    ...
    for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++) iG[i][j] = AA[i][j + 4];
  ...
  private static void blockEncrypt(byte[] in, byte[] result, int inOffset, Object sessionKey) {
    ...
    for (int r = 1; r < ROUNDS; r++) { // apply round transforms
      Ker = Ke[r];
      a0 =
          (T1[(t0 >>> 24) & 0xFF] ^ T2[(t1 >>> 16) & 0xFF] ^ T3[(t2 >>> 8) & 0xFF] ^ T4[t3 & 0xFF])
              ^ Ker[0];
      a1 =
          (T1[(t1 >>> 24) & 0xFF] ^ T2[(t2 >>> 16) & 0xFF] ^ T3[(t3 >>> 8) & 0xFF] ^ T4[t0 & 0xFF])
              ^ Ker[1];
      a2 =
          (T1[(t2 >>> 24) & 0xFF] ^ T2[(t3 >>> 16) & 0xFF] ^ T3[(t0 >>> 8) & 0xFF] ^ T4[t1 & 0xFF])
              ^ Ker[2];
      a3 =
          (T1[(t3 >>> 24) & 0xFF] ^ T2[(t0 >>> 16) & 0xFF] ^ T3[(t1 >>> 8) & 0xFF] ^ T4[t2 & 0xFF])
              ^ Ker[3];
      t0 = a0;
      t1 = a1;
      t2 = a2;
      ...
      if (RDEBUG && (logMINOR))
        System.out.println(
            "CT" + r + '=' + intToString(t0) + intToString(t1) + intToString(t2) + intToString(t3));
    }
  ...
  private static void blockEncrypt256(byte[] in, byte[] result, int inOffset, Object sessionKey) {
    ...
    for (int r = 1; r < ROUNDS; r++) { // apply round transforms
      Ker = Ke[r];
      a0 =
          (T1[(t0 >>> 24) & 0xFF] ^ T2[(t1 >>> 16) & 0xFF] ^ T3[(t3 >>> 8) & 0xFF] ^ T4[t4 & 0xFF])
              ^ Ker[0];

      a1 =
          (T1[(t1 >>> 24) & 0xFF] ^ T2[(t2 >>> 16) & 0xFF] ^ T3[(t4 >>> 8) & 0xFF] ^ T4[t5 & 0xFF])
              ^ Ker[1];

      a2 =
          (T1[(t2 >>> 24) & 0xFF] ^ T2[(t3 >>> 16) & 0xFF] ^ T3[(t5 >>> 8) & 0xFF] ^ T4[t6 & 0xFF])
              ^ Ker[2];

      a3 =
          (T1[(t3 >>> 24) & 0xFF] ^ T2[(t4 >>> 16) & 0xFF] ^ T3[(t6 >>> 8) & 0xFF] ^ T4[t7 & 0xFF])
              ^ Ker[3];

      a4 =
          (T1[(t4 >>> 24) & 0xFF] ^ T2[(t5 >>> 16) & 0xFF] ^ T3[(t7 >>> 8) & 0xFF] ^ T4[t0 & 0xFF])
              ^ Ker[4];

      a5 =
          (T1[(t5 >>> 24) & 0xFF] ^ T2[(t6 >>> 16) & 0xFF] ^ T3[(t0 >>> 8) & 0xFF] ^ T4[t1 & 0xFF])
              ^ Ker[5];

      a6 =
          (T1[(t6 >>> 24) & 0xFF] ^ T2[(t7 >>> 16) & 0xFF] ^ T3[(t1 >>> 8) & 0xFF] ^ T4[t2 & 0xFF])
              ^ Ker[6];

      a7 =
          (T1[(t7 >>> 24) & 0xFF] ^ T2[(t0 >>> 16) & 0xFF] ^ T3[(t2 >>> 8) & 0xFF] ^ T4[t3 & 0xFF])
              ^ Ker[7];
      t0 = a0;
      t1 = a1;
      t2 = a2;
  ...
  private static void blockDecrypt(byte[] in, byte[] result, int inOffset, Object sessionKey) {
    ...
    for (int r = 1; r < ROUNDS; r++) { // apply round transforms
      Kdr = Kd[r];
      a0 =
          (T5[(t0 >>> 24) & 0xFF] ^ T6[(t3 >>> 16) & 0xFF] ^ T7[(t2 >>> 8) & 0xFF] ^ T8[t1 & 0xFF])
              ^ Kdr[0];
      a1 =
          (T5[(t1 >>> 24) & 0xFF] ^ T6[(t0 >>> 16) & 0xFF] ^ T7[(t3 >>> 8) & 0xFF] ^ T8[t2 & 0xFF])
              ^ Kdr[1];
      a2 =
          (T5[(t2 >>> 24) & 0xFF] ^ T6[(t1 >>> 16) & 0xFF] ^ T7[(t0 >>> 8) & 0xFF] ^ T8[t3 & 0xFF])
              ^ Kdr[2];
      a3 =
          (T5[(t3 >>> 24) & 0xFF] ^ T6[(t2 >>> 16) & 0xFF] ^ T7[(t1 >>> 8) & 0xFF] ^ T8[t0 & 0xFF])
              ^ Kdr[3];
      t0 = a0;
      t1 = a1;
      t2 = a2;
  ...
  private static void blockDecrypt256(byte[] in, byte[] result, int inOffset, Object sessionKey) {
    ...
    for (int r = 1; r < ROUNDS; r++) { // apply round transforms
      Kdr = Kd[r];
      a0 =
          (T5[(t0 >>> 24) & 0xFF] ^ T6[(t7 >>> 16) & 0xFF] ^ T7[(t5 >>> 8) & 0xFF] ^ T8[t4 & 0xFF])
              ^ Kdr[0];

      a1 =
          (T5[(t1 >>> 24) & 0xFF] ^ T6[(t0 >>> 16) & 0xFF] ^ T7[(t6 >>> 8) & 0xFF] ^ T8[t5 & 0xFF])
              ^ Kdr[1];

      a2 =
          (T5[(t2 >>> 24) & 0xFF] ^ T6[(t1 >>> 16) & 0xFF] ^ T7[(t7 >>> 8) & 0xFF] ^ T8[t6 & 0xFF])
              ^ Kdr[2];

      a3 =
          (T5[(t3 >>> 24) & 0xFF] ^ T6[(t2 >>> 16) & 0xFF] ^ T7[(t0 >>> 8) & 0xFF] ^ T8[t7 & 0xFF])
              ^ Kdr[3];

      a4 =
          (T5[(t4 >>> 24) & 0xFF] ^ T6[(t3 >>> 16) & 0xFF] ^ T7[(t1 >>> 8) & 0xFF] ^ T8[t0 & 0xFF])
              ^ Kdr[4];

      a5 =
          (T5[(t5 >>> 24) & 0xFF] ^ T6[(t4 >>> 16) & 0xFF] ^ T7[(t2 >>> 8) & 0xFF] ^ T8[t1 & 0xFF])
              ^ Kdr[5];

      a6 =
          (T5[(t6 >>> 24) & 0xFF] ^ T6[(t5 >>> 16) & 0xFF] ^ T7[(t3 >>> 8) & 0xFF] ^ T8[t2 & 0xFF])
              ^ Kdr[6];

      a7 =
          (T5[(t7 >>> 24) & 0xFF] ^ T6[(t6 >>> 16) & 0xFF] ^ T7[(t4 >>> 8) & 0xFF] ^ T8[t3 & 0xFF])
              ^ Kdr[7];
      t0 = a0;
      t1 = a1;
      t2 = a2;
  ...
  static void blockEncrypt(
    ...
    if (RDEBUG)
      trace(
          IN, "blockEncrypt(" + in + ", " + inOffset + ", " + sessionKey + ", " + blockSize + ')');
  ...
  static void blockDecrypt(
    ...
    if (RDEBUG)
      trace(
          IN, "blockDecrypt(" + in + ", " + inOffset + ", " + sessionKey + ", " + blockSize + ')');
