...
final class Rijndael_Algorithm { // implicit no-argument constructor
	...
	private static void debug(String s) {
		if (logDEBUG) Logger.debug(
			Rijndael_Algorithm.class,
			">>> " + NAME + ": " + s
		);
	}

	private static void trace(boolean in, String s) {
		if (TRACE && logDEBUG) Logger.debug(
			Rijndael_Algorithm.class,
			(in ? "==> " : "<== ") + NAME + '.' + s
		);
	}
	...
	static {
		...
		if (RDEBUG && (logDEBUG)) {
			...
			for (i = 0; i < 5; i++) {
				for (j = 0; j < 6; j++) System.out.print(
					"0x" + byteToString(rcon[i * 6 + j]) + ", "
				);
				System.out.println();
			}
	...
	private static void generateSBoxes() {
		...
		for (int i = 2; i < 256; i++) {
			...
			for (int t = 0; t < 8; t++) box[i][t] = (byte) ((j >>> (7 - t)) &
				0x01);
		}
		//
		...
		for (int i = 0; i < 256; i++) for (int t = 0; t < 8; t++) {
			cox[i][t] = B[t];
	...
	private static byte[][] generateInvertedGMatrix(byte[][] gMatrix) {
		...
		for (int i = 0; i < 4; i++) {
			...
			for (int j = 0; j < 8; j++) if (AA[i][j] != 0) AA[i][j] =
				(byte) alog[(255 + log[AA[i][j] & 0xFF] - log[pivot & 0xFF]) %
					255];
			for (int t = 0; t < 4; t++) if (i != t) {
				for (int j = i + 1; j < 8; j++) AA[t][j] ^=
				mul(AA[i][j], AA[t][i]);
				AA[t][i] = 0;
			}
		}
		for (int i = 0; i < 4; i++) for (int j = 0; j < 4; j++) iG[i][j] =
			AA[i][j + 4];
	...
	private static void blockEncrypt(
		...
		if (RDEBUG) trace(
			IN,
			"blockEncrypt(" + in + ", " + inOffset + ", " + sessionKey + ')'
		);
		...
		for (int r = 1; r < ROUNDS; r++) { // apply round transforms
			...
			t2 = a2;
			t3 = a3;
			if (RDEBUG && (logMINOR)) System.out.println(
				"CT" +
				r +
				'=' +
				intToString(t0) +
	...
	private static void blockEncrypt256(
		...
		if (RDEBUG) trace(
			IN,
			"blockEncrypt256(" + in + ", " + inOffset + ", " + sessionKey + ')'
		);
		int[][] Ke = (int[][]) ((Object[]) sessionKey)[0]; // extract encryption round keys
		int ROUNDS = Ke.length - 1;
		int[] Ker = Ke[0];
	...
	private static void blockDecrypt(
		...
		if (RDEBUG) trace(
			IN,
			"blockDecrypt(" + in + ", " + inOffset + ", " + sessionKey + ')'
		);
	...
	static synchronized Object makeKey(byte[] k, int blockSize)
		...
		for (i = 0, j = 0; i < KC;) tk[i++] = ((k[j++] & 0xFF) << 24) |
		((k[j++] & 0xFF) << 16) |
		((k[j++] & 0xFF) << 8) |
		(k[j++] & 0xFF);
	...
	static void blockEncrypt(
		...
		Ke[0][i];
		for (int r = 1; r < ROUNDS; r++) { // apply round transforms
			for (i = 0; i < BC; i++) a[i] = (T1[(t[i] >>> 24) & 0xFF] ^
		...
		for (i = 0; i < BC; i++) { // last round is special
			...
			result[j++] = (byte) (S[(t[(i + s1) % BC] >>> 16) & 0xFF] ^
				(tt >>> 16));
			result[j++] = (byte) (S[(t[(i + s2) % BC] >>> 8) & 0xFF] ^
				(tt >>> 8));
	...
	static void blockDecrypt(
		...
		Kd[0][i];
		for (int r = 1; r < ROUNDS; r++) { // apply round transforms
			for (i = 0; i < BC; i++) a[i] = (T5[(t[i] >>> 24) & 0xFF] ^
		...
		for (i = 0; i < BC; i++) { // last round is special
			...
			result[j++] = (byte) (Si[(t[(i + s1) % BC] >>> 16) & 0xFF] ^
				(tt >>> 16));
			result[j++] = (byte) (Si[(t[(i + s2) % BC] >>> 8) & 0xFF] ^
				(tt >>> 8));
	...
	private static boolean self_test(int keysize) {
		...
		try {
			...
			if (RDEBUG && (debuglevel > 0)) {
				debug(
					"Exception encountered during self-test: " + x.getMessage()
				);
				x.printStackTrace();
			}
		}
