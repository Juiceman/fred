 ...
 *       {@link freenet.crypt.Util#makeKey freenet.crypt.Util.makeKey})
...
public class Yarrow extends RandomSource implements PersistentRandomSource {
  ...
  Yarrow(
    ...
    if (updateSeed
        && !(seed.toString())
            .equals(
                "/dev/urandom")) // Dont try to update the seedfile if we know that it wont be
                                 // possible anyways
    seedfile = seed;
    else seedfile = null;
    if (reseedOnStartup) {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if ((args.length == 0) || args[0].equalsIgnoreCase("latency")) {
      ...
      System.out.println((double) (System.currentTimeMillis() - start) / 1000 + " ms/int");
      start = System.currentTimeMillis();
      for (int i = 0; i < 1000; i++) r.nextLong();
      System.out.println((double) (System.currentTimeMillis() - start) / 1000 + " ms/long");
      ...
      long start = System.currentTimeMillis();
      for (int i = 0; i < 100000; i++) r.acceptEntropy(t, System.currentTimeMillis(), 32);
      ...
      if ((args.length == 1) || args[1].equals("general")) {
        ...
        for (int i = 0; i < 20; i++) System.out.println(r.nextDouble());
  }
