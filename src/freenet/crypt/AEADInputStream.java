...
public class AEADInputStream extends FilterInputStream {
	...
	public int read(byte[] buf, int offset, int length) throws IOException {
		...
		while (true) {
			...
			if (outLength > length) {
				...
				int decryptedBytes = cipher.processBytes(
					temp,
					0,
					read,
					outputTemp,
	...
	public void close() throws IOException {
		if (!finished) skip(Long.MAX_VALUE); // Must read the rest of the data to check hash integrity.
		in.close();
	}
	...
	public static AEADInputStream createAES(InputStream is, byte[] key)
		throws IOException {
