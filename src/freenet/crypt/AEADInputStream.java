...
public class AEADInputStream extends FilterInputStream {
   ...
   * the stream, so do NOT use Closer.close() etc and swallow IOException's on close(), as that's
   * what we will throw if authentication fails. We will read the nonce from the stream; it
