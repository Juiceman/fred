...
public class Util {
  ...
  /** Fill byte array with random data. randomBytes(random, buf) is same as random.nextBytes(buf) */
  public static void randomBytes(Random r, byte[] buf) {
    randomBytes(r, buf, 0, buf.length);
  }

  /**
   * Fill specified range of byte array with random data. Optimised version for Random with fast
   * nextInt(). Must be same as randomBytesSlowNextInt(buf, from, len). WARNING: full compatibility
   * with randomBytesSlowNextInt() is *critical*!
   */
  /*
