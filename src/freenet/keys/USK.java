 ...
 *
 * <p>Contains: - Enough information to produce a real SSK. - Site name. - Site edition number.
 *
...
public class USK extends BaseClientKey implements Comparable<USK>, Serializable {
  ...
  // Extra must be verified on creation, and is fixed for now. FIXME if it becomes changeable, need
  // to keep values here.
  ...
  public final USK copy() {
    ...
    // FIXME when we are sure we don't need to copy the byte[]'s we might be able to switch back to
    // clone().
    return new USK(this);
  }
