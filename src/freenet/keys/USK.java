...
public class USK extends BaseClientKey implements Comparable<USK>, Serializable {
	...
	public USK(byte[] pubKeyHash, byte[] cryptoKey, byte[] extra, String siteName,
			   long suggestedEdition) throws MalformedURLException {
	...
	public static USK create(FreenetURI uri) throws MalformedURLException {
		...
		return new USK(uri.getRoutingKey(), uri.getCryptoKey(), uri.getExtra(), uri.getDocName(),
	...
	public FreenetURI getURI() {
		return new FreenetURI(pubKeyHash, cryptoKey, ClientSSK.getExtraBytes(cryptoAlgorithm), siteName,
							  suggestedEdition);
	}
	...
	public FreenetURI turnMySSKIntoUSK(FreenetURI uri) {
		if(uri.getKeyType().equals("SSK") &&
			...
			return new FreenetURI("USK", siteName, uri.getAllMetaStrings(), pubKeyHash, cryptoKey,
