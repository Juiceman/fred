...
/**
 * Client level CHK. Can be converted into a FreenetURI, can be used to decrypt a CHKBlock, can be
 * produced by a CHKBlock.
 */
public class ClientCHK extends ClientKey implements Serializable {
  ...
  /**
   * Create from a raw binary CHK. This expresses the key information in as few bytes as possible.
   *
   * @throws IOException
   */
  ...
  static {
    for (byte cryptoAlgorithm = Key.ALGO_AES_PCFB_256_SHA256;
        cryptoAlgorithm <= Key.ALGO_AES_CTR_256_SHA256;
        cryptoAlgorithm++) {
      for (short compressionAlgorithm = -1;
          compressionAlgorithm <= (short) (COMPRESSOR_TYPE.countCompressors());
          compressionAlgorithm++) {
        standardExtras.add(
            new ByteArrayWrapper(getExtra(cryptoAlgorithm, compressionAlgorithm, true)));
        standardExtras.add(
            new ByteArrayWrapper(getExtra(cryptoAlgorithm, compressionAlgorithm, false)));
      }
    }
  }
  ...
  /** Read a raw binary CHK. This is an ultra-compact representation, for splitfile metadata etc. */
