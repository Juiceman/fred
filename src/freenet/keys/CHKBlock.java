...
public class CHKBlock implements KeyBlock {
	...
	public static CHKBlock construct(byte[] data, byte[] header,
	...
	public CHKBlock(byte[] data2, byte[] header2, NodeCHK key,
					byte cryptoAlgorithm) throws CHKVerifyException {
	...
	public CHKBlock(byte[] data2, byte[] header2, NodeCHK key, boolean verify,
		...
		if(headers.length != TOTAL_HEADERS_LENGTH) {
			throw new IllegalArgumentException("Wrong length: "+headers.length+" should be "
