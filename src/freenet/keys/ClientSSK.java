...
public class ClientSSK extends ClientKey {
	...
	private ClientSSK(ClientSSK key) {
		...
		hashCode = Fields.hashCode(pubKeyHash) ^ Fields.hashCode(cryptoKey) ^ Fields.hashCode(
					   ehDocname) ^ docName.hashCode();
	}

	public ClientSSK(String docName, byte[] pubKeyHash, byte[] extras, DSAPublicKey pubKey,
					 byte[] cryptoKey) throws MalformedURLException {
		...
		if(pubKeyHash.length != NodeSSK.PUBKEY_HASH_SIZE) {
			throw new MalformedURLException("Pubkey hash wrong length: "+pubKeyHash.length+" should be "
	...
	public ClientSSK(FreenetURI origURI) throws MalformedURLException {
		this(origURI.getDocName(), origURI.getRoutingKey(), origURI.getExtra(), null,
	...
	public synchronized void setPublicKey(DSAPublicKey pubKey) {
		...
		if(!Arrays.equals(newKeyHash, pubKeyHash)) {
			throw new IllegalArgumentException("New pubKey hash does not match pubKeyHash: "+HexUtil.bytesToHex(
												   newKeyHash)+" ( "+HexUtil.bytesToHex(pubKey.asBytesHash())+" != "+HexUtil.bytesToHex(
												   pubKeyHash)+" for "+pubKey);
		}
		this.pubKey = pubKey;
	...
	public Key getNodeKey(boolean cloneKey) {
		try {
			...
			synchronized(this) {
				...
				if (cachedNodeKey == null || cachedNodeKey.getKeyBytes() == null
