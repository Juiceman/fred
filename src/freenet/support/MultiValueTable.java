 ...
 *
 * <p>FIXME improve efficiency - map to Object internally and either use a single V or an ArrayList
 * of V. Then take over where we do this in the code e.g. PrioritizedTicker.
 *
 * @author oskar
 */
public class MultiValueTable<K, V> {
  ...
  public Enumeration<V> getAll(K key) {
    ...
    return (v == null ? new EmptyEnumeration<V>() : v.elements());
  }
  ...
  public Enumeration<V> elements() {
    synchronized (table) {
      if (table.isEmpty()) return new EmptyEnumeration<V>();
      else return new MultiValueEnumeration();
    }
  }
