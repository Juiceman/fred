...
public class MemoryLimitedJobRunner {
	...
	public synchronized void queueJob(final MemoryLimitedJob job) {
		...
		if(job.initialAllocation > capacity) {
			throw new IllegalArgumentException("Job size "+job.initialAllocation+" > capacity "+capacity);
		}
		if(logMINOR) {
	...
	synchronized void deallocate(long size, boolean finishedThread) {
		if(size == 0) {
			return;    // Can't do anything, legal no-op.
		}
		if(size < 0) {
	...
	private synchronized void startJob(final MemoryLimitedJob job) {
		...
		executor.execute(new PrioRunnable() {
			...
			@Override
			public void run() {
				MemoryLimitedChunk chunk = new MemoryLimitedChunk(MemoryLimitedJobRunner.this,
						job.initialAllocation);
				if(job.start(chunk)) {
					chunk.release();
				}
			}
