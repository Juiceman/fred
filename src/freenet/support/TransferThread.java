 ...
 *
 * <p>When calling <code>start()</code>, the thread will iterate the first time after <code>
 * getStartupDelay()</code> milliseconds. After each iteration, it will sleep for <code>
 * getSleepTime()</code> milliseconds.
 *
 * @deprecated This class uses {@link TrivialTicker} in a bogus way. See <a
 *     href="https://bugs.freenetproject.org/view.php?id=6423">issue 6423</a>.<br>
...
public abstract class TransferThread implements PrioRunnable, ClientGetCallback, ClientPutCallback {
  ...
  /**
   * Tells this TransferThread to start it's execution. You have to call this after constructing an
   * object of an implementing class - it must not be called in the constructors of implementing
   * classes.
   */
  public void start() {
  ...
  /**
   * Called by the TransferThread after getStartupDelay() milliseconds for the first time and then
   * after each getSleepTime() milliseconds.
   */
