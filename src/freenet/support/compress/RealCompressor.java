...
public class RealCompressor {
	...
	public RealCompressor() {
		this.executorService = Executors.newFixedThreadPool(
			getMaxRunningCompressionThreads(),
		...
		while (!executorService.isShutdown() && task == null) {
			try {
						...
						public void run() {
							...
							try {
								try {
									...
									Logger.error(
										this,
										"Caught in OffThreadCompressor: " + t,
										t
									);
									System.err.println(
										"Caught in OffThreadCompressor: " + t
				...
				if (logMINOR) Logger.minor(
					this,
					"Compression job: " + j + "has been enqueued."
				);
	...
	private static int getMaxRunningCompressionThreads() {
		...
		Logger.minor(
			RealCompressor.class,
			"Maximum Compressor threads: " + maxRunningThreads
		);
