...
public class PaddedEphemerallyEncryptedBucket implements Bucket, Serializable {
	...
	public PaddedEphemerallyEncryptedBucket(
		...
		this.bucket = bucket;
		if (bucket.size() != 0) throw new IllegalArgumentException(
	...
	public PaddedEphemerallyEncryptedBucket(
		PaddedEphemerallyEncryptedBucket orig,
		Bucket newBucket
	) {
	...
	public OutputStream getOutputStreamUnbuffered() throws IOException {
		...
		return new PaddedEphemerallyEncryptedOutputStream(
			os,
			++lastOutputStream
		);
	}
	...
	private class PaddedEphemerallyEncryptedOutputStream extends OutputStream {
		...
		public PaddedEphemerallyEncryptedOutputStream(
			OutputStream out,
			int streamNumber
		) {
			this.out = out;
		...
		public void write(int b) throws IOException {
			synchronized (PaddedEphemerallyEncryptedBucket.this) {
				...
				if (
					streamNumber != lastOutputStream
				) throw new IllegalStateException(
					"Writing to old stream in " + getName()
				);
			}
			//if((b < 0) || (b > 255))
			...
			try {
				...
				synchronized (PaddedEphemerallyEncryptedBucket.this) {
					...
					if (streamNumber != lastOutputStream) {
						Logger.normal(
							this,
							"Not padding out to length because have been superceded: " +
							getName()
						);
						return;
					}
	...
	public InputStream getInputStreamUnbuffered() throws IOException {
		return new PaddedEphemerallyEncryptedInputStream(
	...
	private class PaddedEphemerallyEncryptedInputStream extends InputStream {
		...
		public int read(byte[] buf, int offset, int length) throws IOException {
			...
			if (
				(length + offset > buf.length) || (offset < 0) || (length < 0)
		...
		while (true) {
			...
			if ((size >= min) && (size <= max)) {
				if (logMINOR) Logger.minor(
					PaddedEphemerallyEncryptedBucket.class,
					"Padded: " + max + " was: " + dataLength
				);
				return max;
			}
			min = max;
	...
	public PCFBMode getPCFB() {
		...
		else return PCFBMode.create(aes); // Since the key is unique it is actually almost safe to use all zeros IV, but it's better to use a real IV. // FIXME CRYPTO We should probably migrate all old buckets automatically so we can get rid of this?
	}
	...
	protected PaddedEphemerallyEncryptedBucket(
		...
		bucket = BucketTools.restoreFrom(
			dis,
			fg,
