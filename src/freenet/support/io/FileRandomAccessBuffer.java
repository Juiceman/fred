...
public class FileRandomAccessBuffer implements LockableRandomAccessBuffer, Serializable {
	...
	public FileRandomAccessBuffer(RandomAccessFile raf, File filename,
	...
	throws IOException {
		...
		if(fileOffset + length > this.length) {
			throw new IOException("Length limit exceeded reading "+length+" bytes from "+fileOffset+" of "
								  +this.length);
		}
	...
	throws IOException {
		...
		if(fileOffset + length > this.length) {
			throw new IOException("Length limit exceeded writing "+length+" bytes from "+fileOffset+" of "
								  +this.length);
		}
		if(readOnly) {
	...
	public FileRandomAccessBuffer(DataInputStream dis) throws IOException, StorageFormatException,
