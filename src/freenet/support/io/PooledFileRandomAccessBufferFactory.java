...
public class PooledFileRandomAccessBufferFactory implements LockableRandomAccessBufferFactory {
	...
	public LockableRandomAccessBuffer makeRAF(long size) throws IOException {
		...
		try {
			ret = new PooledFileRandomAccessBuffer(file, false, size, enableCrypto ? seedRandom : null, id,
												   true);
			return ret;
		} finally {
			if(ret == null) {
	...
	throws IOException {
		...
		LockableRandomAccessBuffer ret = null;
		try {
			ret = new PooledFileRandomAccessBuffer(file, "rw", initialContents, offset, size, id, true,
												   readOnly);
			return ret;
		} finally {
			if(ret == null) {
