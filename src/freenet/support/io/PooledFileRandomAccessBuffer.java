...
public class PooledFileRandomAccessBuffer implements LockableRandomAccessBuffer, Serializable {
	...
	static class FDTracker implements Serializable {
		...
		private final LinkedHashSet<PooledFileRandomAccessBuffer> closables = new
	 ...
	 * @throws IOException
	 */
	public PooledFileRandomAccessBuffer(File file, boolean readOnly, long forceLength,
										Random seedRandom, long persistentTempID, boolean deleteOnFree) throws IOException {
		this(file, readOnly, forceLength, seedRandom, persistentTempID, deleteOnFree, DEFAULT_FDTRACKER);
	}

	// For unit testing
	PooledFileRandomAccessBuffer(File file, boolean readOnly, long forceLength, Random seedRandom,
								 long persistentTempID, boolean deleteOnFree, FDTracker fds) throws IOException {
		this.file = file;
		...
		try {
			...
			if(forceLength >= 0 && forceLength != currentLength) {
				...
				try (WrapperKeepalive wrapperKeepalive = new WrapperKeepalive()) {
					...
					Fallocate.forChannel(raf.getChannel(), raf.getFD(),
										 forceLength).fromOffset(currentLength).execute();
				}
	...
	public PooledFileRandomAccessBuffer(File file, String mode, byte[] initialContents,
										int offset, int size, long persistentTempID, boolean deleteOnFree,
	...
	PooledFileRandomAccessBuffer(DataInputStream dis, FilenameGenerator fg,
