...
public class TempBucketFactory
	implements BucketFactory, LockableRandomAccessBufferFactory {
	...
	public class TempBucket implements Bucket, Migratable, RandomAccessBucket {
		...
		private synchronized void closeInputStreams(boolean forFree) {
			for (
				ListIterator<TempBucketInputStream> i = tbis.listIterator();
				i.hasNext();
			) {
		...
		public final boolean migrateToDisk() throws IOException {
			...
			if (logMINOR) Logger.minor(
				this,
				"We have migrated " + toMigrate.hashCode()
			);
		...
		@Override
		public synchronized OutputStream getOutputStreamUnbuffered()
			throws IOException {
			if (os != null) throw new IOException(
				"Only one OutputStream per bucket on " + this + " !"
			);
			...
			private void _maybeMigrateRamBucket(long futureSize)
				...
				if (isRAMBucket()) {
					...
					if (
						...
						Math.min(
							Integer.MAX_VALUE,
							maxRAMBucketSize * RAMBUCKET_CONVERSION_FACTOR
						)
					) {
						isOversized = true;
						shouldMigrate = true;
					} else if (
						(futureSize - currentSize) + bytesInUse >= maxRamUsed
					) shouldMigrate = true;

					if (shouldMigrate) {
						if (logMINOR) {
							if (isOversized) Logger.minor(
								this,
								"The bucket " +
								TempBucket.this +
								" is over " +
								...
								this,
								"The bucketpool is full: force-migrate before we go over the limit"
							);
						}
						migrateToDisk();
					}
					...
					if (futureSize - lastCheckedSize >= CHECK_DISK_EVERY) {
						if (
							filenameGenerator.getDir().getUsableSpace() +
		...
		@Override
		public synchronized InputStream getInputStreamUnbuffered()
			throws IOException {
			if (!hasWritten) throw new IOException(
		...
		private WeakReference<Migratable> weakRef = new WeakReference<
			Migratable
		>(this);
		...
		protected void finalize() throws Throwable {
			...
			if (!hasBeenFreed) {
				if (TRACE_BUCKET_LEAKS) Logger.error(
					this,
					"TempBucket not freed, size=" +
					size() +
					", isRAMBucket=" +
					isRAMBucket() +
					" : " +
		...
		public LockableRandomAccessBuffer toRandomAccessBuffer()
			...
			synchronized (this) {
				if (hasBeenFreed) throw new IOException("Already freed");
				if (os != null) throw new IOException(
					"Can't migrate with open OutputStream's"
				);
				if (!tbis.isEmpty()) throw new IOException(
					"Can't migrate with open InputStream's"
				);
				setReadOnly();
				TempRandomAccessBuffer raf = new TempRandomAccessBuffer(
	...
	public TempBucketFactory(
		...
		this.underlyingDiskRAFFactory = new PooledFileRandomAccessBufferFactory(
			filenameGenerator,
			weakPRNG
		);
		underlyingDiskRAFFactory.enableCrypto(reallyEncrypt);
		this.minDiskSpace = minDiskSpace;
		this.diskRAFFactory = new DiskSpaceCheckingRandomAccessBufferFactory(
			underlyingDiskRAFFactory,
			filenameGenerator.getDir(),
			minDiskSpace - maxRamUsed
		);
	...
	public RandomAccessBucket makeBucket(long size, float factor)
		throws IOException {
	...
	public TempBucket makeBucket(long size, float factor, long increment)
		...
		if (useRAMBucket) { // No need to consider them for migration if they can't be migrated
			...
			if (size != -1 && size != Long.MAX_VALUE) {
				if (
					filenameGenerator.getDir().getUsableSpace() + size <
					minDiskSpace
				) throw new InsufficientDiskSpaceException();
			}
		}
		...
		public void run() {
			...
			try {
				...
				while (true) {
					try {
						...
						if (!saidSo) {
							Logger.error(
								this,
								"Insufficient disk space to migrate in-RAM buckets to disk!"
							);
				...
				while (true) {
					...
					try {
						if (
							!cleanBucketQueue(System.currentTimeMillis(), true)
						) return;
					} catch (InsufficientDiskSpaceException e) {
						if (!saidSo) {
							Logger.error(
	...
	private boolean cleanBucketQueue(long now, boolean force)
		...
		if (logMINOR) Logger.minor(this, "Starting cleanBucketQueue");
		do {
			synchronized (ramBucketQueue) {
				final WeakReference<Migratable> tmpBucketRef =
					ramBucketQueue.peek();
		...
		if (toMigrate.size() > 0) {
			if (logMINOR) Logger.minor(
				this,
				"We are going to migrate " + toMigrate.size() + " RAMBuckets"
			);
			for (Migratable tmpBucket : toMigrate) {
				try {
					...
					Logger.error(
						tmpBucket,
						"An IOE occured while migrating long-lived buckets:" +
						e.getMessage(),
						e
					);
				}
	...
	private RandomAccessBucket _makeFileBucket() throws IOException {
		RandomAccessBucket ret = new TempFileBucket(
	...
	class TempRandomAccessBuffer
		...
		public TempRandomAccessBuffer(
			...
			super(
				new ByteArrayRandomAccessBuffer(
					initialContents,
					offset,
					size,
					readOnly
		...
		@Override
		protected LockableRandomAccessBuffer innerMigrate(
			LockableRandomAccessBuffer underlying
		) throws IOException {
			ByteArrayRandomAccessBuffer b =
				(ByteArrayRandomAccessBuffer) underlying;
		...
		protected void finalize() throws Throwable {
			...
			if (!hasBeenFreed()) {
				if (TRACE_BUCKET_LEAKS) Logger.error(
					this,
					"TempRandomAccessBuffer not freed, size=" +
					size() +
					" : " +
					this,
					tracer
	...
	public LockableRandomAccessBuffer makeRAF(long size) throws IOException {
		...
		if (raf != null) {
			...
			if (encrypt) {
				...
				paddedSize = PaddedEphemerallyEncryptedBucket.paddedLength(
					realSize,
			...
			if (encrypt) {
				if (realSize != paddedSize) ret = new PaddedRandomAccessBuffer(
					ret,
					realSize
				);
				try {
					ret = new EncryptedRandomAccessBuffer(
						CRYPT_TYPE,
						ret,
						secret,
						true
					...
					Logger.error(
						this,
						"Cannot create encrypted tempfile: " + e,
						e
					);
				}
			}
	...
	public LockableRandomAccessBuffer makeRAF(
		...
		synchronized (this) {
			if (
				...
				raf = new TempRandomAccessBuffer(
					initialContents,
					offset,
					size,
					now,
