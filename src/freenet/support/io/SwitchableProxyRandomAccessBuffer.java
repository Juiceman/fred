...
abstract class SwitchableProxyRandomAccessBuffer implements LockableRandomAccessBuffer {
  ...
  /**
   * Underlying temporary storage. May change! Will be thread-safe as with all RAT's. Current
   * implementations just lock all writes/reads.
   */
  ...
  /**
   * Called after freeing the underlying storage. That includes when migrating, not just when free()
   * is called!
   */
  ...
  protected final void migrate() throws IOException {
    try {
      ...
      if (lockOpenCount > 0) underlyingLock.unlock();
      underlying.close();
      underlying.free();
