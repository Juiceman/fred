...
public class FilenameGenerator {
	...
	public FilenameGenerator(
		...
		this.random = random;
		this.prefix = prefix;
		if (dir == null) tmpDir = FileUtil.getCanonicalFile(
		...
		if (
			!(tmpDir.isDirectory() && tmpDir.canRead() && tmpDir.canWrite())
		) throw new IOException(
			"Not a directory or cannot read/write: " + tmpDir
		);
		if (wipeFiles) {
			long wipedFiles = 0;
			...
			if (filenames != null) {
				for (int i = 0; i < filenames.length; i++) {
					...
					if (
						...
						wipeableFiles++;
						if ((!f.delete()) && f.exists()) System.err.println(
	...
	protected boolean matches(File file) {
		return (
			FileUtil.equals(file.getParentFile(), tmpDir) &&
	...
	public File maybeMove(File file, long id) {
		...
		if (FileUtil.moveTo(file, newFile, false)) return newFile;
			...
			Logger.error(
				this,
				"Unable to move old temporary file " + file + " to " + newFile
			);
			return file;
		}
	}
