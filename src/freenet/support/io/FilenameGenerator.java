		...
		if(wipeFiles) {
			...
			if(filenames != null) {
				for(int i=0; i<filenames.length; i++) {
					...
					{
						System.err.println("Deleted "+wipedFiles+" temp files ("+(i - wipeableFiles)
				...
				System.err.println("Deleted "+wipedFiles+" of "+wipeableFiles+" temporary files ("+
								   (filenames.length-wipeableFiles)+" non-temp files in temp directory) in "+TimeUtil.formatTime(
									   endWipe-startWipe));
			}
		}
	}
