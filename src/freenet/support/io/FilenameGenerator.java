 ...
 * attacks and race conditions). FIXME Consider using File.createTempFile(). Note that using our own
 * code could actually be more secure if we use a better PRNG than they do (they use "new Random()"
 * IIRC, but maybe that's fixed now?). If we do change to using File.createTempFile(), we will need
 * to change TempFileBucket accordingly.
 *
 * @author toad
 */
public class FilenameGenerator {
  ...
  public FilenameGenerator(Random random, boolean wipeFiles, File dir, String prefix)
    ...
    else tmpDir = FileUtil.getCanonicalFile(dir);
    if (!tmpDir.exists()) {
      tmpDir.mkdir();
    }
        ...
        for (int i = 0; i < filenames.length; i++) {
          ...
          if ((((File.separatorChar == '\\') && name.toLowerCase().startsWith(prefix.toLowerCase()))
            ...
            if ((!f.delete()) && f.exists())
              System.err.println(
                  "Unable to delete temporary file " + f + " - permissions problem?");
            else wipedFiles++;
          }
        }
