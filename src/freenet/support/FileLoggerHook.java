...
public class FileLoggerHook extends LoggerHook implements Closeable {
   ...
   *
   * <p>Unfortunately, we can't use ConcurrentBlockingQueue because we need to dump stuff when the
   * queue gets too big.
   *
   * <p>FIXME PERFORMANCE: Using an ArrayBlockingQueue avoids some unnecessary memory allocations,
   * but it means we have to take two locks. Seriously consider reverting
   * 88268b99856919df0d42c2787d9ea3674a9f6f0d..e359b4005ef728a159fdee988c483de8ce8f3f6b to go back
   * to one lock and a LinkedList.
   */
  ...
  public void setInterval(String intervalName) throws IntervalParseException {
    ...
    if (intervalName.equalsIgnoreCase("MINUTE")) INTERVAL = Calendar.MINUTE;
    else if (intervalName.equalsIgnoreCase("HOUR")) INTERVAL = Calendar.HOUR;
    else if (intervalName.equalsIgnoreCase("DAY")) INTERVAL = Calendar.DAY_OF_MONTH;
    else if (intervalName.equalsIgnoreCase("WEEK")) INTERVAL = Calendar.WEEK_OF_YEAR;
    else if (intervalName.equalsIgnoreCase("MONTH")) INTERVAL = Calendar.MONTH;
    else if (intervalName.equalsIgnoreCase("YEAR")) INTERVAL = Calendar.YEAR;
    else throw new IntervalParseException("invalid interval " + intervalName);
  ...
  /**
   * The extra parameter int digit is to be used for creating a logfile name when a log exists
   * already with the same date.
   *
   * @param c
  ...
  class WriterThread extends Thread {
      ...
      if (baseFilename != null) {
        ...
        switch (INTERVAL) {
          ...
          case Calendar.WEEK_OF_YEAR:
            if (INTERVAL == Calendar.WEEK_OF_YEAR) gc.set(Calendar.DAY_OF_WEEK, 0);
        ...
        if (logMINOR) Logger.minor(this, "Start time: " + gc + " -> " + startTime);
        lastTime = startTime;
      ...
      while (true) {
        try {
          ...
          synchronized (list) {
            flush = flushTime;
            long maxWait;
            if (timeWaitingForSync == -1) maxWait = Long.MAX_VALUE;
            else maxWait = timeWaitingForSync + flush;
            o = list.poll();
            while (o == null) {
              if (closed) {
                died = true;
              ...
              try {
                if (thisTime < maxWait) {
                  // Wait no more than 500ms since the CloserThread might be waiting for
                  // closedFinished.
                  list.wait(Math.min(500L, maxWait - thisTime));
                  thisTime = System.currentTimeMillis();
                  if (listBytes < LIST_WRITE_THRESHOLD) {
                    // Don't write at all until the lower bytes threshold is exceeded, or the time
                    // threshold is.
                    assert ((listBytes == 0) == (list.peek() == null));
                    if (listBytes != 0 && maxWait == Long.MAX_VALUE) maxWait = thisTime + flush;
          ...
          if (timeoutFlush || died) {
            ...
            if (altLogStream != null) myWrite(altLogStream, null);
          }
          if (died) {
            try {
              logStream.close();
          ...
          if (o == null) continue;
          myWrite(logStream, o);
          if (altLogStream != null) myWrite(altLogStream, o);
    ...
    private File rotateLog(
        File currentFilename, long lastTime, long nextHour, GregorianCalendar gc) {
      // Switch logs
      try {
        logStream.flush();
  ...
  /**
   * Create a Logger to append to the given file. If the file does not exist it will be created.
   *
   * @param filename the name of the file to log to.
   * @param fmt log message format string
   * @param dfmt date format string
   * @param threshold Lowest logged priority
   * @param assumeWorking If false, check whether stderr and stdout are writable and if not,
  ...
  public void trimOldLogFiles() {
    synchronized (trimOldLogFilesLock) {
      while (oldLogFilesDiskSpaceUsage > maxOldLogfilesDiskUsage) {
        OldLogFile olf;
        // TODO: creates a double lock situation, but only here. I think this is okay because the
        // inner lock is only used for trivial things.
        synchronized (logFiles) {
          if (logFiles.isEmpty()) {
            System.err.println(
  ...
  public void findOldLogFiles(GregorianCalendar gc) {
    ...
    File oldFile = null;
    if (latestFile.exists()) FileUtil.renameTo(latestFile, previousFile);
    ...
    for (File f : files) {
      ...
      if (name.toLowerCase().startsWith(prefix)) {
        ...
        if (nums.length > 2) gc.set(Calendar.MONTH, nums[2] - 1);
        if (nums.length > 3) gc.set(Calendar.DAY_OF_MONTH, nums[3]);
        if (nums.length > 4) gc.set(Calendar.HOUR_OF_DAY, nums[4]);
        if (nums.length > 5) gc.set(Calendar.MINUTE, nums[5]);
        gc.set(Calendar.SECOND, 0);
        gc.set(Calendar.MILLISECOND, 0);
  ...
  public FileLoggerHook(OutputStream os, String fmt, String dfmt, LogLevel threshold)
      throws IntervalParseException {
    this(os, fmt, dfmt, threshold, true);
    logStream = os;
  }

  public FileLoggerHook(OutputStream os, String fmt, String dfmt, String threshold)
      throws InvalidThresholdException, IntervalParseException {
   ...
   */
  public FileLoggerHook(
      OutputStream stream, String fmt, String dfmt, LogLevel threshold, boolean overwrite)
      throws IntervalParseException {
  ...
  public void start() {
    if (redirectStdOut) {
      try {
        ...
        System.setOut(
            new PrintStream(
                new OutputStreamLogger(LogLevel.NORMAL, "Stdout: ", encName), false, encName));
        if (redirectStdErr) {
          System.setErr(
              new PrintStream(
                  new OutputStreamLogger(LogLevel.ERROR, "Stderr: ", encName), false, encName));
        }
  ...
  public FileLoggerHook(
    ...
    // "+threshold);
    if (!assumeWorking) checkStdStreams();
    if (rotate) {
      this.baseFilename = baseFilename;
    } else {
      logStream =
          new BufferedOutputStream(new FileOutputStream(baseFilename, !logOverwrite), 65536);
    }
  }
  ...
  private void setLogFormat(String fmt) {
    if ((fmt == null) || (fmt.length() == 0)) fmt = "d:c:h:t:p:m";
    ...
    for (int i = 0; i < size; ++i) this.fmt[i] = fmtVec.get(i);
  ...
  public void logString(byte[] b) throws UnsupportedEncodingException {
    synchronized (list) {
      ...
      if (!list.offer(b)) {
        ...
        if (list.offer(buf)) listBytes += (buf.length + LINE_OVERHEAD);
        if (list.offer(b)) listBytes += (b.length + LINE_OVERHEAD);
      } else listBytes += (b.length + LINE_OVERHEAD);
      int x = 0;
      if (listBytes > MAX_LIST_BYTES) {
        ...
        if (!list.offer(buf)) {
          ...
          if (ss != null) listBytes -= ss.length + LINE_OVERHEAD;
          if (list.offer(buf)) listBytes += (buf.length + LINE_OVERHEAD);
        } else listBytes += (buf.length + LINE_OVERHEAD);
      }
      if (sz == 0) list.notifyAll();
    }
  }
  ...
  public void listAvailableLogs(OutputStreamWriter writer) throws IOException {
    ...
    for (OldLogFile olf : oldLogFiles) {
      writer.write(
          olf.filename.getName()
              + " : "
              + tempDF.format(new Date(olf.start))
              + " to "
              + tempDF.format(new Date(olf.end))
              + " - "
              + olf.size
              + " bytes\n");
    }
  ...
  public void sendLogByContainedDate(long time, OutputStream os) throws IOException {
    ...
    synchronized (logFiles) {
      ...
      if (toReturn == null) return; // couldn't find it
    }
   ...
   * immediately, but this will likely not be completed by the time the function returns as it is
   * run off-thread.
   */
