...
public class PrioritizedTicker implements Ticker, Runnable {
	...
	private void realRun() {
		...
		synchronized (timedJobsByTime) {
			while (!timedJobsByTime.isEmpty()) {
				...
				if (tRun.longValue() <= now) {
					if (jobsToRun == null) jobsToRun = new ArrayList<Job>();
		...
		if (jobsToRun != null) for (Job r : jobsToRun) {
			...
			if (r.job instanceof FastRunnable) try { // Run in-line
				r.job.run();
			} catch (Throwable t) {
	...
	private void queueTimedJobInner(
		...
		synchronized (timedJobsByTime) {
			if (noDupes) {
				...
				if (alreadyQueuedAt != null) {
					if (alreadyQueuedAt <= runJobAt) {
						Logger.normal(
							this,
							"Not re-running as already queued: " +
							runner +
							" for " +
							name
						);
			...
			else if (o instanceof Job) timedJobsByTime.put(
				runJobAt,
				new Job[] { (Job) o, job }
			);
		...
		if (o instanceof Job) {
			...
			if (jobs.length == 1) {
				...
				if (x == 1) {
					...
					if (x != newJobs.length) newJobs = Arrays.copyOf(
						newJobs,
						x
					);
