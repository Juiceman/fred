 ...
 *
 * <p>Unless otherwise stated, the conventions follow the rules outlined in the Java Language
 * Specification.
 *
 * @author syoung
 */
public class HexUtil {
  private static final boolean logDEBUG = Logger.shouldLog(LogLevel.DEBUG, HexUtil.class);
  ...
  public static void bytesToHexAppend(byte[] bs, int off, int length, StringBuilder sb) {
    if (bs.length < off + length) throw new IllegalArgumentException();
  ...
  public static void writeBigInteger(BigInteger integer, DataOutputStream out) throws IOException {
    ...
    if (buf.length > Short.MAX_VALUE) throw new IllegalStateException("Too long: " + buf.length);
  ...
  /**
   * Turn a BigInteger into a hex string. BigInteger.toString(16) NPEs on Sun/Oracle JDK 1.4.2_05.
