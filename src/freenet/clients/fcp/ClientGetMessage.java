...
public class ClientGetMessage extends BaseDataCarryingMessage {
	...
	public ClientGetMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if(returnType == ReturnType.DIRECT) {
			...
			if(filename == null) {
				throw new MessageInvalidException(ProtocolErrorMessage.MISSING_FIELD, "Missing Filename",
												  identifier, global);
			}
			...
			try {
				...
				throw new MessageInvalidException(ProtocolErrorMessage.COULD_NOT_CREATE_FILE, e.getMessage(),
												  identifier, global);
			}
		} else {
			...
			try {
				...
				if(maxSize < 0) {
					throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
													  "Maximum size must be positive", identifier, global);
				}
			} catch (NumberFormatException e) {
				throw new MessageInvalidException(ProtocolErrorMessage.ERROR_PARSING_NUMBER,
			...
			// default to unlimited
		{
			...
			try {
				...
				if(maxTempSize < 0) {
					throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
													  "Maximum temp size must be positive", identifier, global);
				}
			} catch (NumberFormatException e) {
				throw new MessageInvalidException(ProtocolErrorMessage.ERROR_PARSING_NUMBER,
												  "Error parsing MaxSize field: "+e.getMessage(), identifier, global);
			...
			// default to 0
		{
			...
			try {
				...
				if(maxRetries < -1) {
					throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
													  "Max retries must be -1 or larger", identifier, global);
				}
			} catch (NumberFormatException e) {
				throw new MessageInvalidException(ProtocolErrorMessage.ERROR_PARSING_NUMBER,
												  "Error parsing MaxSize field: "+e.getMessage(), identifier, global);
		...
		if(global && (persistence == Persistence.CONNECTION)) {
			throw new MessageInvalidException(ProtocolErrorMessage.NOT_SUPPORTED,
											  "Global requests must be persistent", identifier, global);
		}
		...
		if(initialMetadataLength < 0) {
			throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
											  "Invalid data length for initial metadata", identifier, global);
		}
	...
	ReturnType parseReturnTypeFCP(String string) throws MessageInvalidException {
		try {
			if(string == null) {
				return ReturnType.DIRECT;
			}
			return ReturnType.valueOf(string.toUpperCase());
		} catch (IllegalArgumentException e) {
			throw new MessageInvalidException(ProtocolErrorMessage.INVALID_FIELD,
											  "Unable to parse ReturnType "+string+" : "+e, identifier, global);
		}
	}
