...
public class ClientPutDiskDirMessage extends ClientPutDirMessage {
	...
	public ClientPutDiskDirMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if(fnam == null) {
			throw new MessageInvalidException(ProtocolErrorMessage.MISSING_FIELD, "Filename missing",
											  identifier, global);
		}
	...
	throws MessageInvalidException {
		if(!handler.server.core.allowUploadFrom(dirname)) {
			throw new MessageInvalidException(ProtocolErrorMessage.ACCESS_DENIED,
											  "Not allowed to upload from "+dirname, identifier, global);
		}
	 ...
	 * @throws MessageInvalidException
	 */
	private HashMap<String, Object> makeBucketsByName(File thisdir,
			String prefix) throws MessageInvalidException {
		...
		if(filelist == null) {
			throw new MessageInvalidException(ProtocolErrorMessage.FILE_NOT_FOUND, "No such directory!",
											  identifier, global);
		}
		for(int i = 0 ; i < filelist.length ; i++) {
			...
			if (filelist[i].canRead() && filelist[i].exists()) {
				if (filelist[i].isFile()) {
					...
					ret.put(f.getName(), new ManifestElement(f.getName(), prefix + f.getName(), bucket,
					...
					throw new MessageInvalidException(ProtocolErrorMessage.FILE_NOT_FOUND,
													  "Not directory and not file: "+filelist[i], identifier, global);
				}
			} else {
				if(!allowUnreadableFiles) {
					throw new MessageInvalidException(ProtocolErrorMessage.FILE_NOT_FOUND,
													  "Not readable or doesn't exist: "+filelist[i], identifier, global);
				}
			}
		}
		return ret;
