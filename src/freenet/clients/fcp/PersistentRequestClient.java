...
public class PersistentRequestClient {
  ...
  /**
   * Called when a client request has finished, but is persistent. It has not been acked yet, so it
  ...
  /**
   * Queue any and all pending messages from already completed, unacknowledged, persistent requests,
   * to be immediately sent. This happens automatically on startup and hopefully will encourage
   * clients to acknowledge persistent requests!
   */
   ...
   * @param verbosityMask If so, what verbosity mask to use (to filter messages generated by the
   *     global queue).
   */
    ...
    if (watchGlobal && !enabled) {
      ...
      if (connHandler != null) {
        if (persistence == Persistence.REBOOT)
          server.globalRebootClient.queuePendingMessagesOnConnectionRestartAsync(
              connHandler.outputHandler, server.core.clientContext);
        else
          server.globalForeverClient.queuePendingMessagesOnConnectionRestartAsync(
              connHandler.outputHandler, server.core.clientContext);
      }
      watchGlobal = true;
    }
