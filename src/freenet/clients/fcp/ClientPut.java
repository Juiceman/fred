...
public class ClientPut extends ClientPutBase {
   ...
   *
   * @param uri The URI to insert data to
   * @param identifier The identifier of the insert
   * @param verbosity The verbosity bitmask
   * @param charset TODO
   * @param priorityClass The priority for this insert
   * @param persistence The persistence type of this insert
   * @param clientToken The client token of this insert
   * @param getCHKOnly Whether only the resulting CHK is requested
   * @param dontCompress Whether the file should not be compressed
   * @param maxRetries The maximum number of retries
   * @param uploadFromType Where the file is uploaded from
   * @param origFilename The original filename
   * @param contentType The content type of the data
   * @param data The data (may be <code>null</code> if <code>uploadFromType</code> is
   *     UPLOAD_FROM_REDIRECT)
   * @param redirectTarget The URI to redirect to (if <code>uploadFromType</code> is
   *     UPLOAD_FROM_REDIRECT)
   * @param handler The FCP connection handler
   * @param global Whether this insert appears on the global queue
  ...
  public ClientPut(
      ...
      throws IdentifierCollisionException,
          ...
          IOException {
    super(
        uri = checkEmptySSK(uri, targetFilename, core.clientContext),
        identifier,
        verbosity,
        charset,
        null,
  ...
  public ClientPut(FCPConnectionHandler handler, ClientPutMessage message, FCPServer server)
    ...
    super(
        checkEmptySSK(message.uri, message.targetFilename, server.core.clientContext),
        message.identifier,
        message.verbosity,
        null,
        handler,
    ...
    if (salt != null) {
      ...
      if (!Arrays.equals(saltedHash, foundHash))
        throw new MessageInvalidException(
            ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED,
            "The hash doesn't match! (salt used : \"" + salt + "\")",
            identifier,
            global);
    }
