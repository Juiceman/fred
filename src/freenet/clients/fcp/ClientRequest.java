...
public abstract class ClientRequest implements Serializable {
    ...
    if (lowLevelClient.persistent() != (persistence == Persistence.FOREVER))
      throw new IllegalStateException(
          "Low level client.persistent="
              ...
              + persistence);
    if (client != null) assert (client.persistence == persistence);
    this.startupTime = System.currentTimeMillis();
    this.realTime = realTime;
  }
  ...
  /**
   * Returns the time of the request’s last activity, or {@code 0} if there is no known last
   * activity.
   *
   * @return The time of the request’s last activity, or {@code 0}
   * @deprecated Use {@link ClientRequester#getLatestSuccess()} instead. You can use {@link
  ...
  public void modifyRequest(String newClientToken, short newPriorityClass, FCPServer server) {
    ...
    if (clientTokenChanged && priorityClassChanged) {
      modifiedMsg =
          new PersistentRequestModifiedMessage(identifier, global, priorityClass, clientToken);
  ...
  /**
   * Called after a RemovePersistentRequest. Send a PersistentRequestRemoved to the clients. If the
   * request is in the database, delete it.
   */
   ...
   * starts there, and we MUST NOT call back to it or we get an infinite recursion. The main purpose
   * of this method is to give us an opportunity to connect to the various (transient) system
   * utilities we get from ClientContext, e.g. bucket factories, the FCP persistent root etc. The
   * base class implementation in ClientRequest will register the request with an
  ...
  /**
   * Return true if we resumed the original fetch from stored data (usually a file for a splitfile
   * download), rather than having to restart it (which happens in most other cases when we resume).
   */
  public abstract boolean fullyResumed();

  /**
   * Called just before the final write when the node is shutting down. Should write any dirty data
   * to disk etc.
   */
