...
public class RequestStatusCache {
	...
	synchronized void addDownload(DownloadRequestStatus status) {
		RequestStatus old = requestsByIdentifier.put(
			status.getIdentifier(),
			status
		);
		if (logMINOR) Logger.minor(
			this,
			"Starting download " + status.getIdentifier()
		);
	...
	synchronized void addUpload(UploadRequestStatus status) {
		...
		if (logMINOR) Logger.minor(
			this,
			"Starting upload " + status.getIdentifier()
		);
	...
	synchronized void finishedDownload(
		...
		DownloadRequestStatus status =
			(DownloadRequestStatus) requestsByIdentifier.get(identifier);
	...
	public void updateCompressionStatus(
		String identifier,
		COMPRESS_STATE compressing
	) {
		UploadFileRequestStatus status =
			(UploadFileRequestStatus) requestsByIdentifier.get(identifier);
		if (status == null) return; // Can happen during cancel etc.
	...
	public synchronized void addTo(List<RequestStatus> status) {
		...
		for (RequestStatus req : requestsByIdentifier.values()) status.add(
			req.clone()
		);
	}

	public synchronized void updateExpectedMIME(
		String identifier,
	...
	public synchronized void updateExpectedDataLength(
		String identifier,
