...
public class ClientPutMessage extends DataCarryingMessage {
	...
	public ClientPutMessage(SimpleFieldSet fs) throws MessageInvalidException {
		...
		if (s == null) cmode = InsertContext.CompatibilityMode.COMPAT_DEFAULT;
			...
			try {
				...
				try {
					cmode =
						InsertContext.CompatibilityMode.values()[Integer.parseInt(
								s
							)];
		...
		try {
			if (binaryBlob) uri = new FreenetURI("CHK@");
				...
				String u = fs.get("URI");
				if (u == null) throw new MessageInvalidException(
					ProtocolErrorMessage.MISSING_FIELD,
					"No URI",
					identifier,
					global
		...
		if ((uploadFrom == null) || uploadFrom.equalsIgnoreCase("direct")) {
			...
			if (dataLengthString == null) throw new MessageInvalidException(
				ProtocolErrorMessage.MISSING_FIELD,
			...
			if (target == null) throw new MessageInvalidException(
				ProtocolErrorMessage.MISSING_FIELD,
		...
		if (fs.get("ForkOnCacheable") != null) forkOnCacheable = fs.getBoolean(
			"ForkOnCacheable",
			false
		);
		else forkOnCacheable = Node.FORK_ON_CACHEABLE_DEFAULT;
		extraInsertsSingleBlock = fs.getInt(
			"ExtraInsertsSingleBlock",
			HighLevelSimpleClientImpl.EXTRA_INSERTS_SINGLE_BLOCK
		);
		extraInsertsSplitfileHeaderBlock = fs.getInt(
	...
	public SimpleFieldSet getFieldSet() {
		...
		if (compressorDescriptor != null) sfs.putSingle(
			"Codecs",
	...
	@Override
	RandomAccessBucket createBucket(
		BucketFactory bf,
		long length,
		FCPServer server
	...
	public void freeData() {
		if (bucket == null) {
			...
			Logger.error(
				this,
				"bucket is null on " + this + " - freed twice?",
				new Exception("error")
			);
			return;
		}
