...
public class ClientGet extends ClientRequest implements ClientGetCallback, ClientEventListener,
	...
	public ClientGet(PersistentRequestClient globalClient, FreenetURI uri, boolean dsOnly,
					 ...
					 long maxOutputLength, ReturnType returnType, boolean persistRebootOnly, String identifier,
					 int verbosity,
					 short prioClass, File returnFilename, String charset, boolean writeToClientCache,
					 boolean realTimeFlag, boolean binaryBlob, NodeClientCore core) throws IdentifierCollisionException,
	...
	public ClientGet(FCPConnectionHandler handler, ClientGetMessage message,
		...
		super(message.uri, message.identifier, message.verbosity, message.charset, handler,
			  message.priorityClass, message.persistence, message.realTimeFlag, message.clientToken,
			  message.global);
		...
		if(returnType == ReturnType.DISK) {
			...
			if(!core.allowDownloadTo(targetFile)) {
				throw new MessageInvalidException(ProtocolErrorMessage.ACCESS_DENIED,
												  "Not allowed to download to "+targetFile, identifier, global);
				...
				throw new MessageInvalidException(ProtocolErrorMessage.DIRECT_DISK_ACCESS_DENIED,
												  "Not allowed to download to " + targetFile + ". You might need to do a " +
	...
	private ClientGetter makeGetter(NodeClientCore core, Bucket ret) throws IOException {
		if (binaryBlob && ret == null) {
			ret = core.clientContext.getBucketFactory(persistence == Persistence.FOREVER).makeBucket(
					  fctx.maxOutputLength);
		}

		return new ClientGetter(this,
								uri, fctx, priorityClass,
								binaryBlob ? new NullBucket() : ret, binaryBlob ? new BinaryBlobWriter(ret) : null, false,
								initialMetadata, extensionCheck);
	}
	...
	public void setSuccessForMigration(ClientContext context, long completionTime,
									   Bucket data) throws ResumeFailedException {
		synchronized(this) {
			succeeded = true;
			started = true;
			finished = true;
	...
	private void trySendDataFoundOrGetFailed(FCPConnectionOutputHandler handler,
		...
		if(succeeded) {
			...
			msg = new DataFoundMessage(foundDataLength, foundDataMimeType, identifier, global, startupTime,
	...
	private void trySendAllDataMessage(FCPConnectionOutputHandler handler,
		...
		if(handler != null) {
			FCPMessage allData = FCPMessage.withListRequestIdentifier(getAllDataMessage(),
	...
	private void queueProgressMessageInner(FCPMessage msg, FCPConnectionOutputHandler handler,
										   int verbosityMask) {
	...
	public void sendPendingMessages(FCPConnectionOutputHandler handler, String listRequestIdentifier,
		...
		if(!onlyData) {
			...
			if(sentToNetwork) {
				handler.queue(FCPMessage.withListRequestIdentifier(new SendingToNetworkMessage(identifier, global),
							  listRequestIdentifier));
			}
			if(finished) {
			...
			ProtocolErrorMessage msg = new ProtocolErrorMessage(ProtocolErrorMessage.WRONG_RETURN_TYPE, false,
					"No AllData", identifier, global);
			handler.queue(msg);
			return;
		}
	...
	private FCPMessage persistentTagMessage() {
		return new PersistentGet(identifier, uri, verbosity, priorityClass, returnType, persistence,
								 targetFile, clientToken, client.isGlobalQueue, started, fctx.maxNonSplitfileRetries, binaryBlob,
								 fctx.maxOutputLength, isRealTime());
	}
	...
	private void handleCompatibilityMode(final SplitfileCompatibilityModeEvent ce,
	...
	private void innerHandleCompatibilityMode(SplitfileCompatibilityModeEvent ce,
			ClientContext context) {
		compatMode.merge(ce.minCompatibilityMode, ce.maxCompatibilityMode, ce.splitfileCryptoKey,
						 ce.dontCompress, ce.bottomLayer);
		if(client != null) {
			RequestStatusCache cache = client.getRequestStatusCache();
			if(cache != null) {
				cache.updateDetectedCompatModes(identifier, compatMode.getModes(), compatMode.getCryptoKey(),
												compatMode.dontCompress());
			}
		}
		if((verbosity & VERBOSITY_COMPATIBILITY_MODE) != 0) {
			queueProgressMessageInner(new CompatibilityMode(identifier, global, compatMode), null,
									  VERBOSITY_COMPATIBILITY_MODE);
		}
	}
	...
	synchronized RequestStatus getStatus() {
		...
		if(shadow != null) {
			if(dataSize != shadow.size()) {
				Logger.error(this, "Size of downloaded data has changed: "+dataSize+" -> "+shadow.size()+" on "
							 +shadow);
				shadow = null;
			} else {
				...
				if(returnType == ReturnType.DIRECT) {
					try {
						DataInputStream innerDIS =
							new DataInputStream(checker.checksumReaderWithLength(dis, context.tempBucketFactory, 65536));
						try {
							returnBucketDirect = BucketTools.restoreFrom(innerDIS, context.persistentFG,
	...
	private void readTransientProgressFields(DataInputStream dis) throws IOException,
