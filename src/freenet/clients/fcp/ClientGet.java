...
public class ClientGet extends ClientRequest
  ...
  public ClientGet(
    ...
    if (returnType == ReturnType.DISK) {
      ...
      if (!(core.allowDownloadTo(returnFilename))) throw new NotAllowedException();
      ...
      if (filterData) {
        ...
        if (idx != -1) {
          idx++;
          if (idx != name.length()) extensionCheck = name.substring(idx);
        }
      }
    ...
    if (returnType == ReturnType.DISK) {
      ...
      if (!core.allowDownloadTo(targetFile))
        throw new MessageInvalidException(
            ...
            "Not allowed to download to " + targetFile,
            identifier,
            global);
  ...
  void register(boolean noTags) throws IdentifierCollisionException {
    if (client != null) assert (this.persistence == client.persistence);
  ...
  private void trySendAllDataMessage(
    ...
    if (persistence == Persistence.CONNECTION) {
      if (handler == null) handler = origHandler.outputHandler;
    }
    if (handler != null) {
      FCPMessage allData =
          FCPMessage.withListRequestIdentifier(getAllDataMessage(), listRequestIdentifier);
      if (allData != null) handler.queue(allData);
    }
  }

  private void queueProgressMessageInner(
      FCPMessage msg, FCPConnectionOutputHandler handler, int verbosityMask) {
    if (persistence == Persistence.CONNECTION && handler == null)
      handler = origHandler.outputHandler;
    if (handler != null) handler.queue(msg);
    else client.queueClientRequestMessage(msg, verbosityMask);
  }
  ...
  private void innerHandleCompatibilityMode(
    ...
    if ((verbosity & VERBOSITY_COMPATIBILITY_MODE) != 0)
      queueProgressMessageInner(
          new CompatibilityMode(identifier, global, compatMode),
          null,
   ...
   *
   * @return The data in a {@link Bucket}, or <code>null</code> if this isn&rsquo;t applicable
   */
  ...
  public boolean restart(ClientContext context, final boolean disableFilterData) {
    ...
    synchronized (this) {
      ...
      } else if (getFailedMessage != null) redirect = getFailedMessage.redirectURI;
  ...
  private ClientGet(
    ...
    if (magic != CLIENT_DETAIL_MAGIC) throw new StorageFormatException("Bad magic for request");
