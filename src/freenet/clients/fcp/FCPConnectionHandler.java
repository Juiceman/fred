...
public class FCPConnectionHandler implements Closeable {
  ...
  public void close() {
    ...
    for (ClientRequest req : requests) req.onLostConnection(server.core.clientContext);
  ...
  public void setClientName(final String name) {
    ...
    rebootClient.queuePendingMessagesOnConnectionRestartAsync(
        outputHandler, server.core.clientContext);
  ...
  public void startClientPutDir(
    ...
    if (failedMessage != null) {
      ...
      outputHandler.queue(failedMessage);
      if (cp != null) cp.cancel(server.core.clientContext);
      return;
    } else {
  ...
  public ClientRequest removeRequestByIdentifier(String identifier, boolean kill) {
    ...
    if (req != null) {
      if (kill) req.cancel(server.core.clientContext);
  ...
  ClientRequest getRebootRequest(boolean global, FCPConnectionHandler handler, String identifier) {
    if (global) return handler.server.globalRebootClient.getRequest(identifier);
  ...
  ClientRequest getForeverRequest(boolean global, FCPConnectionHandler handler, String identifier) {
    if (global) return handler.server.globalForeverClient.getRequest(identifier);
  ...
  ClientRequest removePersistentRebootRequest(boolean global, String identifier)
    ...
    PersistentRequestClient client = global ? server.globalRebootClient : getRebootClient();
  ...
  ClientRequest removePersistentForeverRequest(boolean global, String identifier)
      throws MessageInvalidException {
    PersistentRequestClient client = global ? server.globalForeverClient : getForeverClient();
    ClientRequest req = client.getRequest(identifier);
    if (req != null) {
      client.removeByIdentifier(identifier, true, server, server.core.clientContext);
    }
