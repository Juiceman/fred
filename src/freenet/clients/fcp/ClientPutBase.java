...
public abstract class ClientPutBase extends ClientRequest
  ...
  /**
   * If the request failed, how did it fail? PutFailedMessage is the most convenient way to store
   * this (InsertException has a stack trace!).
   */
  ...
  public void receive(final ClientEvent ce, ClientContext context) {
    ...
    if (ce instanceof SplitfileProgressEvent) {
      ...
      if ((verbosity & VERBOSITY_EXPECTED_HASHES) == VERBOSITY_EXPECTED_HASHES) {
        ExpectedHashes msg = new ExpectedHashes((ExpectedHashesEvent) ce, identifier, global);
  ...
  private void trySendProgressMessage(
      final FCPMessage msg,
      final int verbosity,
      FCPConnectionOutputHandler handler,
      ClientContext context) {
    synchronized (this) {
      if (persistence != Persistence.CONNECTION) progressMessage = msg;
    }
    if (persistence == Persistence.CONNECTION && handler == null)
      handler = origHandler.outputHandler;
    if (handler != null) handler.queue(msg);
    else client.queueClientRequestMessage(msg, verbosity);
  }
