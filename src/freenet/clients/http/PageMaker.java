...
public final class PageMaker {
  ...
  public enum THEME {
    ...
    /**
     * If true, the activelinks will appear on the welcome page, whether the user has enabled them
     * or not.
     */
    public final boolean forceActivelinks;

    /**
     * If true, the "Fetch a key" infobox will appear above the bookmarks infobox on the welcome
     * page.
     */
    ...
    public static THEME themeFromName(String cssName) {
      for (THEME t : THEME.values()) {
        if (t.code.equalsIgnoreCase(cssName) || t.name.equalsIgnoreCase(cssName)) {
          return t;
        }
      }
      return getDefault();
  ...
  private static class SubMenu {
    ...
    public void addNavigationLink(
      ...
      if (!fullOnly) navigationLinkTextsNonFull.add(name);
      navigationLinkTitles.put(name, title);
      navigationLinks.put(name, path);
      if (cb != null) navigationLinkCallbacks.put(name, cb);
      if (l10n != null) navigationLinkL10n.put(name, l10n);
    }
  ...
  public void setTheme(THEME theme2) {
    if (theme2 == null) {
      ...
      if (themeurl == null) this.theme = THEME.getDefault();
      else this.theme = theme2;
    }
  }
  ...
  public synchronized void addNavigationLink(
    ...
    if (menu == null) throw new NullPointerException("there is no menu named " + menutext);
    menu.addNavigationLink(path, name, title, fullOnly, cb, l10n);
  }

  /**
   * Remove a navigation link from a sub-menu. Applies globally, do not use this to customise menus
   * when sending one page!
   */
  public synchronized void removeNavigationLink(String menutext, String name) {
    SubMenu menu = subMenus.get(menutext);
    // The menu may have already been removed.
    if (menu != null) menu.removeNavigationLink(name);
  }
  ...
  public HTMLNode createBackLink(ToadletContext toadletContext, String name) {
    ...
    return new HTMLNode(
        "a", new String[] {"href", "title"}, new String[] {"javascript:back()", name}, name);
  }
  ...
  /**
   * Generates an FProxy template page with optional navigation bar suitable for adding content to.
   *
   * @param title Title of the page.
   * @param renderNavigationLinks Whether to render navigation links.
   * @param ctx ToadletContext to use to render the page.
   * @return A template PageNode.
   * @deprecated Use {@link #getPageNode(String, ToadletContext, RenderParameters)} instead
   */
  @Deprecated
  ...
  /**
   * Generates an FProxy template page with optional navigation bar and status information suitable
   ...
   *
   * @param title Title of the page.
   * @param ctx ToadletContext to use to render the page. Can be null, e.g. if the HTML is not being
   *     generated as part of a toadlet request, for example if it's using the old FredPluginHTTP
   *     interface.
  ...
  public PageNode getPageNode(String title, ToadletContext ctx, RenderParameters renderParameters) {
    ...
    if (override != null) headNode.addChild(getOverrideContent());
    else
      headNode.addChild(
          "link",
          new String[] {"rel", "href", "type", "title"},
          new String[] {
            "stylesheet", "/static/themes/" + theme.code + "/theme.css", "text/css", theme.code
          });
      ...
      for (THEME t : THEME.values()) {
        ...
        headNode.addChild(
            ...
            new String[] {
              ...
              "/static/themes/" + themeName + "/theme.css",
              "text/css",
              "screen",
              themeName
            });
    ...
    if (ctx != null && ctx.getContainer().isFProxyJavascriptEnabled()) {
      ...
      if (themeJsUrl != null) {
        headNode.addChild(
            ...
            new String[] {
              "text/javascript", "javascript", "/static/themes/" + theme.code + "/script.js"
            });
      }
    }
    ...
    if (renderParameters.isRenderStatus() && fullAccess) {
      final HTMLNode statusBarDiv =
          pageDiv.addChild("div", "id", "statusbar-container").addChild("div", "id", "statusbar");
      ...
      statusBarDiv
          .addChild("div", "id", "statusbar-language")
          .addChild(
              "a", "href", "/config/node#l10n", NodeL10n.getBase().getSelectedLanguage().fullName);
      ...
      if (node.clientCore != null && ctx != null && renderParameters.isRenderModeSwitch()) {
        ...
        switchMode.addChild(
            "a",
            "href",
            "?" + HTTPRequestImpl.createQueryString(parameters, false),
            isAdvancedMode
      ...
      if (node != null && node.clientCore != null) {
        ...
        network.addAttribute(
            "title", NodeL10n.getBase().getString("SecurityLevels.networkThreatLevelShort"));
        network.addAttribute(
            "class", node.securityLevels.getNetworkThreatLevel().toString().toLowerCase());

        final HTMLNode physical =
            secLevels.addChild(
                "a",
                "href",
                "/seclevels/",
        ...
        physical.addAttribute(
            "title", NodeL10n.getBase().getString("SecurityLevels.physicalThreatLevelShort"));
        physical.addAttribute(
            "class", node.securityLevels.getPhysicalThreatLevel().toString().toLowerCase());
            ...
            new String[] {
              ...
              NodeL10n.getBase()
                  .getString(
                      ...
                      new String[] {"X", "Y"},
                      new String[] {
                        Integer.toString(node.peers.countConnectedDarknetPeers()),
                        Integer.toString(node.peers.countConnectedOpennetPeers())
                      })
            },
            Integer.toString(connectedPeers)
                + ((totalPeers != Integer.MAX_VALUE) ? " / " + Integer.toString(totalPeers) : ""));
      }
    }
    ...
    if (renderParameters.isRenderNavigationLinks()) {
          ...
          for (String navigationLink :
            ...
            if (navigationTitle != null)
              sublistItem.addChild(
                  ...
                  new String[] {navigationPath, navigationTitle},
                  navigationLink);
            else sublistItem.addChild("a", "href", navigationPath, navigationLink);
          }
          if (nonEmpty) {
            HTMLNode listItem;
            if (isSelected) {
      ...
      if (selected != null) {
        ...
        if (nonEmpty) pageDiv.addChild(div);
      }
    }
  ...
  /**
   * Filters a given string so that it will be a valid CSS identifier. It replaces all characters
   * that are not a dash, underscore, or alphanumeric with an underscore. If the first character is
   * a dash and the second character is not a letter or underscore, replaces the second character
   * with an underscore. This filter is overly strict as it does not allow non-ASCII characters or
   * escapes. If the given string is below two characters in length, it appends underscores until it
   * is not.
   *
  ...
  private HTMLNode getOverrideContent() {
    return new HTMLNode(
        "link",
        new String[] {"rel", "href", "type", "media", "title"},
        new String[] {"stylesheet", override, "text/css", "screen", "custom"});
  }
  ...
  public int parseMode(HTTPRequest req, ToadletContainer container) {
    ...
    if (req.isParameterSet(MODE_SWITCH_PARAMETER)) {
      ...
      if (mode == MODE_ADVANCED) container.setAdvancedMode(true);
      else container.setAdvancedMode(false);
    }
  ...
  /**
   * Bundles parameters that are used to create the page node. The default for the render parameters
   * is to include all optional render tasks. Individual tasks may be enabled or disabled by calling
   * the appropriate methods which returns a new {@link RenderParameters} object as {@link
   * RenderParameters} are immutable.
   *
  ...
  public static class RenderParameters {
     ...
     *
     * @return {@code true} if the navigation links should be included in the page, {@code false}
     *     otherwise
     */
    ...
    /**
     * Returns a new {@link RenderParameters} object that renders the navigation links according to
     * the given parameter.
     *
     ...
     *
     * @return {@code true} if the status bar should be included in the page, {@code false}
     *     otherwise
     */
    ...
    /**
     * Returns a new {@link RenderParameters} object that renders the status bar according to the
     * given parameter.
     *
     ...
     *
     * @return {@code true} if the mode switch should be included in the page, {@code false}
     *     otherwise
     */
    ...
    /**
     * Returns a new {@link RenderParameters} object that renders the mode switch according to the
     * given parameter.
     *
