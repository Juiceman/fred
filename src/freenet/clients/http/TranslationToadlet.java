...
public class TranslationToadlet extends Toadlet {
  ...
  public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
    ...
    if (request.isParameterSet("getOverrideTranlationFile")) {
      ...
      contentRow.addChild("td", "class", "translation-orig", this.base.getDefaultString(key));
      contentRow.addChild("td", "class", "translation-new", this.base.getString(key));

      HTMLNode footer = translationNode.addChild("div", "class", "warning");
      footer
          .addChild("a", "href", TOADLET_URL + "?getOverrideTranlationFile")
          .addChild("#", l10n("downloadTranslationsFile"));
      footer.addChild("%", "&nbsp;&nbsp;");
      footer
          .addChild(
              "a",
              "href",
              TOADLET_URL + "?translate=" + key + (showEverything ? "" : "&toTranslateOnly"))
          .addChild("#", l10n("reEdit"));
      footer.addChild("%", "&nbsp;&nbsp;");
      footer
          .addChild("a", "href", TOADLET_URL + (showEverything ? "" : "?toTranslateOnly"))
          .addChild("#", l10n("returnToTranslations"));
      ...
      contentRow
          .addChild("td", "class", "translation-new")
          .addChild(
              "textarea",
    ...
    final HTMLNode translatingForForm =
        ctx.addFormChild(translatingForBox, TOADLET_URL, "ChooseWhatToTranslate")
            .addChild("p", l10n("showTranslationOf"));
    final HTMLNode translatingForOption =
        translatingForForm.addChild("select", "name", "translating_for");
    ...
    translationHeaderNode.addChild(
        "#", l10n("contributingToLabelWithLang", "lang", this.base.getSelectedLanguage().fullName));
    ...
    if (showEverything)
      translationHeaderNode
          .addChild("a", "href", TOADLET_URL + "?toTranslateOnly")
          .addChild("#", l10n("hideAlreadyTranslated"));
    else
      translationHeaderNode
          .addChild("a", "href", TOADLET_URL)
          .addChild("#", l10n("showEverything"));
    HTMLNode legendTable = translationNode.addChild("table", "class", "translation");
    ...
    if (request.getPartAsStringFailsafe("translation_update", 32).length() > 0) {
      ...
      if ("on".equalsIgnoreCase(request.getPartAsStringFailsafe("gotoNext", 7))) {
        ...
        while (it.hasNext()) {
          ...
          if (isOverriden || (this.base.getString(newKey, true) != null)) continue;
          redirectTo(
              ctx,
              TOADLET_URL
                  + "?gotoNext&translate="
  ...
  private HTMLNode _setOrRemoveOverride(String key, boolean isOverriden, boolean showEverything) {
    ...
    HTMLNode translationField =
        new HTMLNode("span", "class", isOverriden ? "translate_d" : "translate_it");
    if (value == null) {
