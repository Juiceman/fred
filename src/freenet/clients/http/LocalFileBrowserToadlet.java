...
public abstract class LocalFileBrowserToadlet extends Toadlet {
  ...
  /**
   * Last directory from which an action was performed. If accessible, this is used instead of the
  ...
  /**
   * Whether the directory is allowed for the purposes of the specific browser. For example, do the
   * node settings allow downloading to the given directory?
   *
   * @param path The path to check permissions for.
   * @return Whether browsing that directory is allowed. If it is not, the LocalFileBrowserToadlet
   *     will not render a selection button or directory-changing button for it.
   */
  ...
  /**
   * Determines the appropriate directory to start out in for the given browser. If a path is not
   * already specified, the browser will attempt to display this directory.
   *
   ...
   *
   * @return If all directories or no directories are allowed, the default downloads directory is
   *     returned. Otherwise, returns the first allowed directory.
   */
  ...
  private void createChangeDirButton(
    ...
    node.addChild(
        "input", new String[] {"type", "name", "value"}, new String[] {"hidden", "path", path});
    node.addChild(persistence);
  }
   ...
   *     The user might be denied access to this directory, which could be their home directory.
  ...
  private void renderPage(
      Hashtable<String, String> fieldPairs, String path, final ToadletContext ctx, String filename)
      throws ToadletContextClosedException, IOException, RedirectException {
    ...
    if (currentPath != null && !allowedDir(currentPath)) {
      ...
      pageMaker
          .getInfobox("infobox-error", "Forbidden", page.content, "access-denied", true)
    ...
    if (currentPath != null
        ...
        && currentPath.canRead()) {
      PageNode page =
          pageMaker.getPageNode(l10n("listingTitle", "path", currentPath.getAbsolutePath()), ctx);
      pageNode = page.outer;
      ...
      infoboxDiv.addChild(
          "div", "class", "infobox-header", l10n("listing", "path", currentPath.getAbsolutePath()));
      ...
      if (currentPath.getParent() != null) {
        if (allowedDir(currentPath.getParentFile())) {
          ...
          HTMLNode backLinkFormNode =
              ctx.addFormChild(backLinkCellNode, path(), "insertLocalFileForm");
      ...
      for (File currentFile : files) {
        if (currentFile.isHidden()) {
          ...
          // FIXME in advanced mode, show them, and have a different style, and a toggle for whether
          // to include them in a folder insert.
          continue;
        }
        ...
        if (currentFile.isDirectory()) {
          if (currentFile.canRead()) {
            ...
            if (allowedDir(currentFile)) {
              ...
              HTMLNode formNode = ctx.addFormChild(cellNode, path(), "insertLocalFileForm");

              createSelectDirectoryButton(
                  formNode, currentFile.getAbsolutePath(), persistenceFields);

              // Change directory
              HTMLNode directoryCellNode = fileRow.addChild("td");
              HTMLNode directoryFormNode =
                  ctx.addFormChild(directoryCellNode, path(), "insertLocalFileForm");
              createChangeDirButton(
                  directoryFormNode,
                  currentFile.getName(),
  ...
  private String l10n(String key, String pattern, String value) {
    return NodeL10n.getBase()
        .getString("LocalFileInsertToadlet." + key, new String[] {pattern}, new String[] {value});
  }
