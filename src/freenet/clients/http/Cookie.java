		...
		*/
		...
		for(Character c : name.toCharArray()) {
			...
			if(httpSeparatorCharacters.contains(c)) {
				throw new IllegalArgumentException("Invalid name, contains one of the explicitely disallowed characters: "
												   + name);
			}
		}
		...
		*/
		...
		for(Character c : value.toCharArray()) {
			...
			if(invalidValueCharacters.contains(c)) {
				throw new IllegalArgumentException("Invalid value, contains one of the explicitely disallowed characters: "
												   + value);
			}
		}
...
//	}
	...
	protected String encodeToHeaderValue() {
		StringBuilder sb = new StringBuilder(
			512); // TODO: As soon as something else besides Freetalk uses cookies, adjust this value.
