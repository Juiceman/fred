...
public class DATASTORE_SIZE implements Step {
  ...
  public void getStep(HTTPRequest request, PageHelper helper) {
    ...
    if (maxSize >= 2l * 1024 * 1024 * 1024) {
      if (autodetectedSize != -1 || !sizeOption.isDefault()) {
        ...
        result.addChild(
            "option", new String[] {"value", "selected"}, new String[] {"2G", "on"}, "2GiB");
      }
    }
  ...
  public String postStep(HTTPRequest request) {
    // drop down options may be 6 chars or less, but formatted ones e.g. old value if re-running can
    // be more
  ...
  private static void _setDatastoreSize(
      String selectedStoreSize, boolean firsttime, Config config, Object callback) {
    try {
      ...
      System.out.println(
          "Setting client cache size to " + Fields.longToString(clientCacheSize, true));
