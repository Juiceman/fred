...
public class DATASTORE_SIZE implements Step {
	...
	public void getStep(HTTPRequest request, PageHelper helper) {
		...
		HTMLNode bandwidthInfoboxContent = helper.getInfobox("infobox-header",
		...
		if(!sizeOption.isDefault()) {
			long current = sizeOption.getValue() + clientCacheSizeOption.getValue() +
						   slashdotCacheSizeOption.getValue();
			result.addChild("option",
							new String[] { "value", "selected" },
							new String[] { SizeUtil.formatSize(current), "on" }, WizardL10n.l10n("currentPrefix")+" "
							+SizeUtil.formatSize(current));
		} else if(autodetectedSize != -1) {
			result.addChild("option",
							new String[] { "value", "selected" },
							new String[] { SizeUtil.formatSize(autodetectedSize), "on" }, SizeUtil.formatSize(
								autodetectedSize));
		}
	...
	private static void _setDatastoreSize(
		...
		try {
			...
			System.out.println("Setting slashdot/ULPR/recent requests cache size to "+Fields.longToString(
								   slashdotCacheSize, true));
