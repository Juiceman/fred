...
public class SECURITY_PHYSICAL implements Step {
  ...
  public void getStep(HTTPRequest request, PageHelper helper) {
    ...
    for (SecurityLevels.PHYSICAL_THREAT_LEVEL level :
      ...
      if (level == SecurityLevels.PHYSICAL_THREAT_LEVEL.HIGH
        ...
        p.addChild(
            "label", "for", "confirmPasswordBox", WizardL10n.l10nSec("confirmPasswordLabel") + ":");
        p.addChild(
            "input",
            new String[] {"id", "type", "name"},
  ...
  private boolean errorHandler(HTTPRequest request, PageHelper helper) {
    ...
    if (error.equals("pass")) {
      ...
      SecurityLevelsToadlet.sendPasswordFileCorruptedPageInner(
          helper, core.node.getMasterPasswordFile().getPath());
      return true;
    } else if (error.equals("delete")) {
      SecurityLevelsToadlet.sendCantDeleteMasterKeysFileInner(
          helper, core.node.getMasterPasswordFile().getPath(), newThreatLevel.name());
      return true;
    }
  ...
  public String postStep(HTTPRequest request) throws IOException {
    ...
    String physicalThreatLevel =
        request.getPartAsStringFailsafe("security-levels.physicalThreatLevel", 128);
    ...
    if (newThreatLevel == null
        || !request.isPartSet("security-levels.physicalThreatLevel")
    ...
    if ((newThreatLevel == SecurityLevels.PHYSICAL_THREAT_LEVEL.LOW
      ...
      if (passwordIsBlank) {
        ...
        try {
          ...
          System.err.println(
              "Already set a password when changing it - maybe master.keys copied in at the wrong moment???");
        }
      }
    }
    ...
    try {
      ...
      System.err.println(
          "Failed starting up database while switching physical security level to "
              ...
              + oldThreatLevel
              + " : "
              + core.node.getMasterPasswordFile()
              + " is too "
              + e.sizeToString());
    }
  }
