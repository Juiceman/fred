...
public class BookmarkManager implements RequestClient {
  ...
  public BookmarkManager(NodeClientCore n, boolean publicGateway) {
    ...
    try {
      ...
      Logger.error(
          this,
          "Error reading the bookmark file (" + bookmarksFile.toString() + "):" + ioe.getMessage(),
          ioe);
      ...
      try {
        if (backupBookmarksFile.exists()
          ...
          Logger.normal(
              this,
              "Attempting to read the backup bookmark file from " + backupBookmarksFile.toString());
  ...
  public BookmarkCategory getCategoryByPath(String path) {
    ...
    if (cat instanceof BookmarkCategory) return (BookmarkCategory) cat;
  ...
  public void addBookmark(String parentPath, Bookmark bookmark) {
    ...
    if (bookmark instanceof BookmarkItem) subscribeToUSK((BookmarkItem) bookmark);
  }
  ...
  public void storeBookmarksLazy() {
    synchronized (bookmarks) {
      ...
      node.node.ticker.queueTimedJob(
          new Runnable() {
