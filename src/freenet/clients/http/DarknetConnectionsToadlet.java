...
public class DarknetConnectionsToadlet extends ConnectionsToadlet {
	...
	protected class DarknetComparator extends ComparatorByStatus {
		...
		protected int customCompare(PeerNodeStatus firstNode, PeerNodeStatus secondNode, String sortBy) {
			if(sortBy.equals("name")) {
				...
				return ((DarknetPeerNodeStatus)firstNode).getPrivateDarknetCommentNote().compareToIgnoreCase(((
				...
				return ((DarknetPeerNodeStatus)firstNode).getTheirVisibility().compareTo(((
	...
	protected void drawNameColumn(HTMLNode peerRow, PeerNodeStatus peerNodeStatus, boolean advanced) {
		...
		cell.addChild("a", "href", "/send_n2ntm/?peernode_hashcode=" + peerNodeStatus.hashCode(),
	...
	protected void drawTrustColumn(HTMLNode peerRow, PeerNodeStatus peerNodeStatus) {
		peerRow.addChild("td", "class", "peer-trust").addChild("#",
				((DarknetPeerNodeStatus)peerNodeStatus).getTrustLevel().name());
	}
	...
	@Override
	protected void drawPrivateNoteColumn(HTMLNode peerRow, PeerNodeStatus peerNodeStatus,
		...
		if(fProxyJavascriptEnabled) {
			peerRow.addChild("td", "class", "peer-private-darknet-comment-note").addChild("input", new String[] { "type", "name", "size", "maxlength", "onChange", "value" },
					new String[] { "text", "peerPrivateNote_" + peerNodeStatus.hashCode(), "16", "250", "peerNoteChange();", status.getPrivateDarknetCommentNote() });
		} else {
			peerRow.addChild("td", "class", "peer-private-darknet-comment-note").addChild("input", new String[] { "type", "name", "size", "maxlength", "value" },
					new String[] { "text", "peerPrivateNote_" + peerNodeStatus.hashCode(), "16", "250", status.getPrivateDarknetCommentNote() });
		}
	}
	...
	protected void drawPeerActionSelectBox(HTMLNode peerForm, boolean advancedModeEnabled) {
		...
		if(advancedModeEnabled) {
			...
			actionSelect.addChild("option", "value", "set_ignore_source_port",
								  l10n("peersSetIgnoreSourcePort"));
			actionSelect.addChild("option", "value", "clear_ignore_source_port",
		...
		HTMLNode changeTrustLevelSelect = peerForm.addChild("select", new String[] { "id", "name" }, new
										  String[] { "changeTrust", "changeTrust" });
		...
		HTMLNode changeVisibilitySelect = peerForm.addChild("select", new String[] { "id", "name" }, new
										  String[] { "changeVisibility", "changeVisibility" });
		for(FRIEND_VISIBILITY trust : FRIEND_VISIBILITY.values()) {
			changeVisibilitySelect.addChild("option", "value", trust.name(),
											l10n("peerVisibility."+trust.name()));
		}
	}
		...
		if (request.isPartSet("doSendMessageToPeers")) {
			...
			for(DarknetPeerNode pn: peerNodes) {
				if (request.isPartSet("peerPrivateNote_"+pn.hashCode())) {
					if(!request.getPartAsStringFailsafe("peerPrivateNote_"+pn.hashCode(),
														250).equals(pn.getPrivateDarknetCommentNote())) {
						pn.setPrivateDarknetCommentNote(request.getPartAsStringFailsafe("peerPrivateNote_"+pn.hashCode(),
														250));
					}
				}
			}
			...
			FRIEND_VISIBILITY trust = FRIEND_VISIBILITY.valueOf(
										  request.getPartAsStringFailsafe("changeVisibility", 10));
			DarknetPeerNode[] peerNodes = node.getDarknetConnections();
			for(DarknetPeerNode pn: peerNodes) {
				if (request.isPartSet("node_"+pn.hashCode())) {
			...
			for(DarknetPeerNode pn: peerNodes) {
				if (request.isPartSet("node_"+pn.hashCode())) {
					if((pn.timeLastConnectionCompleted() < (System.currentTimeMillis() -
						...
						if(logMINOR) {
							Logger.minor(this, "Refusing to remove : node_"+pn.hashCode()
						...
						HTMLNode content =ctx.getPageMaker().getInfobox("infobox-warning",
										  l10n("confirmRemoveNodeWarningTitle"), contentNode, "darknet-remove-node", true);
						content.addChild("p").addChild("#",
													   NodeL10n.getBase().getString("DarknetConnectionsToadlet.confirmRemoveNode", new String[] { "name" },
			...
			for(DarknetPeerNode pn: peerNodes) {
				if (request.isPartSet("node_"+pn.hashCode())) {
					long id = Long.parseLong(request.getPartAsStringFailsafe("id",
											 32)); // FIXME handle NumberFormatException
	...
	private boolean tryHandlePeerNoderef(URI uri, HTTPRequest request,
		...
		if(path.endsWith(".fref") && path.startsWith(path()+"friend-")) {
			...
			input_hashcode_string = input_hashcode_string.substring(0,
									input_hashcode_string.length() - ".fref".length());
			int input_hashcode;
			try {
