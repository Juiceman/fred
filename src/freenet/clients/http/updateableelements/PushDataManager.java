...
/**
 * A manager class that manages all the pushing. All it's public method must be synchronized to
...
public class PushDataManager {
  ...
  private Map<String, List<UpdateEvent>> awaitingNotifications =
      new HashMap<String, List<UpdateEvent>>();

  /** What elements are on the page */
  private Map<String, List<BaseUpdateableElement>> pages =
  ...
  public synchronized void updateElement(String id) {
    ...
    if (elements.containsKey(id) == false) {
      if (logMINOR) {
        Logger.minor(
            this,
            "Element is updating, but not present on elements! elements:"
                + elements
                + " pages:"
                + pages
          ...
          if (notificationList.contains(updateEvent) == false) {
            ...
            if (logMINOR) {
              Logger.minor(
                  ...
                  "Notification("
                      + updateEvent
                      + ") added to a notification list for "
                      + entry.getKey());
            }
          } else {
            if (logMINOR)
  ...
  public synchronized BaseUpdateableElement getRenderedElement(String requestId, String id) {
    ...
    Logger.error(
        this,
        "Could not find data for the element requested. requestId:"
            + requestId
            + " id:"
            + id
            + " pages:"
  ...
  public synchronized UpdateEvent getNextNotification(String requestId) {
    ...
    while (awaitingNotifications.get(requestId) != null
            && awaitingNotifications.get(requestId).size() == 0
        || // No notifications
        (awaitingNotifications.get(requestId) != null
            && awaitingNotifications.get(requestId).size() != 0
            && isFirstKeepaliveReceived.containsKey(
                    awaitingNotifications.get(requestId).get(0).requestId)
                == false)) { // Not asked us yet
      try {
        wait();
    ...
    if (logMINOR) {
      Logger.minor(
          this,
          "Getting notification, notification:"
              + awaitingNotifications.get(requestId).get(0)
              + ",remaining:"
  ...
  public class UpdateEvent {
    ...
    public boolean equals(Object obj) {
      ...
      if (obj instanceof UpdateEvent) {
        ...
        if (o.getRequestId().compareTo(requestId) == 0
