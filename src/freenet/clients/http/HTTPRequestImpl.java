...
public class HTTPRequestImpl implements HTTPRequest {
  ...
  private void parseMultiPartData() throws IOException {
    ...
    try {
      ...
      while (is.available() > 0) {
        ...
        byte[] bbound =
            boundary.getBytes(StandardCharsets.UTF_8); // ISO-8859-1? boundary should be in US-ASCII
        int offset = 0;
        ...
        if (logMINOR)
          Logger.minor(
              this, "Name = " + name + " length = " + filedata.size() + " filename = " + filename);
        if (filename != null)
  ...
  public boolean isIncognito() {
    if (isParameterSet("incognito")) return Boolean.valueOf(getParam("incognito"));
    return false;
  }
