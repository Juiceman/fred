...
public class PproxyToadlet extends Toadlet {
    ...
    if (path.length() > 0) {
      ...
      try {
        ...
        ctx.sendReplyHeaders(
            DownloadPluginHTTPException.CODE, "Found", head, e.mimeType, e.data.length);
      ...
      if (request.isPartSet("submit-other")) {
        ...
        final boolean fileonly =
            "on".equalsIgnoreCase(request.getPartAsStringFailsafe("fileonly", 20));
      ...
      if (request.getPartAsStringFailsafe("unloadconfirm", MAX_PLUGIN_NAME_LENGTH).length() > 0) {
        String pluginThreadName =
            request.getPartAsStringFailsafe("unloadconfirm", MAX_PLUGIN_NAME_LENGTH);
      ...
      if (request.getPartAsStringFailsafe("unload", MAX_PLUGIN_NAME_LENGTH).length() > 0) {
        ...
        tempNode.addChild(
            "input", new String[] {"type", "name"}, new String[] {"checkbox", "purge"});
        ...
        final String pluginFilename =
            request.getPartAsStringFailsafe("update", MAX_PLUGIN_NAME_LENGTH);
      ...
      } else if (request.getPartAsStringFailsafe("reloadconfirm", MAX_PLUGIN_NAME_LENGTH).length()
          > 0) {
        boolean purge = request.isPartSet("purge");
        String pluginThreadName =
            request.getPartAsStringFailsafe("reloadconfirm", MAX_PLUGIN_NAME_LENGTH);
  ...
  public void handleMethodGET(URI uri, HTTPRequest request, ToadletContext ctx)
    ...
    try {
      if (path.isEmpty()) {
        ...
        for (List<OfficialPluginDescription> pluginDescriptions :
        ...
        showOfficialPluginLoader(
            ctx, contentNode, groupedAvailablePlugins, pm, advancedModeEnabled);
        ...
        // writeReply(ctx, 200, "text/html", "OK", mkPage("plugin", pm.handleHTTPGet(plugin,
        // data)));
  ...
  private void showPluginList(
    ...
    if (pm.getPlugins().isEmpty()) {
      ...
      if (advancedMode) headerRow.addChild("th", l10n("classNameTitle"));
      ...
      while (it.hasNext()) {
        ...
        if (advancedMode) pluginRow.addChild("td", pi.getPluginClassName());
        long ver = pi.getPluginLongVersion();
        if (ver != -1) pluginRow.addChild("td", pi.getPluginVersion() + " (" + ver + ")");
        else pluginRow.addChild("td", pi.getPluginVersion());
        if (advancedMode) {
        ...
        if (pi.isStopping()) {
            ...
            HTMLNode visitForm =
                pluginRow
                    ...
                    .addChild(
                        ...
                        new String[] {
                          "get", pi.getPluginClassName(), "_blank", "noreferrer noopener"
                        });
            visitForm.addChild(
                "input",
                new String[] {"type", "name", "value"},
          ...
          unloadForm.addChild(
              "input", new String[] {"type", "value"}, new String[] {"submit", l10n("unload")});
          ...
          reloadForm.addChild(
              "input", new String[] {"type", "value"}, new String[] {"submit", l10n("reload")});
        }
      }
    }
  }
  ...
  private List<OfficialPluginDescription> getNotLoadedPlugins(
      PluginManager pluginManager, List<OfficialPluginDescription> plugins) {
  ...
  private void showUnofficialPluginLoader(ToadletContext toadletContext, HTMLNode contentNode) {
    ...
    addOtherForm.addChild(
        "input", new String[] {"type", "name", "size"}, new String[] {"text", "plugin-url", "80"});
    addOtherForm.addChild("#", " ");
    addOtherForm.addChild(
        "input",
        new String[] {"type", "name", "value"},
    ...
    addOtherForm.addChild(
        "label", new String[] {"for"}, new String[] {"fileonly"}, " " + l10n("fileonly"));
  }
  ...
  private void showFreenetPluginLoader(ToadletContext toadletContext, HTMLNode contentNode) {
    ...
    addFreenetForm.addChild(
        "input", new String[] {"type", "name", "size"}, new String[] {"text", "plugin-uri", "80"});
    addFreenetForm.addChild("#", " ");
    addFreenetForm.addChild(
        "input",
        new String[] {"type", "name", "value"},
