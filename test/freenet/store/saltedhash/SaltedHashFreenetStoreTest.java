...
public class SaltedHashFreenetStoreTest {
	...
	public void testSimpleCHK() throws IOException, CHKEncodeException, CHKVerifyException,
		...
		SaltedHashFreenetStore<CHKBlock> saltStore = SaltedHashFreenetStore.construct(f,
				"testSaltedHashFreenetStoreCHK", store, weakPRNG, 10, false, SemiOrderedShutdownHook.get(), true,
				true, ticker, null);
	...
	public void testSimpleSSK() throws IOException, KeyCollisionException, SSKVerifyException,
		KeyDecodeException, SSKEncodeException, InvalidCompressionCodecException {
		File f = new File(tempDir, "saltstore");
		FileUtil.removeAll(f);
		...
		SaltedHashFreenetStore<SSKBlock> saltStore = SaltedHashFreenetStore.construct(f,
				"testSaltedHashFreenetStoreSSK", store, weakPRNG, 20, false, SemiOrderedShutdownHook.get(), true,
				true, ticker, null);
		saltStore.start(null, true);
	...
	private String decodeBlockCHK(CHKBlock verify, ClientCHK key) throws CHKVerifyException,
	...
	@Test
	public void testOnCollisionsSSK() throws IOException, SSKEncodeException,
	...
	private void checkOnCollisionsSSK(boolean useSlotFilter) throws IOException, SSKEncodeException,
		...
		SSKStore store = new SSKStore(pubkeyCache);
		SaltedHashFreenetStore<SSKBlock> saltStore = SaltedHashFreenetStore.construct(f,
				"testSaltedHashFreenetStoreOnCloseSSK", store, weakPRNG, 10, true, SemiOrderedShutdownHook.get(),
				true, true, ticker, null);
		saltStore.start(null, true);
		RandomSource random = new DummyRandomSource(12345);
	...
	private String decodeBlockSSK(SSKBlock verify, ClientSSK key) throws SSKVerifyException,
	...
	private ClientSSKBlock encodeBlockSSK(String test, RandomSource random) throws IOException,
